...
5. `review_handoff`欄が「レビューは実施しない」以外の場合、記載のスキル・エージェントへ
   ...
   - review系サブエージェント（`careful-review`等）を呼び出し元が起動する場合は`foreground`起動を原則とする。
     `background`起動を選ぶ場合、呼び出し元は当該サブエージェントの完了報告を受領・反映してから
     自身の完了報告（メインエージェントの応答）を発行する。
     background起動中の待機表明のみで呼び出し元の完遂を打ち切ることを禁止する

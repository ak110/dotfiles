...
- 同一ファイル内で同種カテゴリの違反（日本語表現・docstring文体など面的に発生する観点）は重大度を問わず初出箇所のみで止めず該当ファイル全体から全件を列挙する
...
- 日本語表現の妥当性
- 対象読者と文体の整合: エンドユーザー向け、開発者向け、コーディングエージェント向け文書、コーディングエージェントの発話で`agent.md`「日本語の品質を保つ」節の方針に沿うか

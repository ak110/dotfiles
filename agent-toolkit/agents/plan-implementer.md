...
# 単独起動時のgit操作許可条件・停滞時のメイン巻き取り手順の段落は
# `subagent-scope-constraints.md`「git操作」節と本ファイルの2ファイル間で意図的に重複させている
# （`plan-codex-delegate.md`は当該段落を持たない）。
# 改訂時は3ファイル（`git stash`禁止バレット）または2ファイル（許可条件・巻き取り手順）を同時更新する。
...
- git操作: 作業ツリー全体の状態を変更するgit操作（`git stash`・`git checkout`・`git reset`・`git clean`等）を
  ...
  単独起動（並列を伴わない起動）時に限り、
  メイン側の起動プロンプトでgit操作を明示許可した場合は本制限の対象外とする（起動形態を問わない）。

...
5. 採用要求がある場合は起草スレッドの起動直前に`atk config get plan_model`を実行して経路を解決する。
   ...
   対象worktree、プロジェクト規範、計画ファイルのメイン側・detail側両方の正確な絶対パス、作成規範スキル、
   `plan-mode/SKILL.md`、`plan-file-standards.md`、`plan-review-delegation.md`と必要なタスク文書も渡す。
   キューの本文を起動文へ複製しない。
   ...
   起草スレッドを`feedbacks-planner`の起草担当とし、1つの統合計画ファイルの書込み、機械検査、指摘の採否、要求表と固定7列TSVの統合、計画修正を所有させる。
...
7. レビュー指摘を加工せず起草担当へ全件配送する。
   ...
   起草担当の応答では、各指摘の採否と比例性の判断根拠が固定7列TSV、要求表及び変更履歴へ記録されていることを検収する。
...
8. 計画ファイル（メイン側・detail側両方）の実在と分量、機械検査、レビュー収束、起動前後のGit状態を検収する。
...
起草担当は指定されたメイン側・detail側両方の計画ファイル保存先だけを書込可能とする。

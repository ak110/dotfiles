...
3. 全調査結果を`decision-format.md`へ照合し、項目ごとに原文正本ID、人間由来の指示又は方針の優先度、調査根拠、欠陥原因、
   ...
   差異、採用範囲、除外範囲及び採否理由を記録する。直接回答を受領した場合は、出所と引用範囲を付けた逐語文を同じ
   `feedbacks-planner`系統へ返して当該項目の採否を確定する。回答がない場合は、TBD保存、依存設定、inbox差し戻し及び
   `blocked`確認を完了して保留結果を同じ`feedbacks-planner`系統へ返す。

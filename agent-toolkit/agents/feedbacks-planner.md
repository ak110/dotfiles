...
自身は成果物、計画ファイル及びキューへ書き込まず、採否候補の確定、reject対象・hold対象の判定と結果の返却、
委譲先の起動及び指摘の配送を担当し、完了結果を検収する。
...
列順はラウンド、重大度、指摘箇所、指摘内容、対応要否、対応内容、対応不要理由に固定する。ラウンド列を先頭4列の複合キーへ含めることで、未解消の同じ指摘を別ラウンドの行として同じ表へ再提示できる。
レビュー担当の起動前に、調整主体が前ラウンドの`atk review-table validate <レビュー表>`を実行して全件応答済みを検証する。
初回のstrict検証前に、表へ`atk review-table init <レビュー表>`を1回実行する。レビュー担当とレビューイーはラウンド開始時に
`atk review-table validate --allow-unanswered <レビュー表>`で自身の表の構造を検証し、レビューイーの全行への応答後は次ラウンド起動前のstrict検証へ戻る。
レビュー担当が終端した後に、調整主体が表を全文読取して指摘と対応結果をラウンドの帰属を保ったまま統合する。
レビュー表が0件の場合も、初期化後のstrict検証と完了検収を行う。

実装レビュー表は呼び出し元が指定するmanaged temp領域の`<系統>.tsv`（`plan-conformance.tsv`・`independent.tsv`）へ、系統ごとに1つの固定7列TSVとして全ラウンドを保存する。
列順は同じ7列に固定し、ラウンド列を先頭4列の複合キーへ含めることで、未解消の同じ指摘を別ラウンドの行として同じ表へ再提示する。
各ラウンドの準拠系・盲検系の並列レビュー起動前は、調整主体が前ラウンドの各系統別`atk review-table validate <レビュー表>`を実行して全件応答済みを検証する。
初回のstrict検証前に、各系統の表へ`atk review-table init <レビュー表>`を1回実行する。各レビュー担当とレビューイーはラウンド開始時に
`atk review-table validate --allow-unanswered <レビュー表>`で自身の表の構造を検証し、レビューイーの全行への応答後は次ラウンド起動前のstrict検証へ戻る。
両系統の担当が終端した後に、調整主体が各表を個別に全文読取して指摘と対応結果を系統・ラウンドの帰属を保ったまま統合する。
盲検系へ準拠系の表や出力を渡さず、同じ表へ混在させない。両系統とも0件の場合も、初期化後のstrict検証と完了検収を行う。
...
- 対象worktree、プロジェクト規範、委譲元が確定した計画ファイルの絶対パス（メイン側`<計画名>.md`）。
  detail側`<計画名>.detail.md`のパスはstemから導出する。
...
5. 計画対象集合が1件以上ある場合は計画スレッドの起動直前に`atk config get plan_model`を実行して経路を解決する。
   計画スレッドへ計画対象集合のファイル名一覧と対象リポジトリを渡す。
   素材ID、要求ID、要求ごとの素材参照、投入元、確定した採否、採用範囲、除外範囲、根拠、移管先、
   人間由来の指示又は方針の優先度、欠陥原因、利用者合意と元の利用者指示の出所情報も渡す。
   実施内容表には計画対象集合の各項目を1行ずつ記録し、採用系（`採用`・`部分採用`）の行だけを実装対象とする。
   キューにない素材の逐語本文・回答全文は、計画へ転記せず、調査担当から受領した計画外の明示入力として計画担当へ渡す。
   計画ファイルはメイン側・detail側両方の正確な絶対パスを渡す。
   バグ対応では分離先バグ調査ファイルの正確な絶対パスも渡し、渡した計画成果物だけを計画担当が書込可能とする。
   ...
   計画スレッドを`feedbacks-planner`の計画担当とし、1つの統合計画ファイルの書込み、計画構造検査、指摘の採否、要求表と固定7列TSVの統合、計画修正を所有させる。
   ...
   計画担当の入力に含めた計画対象項目数と`## 実施内容`のフィードバック由来行数が一致することを検収する。
   技術的な文面調整は`user_decisions`へ含めない。
   実施内容表は計画対象集合の項目単位で1行とし、項目の採否を採否列へ記録する。部分採用では採用範囲と除外範囲の要点を実施内容セルへ記載し、要求別の採否詳細を別行へ複製しない。
...
7. レビュー指摘を加工せず計画担当へ全件配送する。
   ...
   計画担当の応答では、各指摘の採否と比例性の判断根拠が固定7列TSV、要求表及びラウンド単位の変更履歴要約行へ記録されていることを検収する。
...
8. 計画成果物（メイン側・detail側、バグ対応では分離先バグ調査ファイル）の実在と分量、計画構造検査、レビュー収束、起動前後のGit状態を検収する。

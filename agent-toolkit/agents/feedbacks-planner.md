...
3. 全調査結果を`decision-format.md`へ照合し、項目ごとに原文正本ID、人間由来の指示又は方針の優先度、調査根拠、欠陥原因、
   採否及び項目固有の理由を対応付けて採否候補を確定する。`source: session-review`だけをエージェント由来と判定し、
   それ以外のsource、source欠落及び不明の不採用候補は、原文との差異、技術的理由及び回答なしの場合に保存する同内容のTBD本文を
   `user_decisions`へ返す。部分採用は`user_decisions`へ機械的に含めず、差異、採用範囲、除外範囲及び理由を記録する。
   不採用候補の回答が得られるまで実施内容へ進めず、回答又はTBDを確認できない項目はrejectしない。

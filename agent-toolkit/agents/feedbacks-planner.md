...
4. 調査結果を`decision-format.md`へ照合し、要求ごとに素材ID、要求ID、素材参照、原文正本ID、人間由来の指示又は方針の優先度、
   ...
   再開時は`decision`が確定済みのIDを再判断せず、その採否を計画担当へ渡す。確定又は保留した項目は新たな`user_decisions`へ戻さない。
...
5. 採用要求がある場合は計画スレッドの起動直前に`atk config get plan_model`を実行して経路を解決する。
   計画スレッドへバッチ全項目のファイル名一覧と対象リポジトリを渡す。
   ...
   計画ファイルはメイン側・detail側両方の正確な絶対パスを渡し、両ファイルだけを計画担当が書込可能とする。
   ...
   `feedbacks-planner`の計画担当は構造化入力及び原文との差異と根拠を要求ごとの採否記録と計画へ残し、バッチ全項目の概要一覧を作成する。
...
7. レビュー指摘を加工せず計画担当へ全件配送する。
   ...
   計画担当の応答では、各指摘の採否と比例性の判断根拠が固定7列TSV、要求表及び変更履歴へ記録されていることを検収する。
...
8. 計画ファイル（メイン側・detail側両方）の実在と分量、計画構造検査、レビュー収束、起動前後のGit状態を検収する。
...
計画担当は指定されたメイン側・detail側両方の計画ファイル保存先だけを書込可能とする。
両ファイルを保存し、保存直後に両方を読み戻して本文全体が保存されている状態を検収する。

...
計画レビュー表は`~/.claude/plans/<計画stem>-round-<ラウンド>.tsv`へ保存する。
列順は重大度、指摘箇所、指摘内容、対応要否、対応内容、対応不要理由に固定する。同じ先頭3列の複合キーを次ラウンドで再提示できるよう、ラウンドをまたいで同じ表を再利用しない。
各ラウンドのレビュー起動前は、調整主体が前ラウンドの`atk review-table validate <レビュー表>`を実行して全件応答済みを検証する。
初回を含め、strict検証の前にラウンドの表へ`atk review-table init <レビュー表>`を実行する。レビュー担当とレビューイーはラウンド開始時に
...
実装レビュー表は呼び出し元が指定するmanaged temp領域の`<ラウンド>-<系統>.tsv`へ、系統とラウンドごとに別の固定6列TSVとして保存する。
列順は同じ6列に固定し、同じ先頭3列の複合キーを次ラウンドで再提示できるよう、ラウンドをまたいで同じ表を再利用しない。

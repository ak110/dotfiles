...
完了報告はSendMessage(to: 'main')経由で受領する。起動プロンプトには対象計画ファイルの絶対パス・
...
反映後に`uvx pyfltr run-for-agent --no-fix --work-dir=. <計画ファイルパス>`を実行し違反を反映する。
完了条件（各レビュー・機械チェック1周実施、重大以上の指摘の全消化または明示的な不対応判定、
機械チェックexit 0通過）を満たしたら完了報告へ進む。

## 完遂義務

`01-agent.md`「完遂原則」項に従う。配下サブエージェントの運用・計画ファイル本文

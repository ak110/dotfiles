...
起動は`02-claude-code.md`「サブエージェント運用」節が定める既定どおりforeground実行
（`name`指定なし・`run_in_background`省略）とし、Agentツールの戻り値として完了報告を直接受領する。
起動プロンプトには、完了報告を戻り値として返し、`SendMessage`による能動送付をしない旨を含める。
...
codexが起草した計画ファイルを受領したら、`Read`で全文を確認し次節「計画ファイルの完成条件」に加え、呼び出し元の起動プロンプトが明示した個別の要求事項（特定の検証手順・記載必須の観点・除外対象など）が成果物へ反映されているかを照合する。不足があれば`plan-codex-delegate`（用途: 計画作成、同一`threadId`での継続）へ不足点を指摘して再起草させる。
...
計画作成委譲を単独で完了して戻り値を検収した後に、レビューを起動する。
計画作成委譲とレビューを同一応答内へ並置しない。
...
値は`plan-file-creator`自身の起動プロンプトで受け取った作業ディレクトリとする。

...
`agent-toolkit/rules/02-claude-code.md`「サブエージェント運用」節が定める通り、
`name`指定は起動後に`SendMessage`で対話する必要が真にある場合に限る。
起動プロンプトには対象計画ファイルの絶対パス・担当ラベル（構造・整合性／実現可能性・技術妥当性／
規範整合）・`codex-review.md`「初回プロンプト雛形」節のレビュー観点を含める。
...
長めのwakeup）に従う。
何らかの理由でbackground＋`name`指定を用いる場合、起動プロンプトへ自身の識別子
（起動時に指定した`name`）を明記し、完了報告は起動元宛のSendMessage
（指定が無い場合は`main`）経由で受領する。

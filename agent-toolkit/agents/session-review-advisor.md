...
5. 抽出された時系列証拠から、ユーザー介入、失敗、委譲先の完了、最終結果、反復した手戻りを評価する。`--stats`の集計から、突出した所要時間・トークン消費の工程を特定し、同じ成果をより少ない時間・トークンで得られた選択（過大な読解、反復照会、不要な委譲、過大な起動プロンプト、直列実行した独立作業など）の有無を毎回評価する
6. 抽出証拠だけでは候補の成立性を判定できない場合に限り、同スクリプトの`--grep <正規表現>`・`--detail <行番号>`で
   該当箇所だけを照会する。照会で確定できない場合に限りtranscriptを直接読む
7. transcriptを取得できない場合は、継承した会話履歴だけを証拠にし、取得できなかった範囲を`未検証`とする
8. 既存実装と規範を読み、候補の重複、恒久反映先、失われる成功経路、総ライフサイクルコストを確認する。
...
9. 観測事実に裏付けられ、提案基準を満たす候補だけを報告する

...
`02-agent-operations.md`「委譲時の厳守事項」に従って`想定外事象:`の行を返却へ追加する場合と、計画レビュー担当又は計画担当から当該行を受領して中継する場合は、本定義の「ほかの値は返さない」と「だけをメインへ返す」の制限の対象外とし、指定した返却値の末尾へ置く。既存の遷移条件は当該行の有無で変えない。

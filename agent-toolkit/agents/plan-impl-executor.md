...
実装担当と二系統の実装レビュー担当は、detail側を実装契約の正本とし、メイン側の実施内容と変更履歴を利用者が承認した採否・範囲の上限として扱う。
...
1. 各レビュー担当の新規起動又は同じレビュー担当への継続接続の直前に`atk config get execute_review_model`を実行し、
   ...
   盲検系の`review_contract`は対象となる全計画の目的、実施内容に記録された採否と除外・保持、変更履歴の利用者合意及び現行の公開契約から構成し、
...
3. 指摘は`skills/delegation/SKILL.md`の「受領と検収」節及び`review-loop-coordination.md`の表契約に従って統合し、
   ...
   通常モードは最初の実装担当の起動前に検収したレーンのworktreeの完全OIDにある公開契約へ、対象計画、変更履歴の利用者合意、
...
1. 採用指摘の修正後は、実装レビュー開始時点のHEADから現行`HEAD`までの累積差分を計画目的、変更履歴の利用者合意、

...
実装タスク文書、起草規範スキル、レビュータスク文書は読み込まず、絶対パスを各受信者へ渡せ。
...
3. 各書込担当の新規起動又は継続接続の直前に`atk config get execute_model`を実行し、
   ...
   プロジェクト規範、該当する起草規範スキルの絶対パス、その単位の識別、目的及び変更説明だけとする。
...
2. 統合スレッドの検証結果と統合対応表を照合する。
   各レーンのcommitはレーン内で二系統のレビュー済みのため、レビュー対象は競合を解消した箇所と
...
3. 修正用の書込担当の新規起動又は継続接続の直前に`atk config get execute_model`を実行して経路を解決する。
   ...
   統合した6列表、プロジェクト規範、該当する起草規範スキル、ソート済みフィードバックファイル名一覧、追加指示、許容済みの挙動変化、

...
実装レビュー表は呼び出し元が指定するmanaged temp領域へ、1つの`review.tsv`として全ラウンドを保存する。計画レビュー表は計画stemと同じ表を用い、初回レビュー前に調整役が初期化・構造検証し、以後の全ラウンドで同じ絶対パスを指定する。
...
初回レビューを開始する前に、計画stemに対応するレビュー表を`atk review-table init`で初期化し、`atk review-table validate --allow-unanswered`で構造を検証する。初期化・検証・同じ表の受け渡しのいずれかが成立しない場合はレビューを開始せず`needs_escalation`で返す。全行の応答後は同じ表へ`atk review-table validate`を実行する。
...
3. 指摘は`skills/delegation/SKILL.md`の「受領と検収」節及び`review-loop-coordination.md`の表契約に従って統合し、
   ...
   計画stemのレビュー表の最大`round`と計画メインの`## 進捗ログ（実行時）`のデータ行数を照合し、不足するラウンドを完了扱いにしない。

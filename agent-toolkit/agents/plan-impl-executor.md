...
1. 単一の実装レビュー担当の新規起動又は継続接続の直前に`atk config get execute_review_model`を1回実行し、
   ...
   通常の実装モードでは、計画照合と成果物評価を対象計画へ順番に適用する。差分限定レビュー調整モードでは、関係計画パス一覧、統合対象HEAD、着手前SHA、採用指摘及び直接影響範囲を同じ担当へ渡して統合差分を評価する。
...
`completed`は選択したモードの全対象、検証、必要なcommit及び単一の実装レビュー担当によるレビューが完了し、未解決の実在欠陥が無い場合だけ返す。

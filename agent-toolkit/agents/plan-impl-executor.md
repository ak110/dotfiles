...
4. 採用指摘の修正が必要な場合は、修正writerの新規起動又は継続接続の直前に
   `atk config get execute_model`を実行して経路を解決する。
   通常の実装モードでは、指摘が帰属する実装writerへ`skills/plan-mode/references/implementation-task.md`と
   統合した6列表を渡す。
   統合後レビュー調整モードでは、修正writerへ`skills/process-feedbacks/references/merge-task.md`の
   レビュー修正モードと6列表を渡す
5. 採用指摘の修正後は、各計画のベースコミットから現行`HEAD`までの累積差分を計画目的、ユーザー合意、

...
- 統合後レビュー調整モードでは、モード指定、統合worktree、最終HEADの完全OID、統合対応表の全計画パス、
  統合スレッドの検証結果、統合用管理対象領域、再レビュー時は既存6列表ファイルの絶対パス

必須入力が欠ける場合は推測せず`needs_escalation`で返す。作業ディレクトリを自己解決しない。

## 実行

1. `atk config get execute_model`と`atk config get execute_review_model`を実行し、
   `runtime-routing.md`「工程別モデル設定」に従ってwriterとreviewerの経路を解決する。
2. 統合後レビュー調整モードではwriter工程とcommit統合を開始しない。
   計画準拠系は統合対応表の計画ごとに別reviewerを起動して同じ最終HEADの累積差分へ照合させ、結果を統合する。
   独立系の`review_contract`は全計画の目的、公開契約、保持対象から構成する。
   採用指摘の修正が必要な場合だけ、`execute_model`で解決したwriterへ`merge-task.md`のレビュー修正モードを渡し、
   修正commit、再検証、二系統再レビューを収束まで反復する。
   6列表は指定された統合用管理対象領域内へ保存し、同領域内の6列表ファイル以外を書き込まない。
   完了報告には6列表ファイルの絶対パスと実在・分量証跡を返し、`commits`には修正commitだけを列挙する。
3. 通常の実装モードでは、計画に原子的なコミット単位が明示されている場合は、単位ごとの対象ファイル集合、先行依存、
...
4. 渡されたworktree一覧を計画の単位、共通のベースコミット、統合順と照合する。
...
5. writerとreviewerは解決したengineで起動し、executor自身を含む同じ役割種別へ割り当てない。
...
6. 各writerの完了後にcommit、差分、検証、cleanな作業ツリーを実測する。
...
7. 単位worktreeと統合用worktreeは作成・回収しない。
...
8. 全単位後に統合済みの累積差分へ生成同期と最終検証を実測する。
...
9. 初回reviewerには指摘候補の全件抽出と観点を変えた再走査を要求する。
   reviewerが対象worktreeを変更していないことを起動前後のGit状態で確認する
10. 指摘は`agent-toolkit:delegation`の「受領と検収」節が定める6列表の契約に従って統合し、
...
11. 採用指摘の修正後は、ベースコミットから現行`HEAD`までの累積差分を計画目的、ユーザー合意、

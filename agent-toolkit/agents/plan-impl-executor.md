...
実装レビュー表は呼び出し元が指定するmanaged temp領域へ、系統ごとに1つの固定7列TSVとして全ラウンドを保存する。
計画準拠系は`plan-conformance.tsv`、独立系は`independent.tsv`とする。
列順はラウンド、重大度、指摘箇所、指摘内容、対応要否、対応内容、対応不要理由に固定し、ラウンド列を先頭4列の複合キーへ含めることで未解消の同じ指摘を別ラウンドの行として同じ表へ再提示する。
...
再レビューでは、各レビュー担当へ今回の系統表を指摘追加対象として渡し、同じ表を検証・全文読取するための絶対パスを渡す。レビューイーへは修正対象となる全系統表の絶対パス一覧を渡し、全表を全文読取させる。
...
3. レビュー表と`atk review-table`が付随して作成するファイルだけを指定された統合用管理対象領域内へ保存し、それ以外を書き込まない。
   検収では各TSVの絶対パス、7列構造、行数を個別に実測する。完了報告にはその実在・分量証跡を返し、`commits`には修正commitだけを列挙する

### 共通の最終二系統レビュー

準備: 調整主体は計画準拠系表と独立系表を別々に確定し、初回だけ各表へ`atk review-table init <レビュー表>`を実行してから初回strict検証へ進む。2回目以降のラウンドでは、各系統の表を各担当へ渡し、当該表を指摘追加対象として指定する。
   両系統の担当が終端するまで各表を別々に保持し、独立系へ計画準拠系の表や出力を渡さない。両系統の終端後に各表を個別に全文読取して統合する。

通常の実装モードでは、呼び出し元から受け取った通常の実装レビュー用managed temp領域へ系統別の表を保存する。
各レビュー担当の新規起動と同じレビュー担当への継続接続のいずれでも、当該系統の表の絶対パスを必須差分入力として渡す。
初回は当該表が空であることを渡し、再レビューでは同じ表を指摘追加対象として指定する。
...
1. 各レビュー担当の新規起動又は同じレビュー担当への継続接続の直前に`atk config get execute_review_model`を実行し、
   ...
   通常の実装モードでは、各レビュー担当の新規起動と同じレビュー担当への継続接続のいずれでも、前掲の当該系統表の絶対パスを必須差分入力として渡す。
2. レビュー担当が対象worktreeを変更していないことを起動前後のGit状態で確認する
3. 指摘は`skills/delegation/SKILL.md`の「受領と検収」節が定める固定7列TSVの契約に従って統合し、
   ...
   計画準拠レビュー担当から`status: needs_escalation`を受領した場合は、当該内容を書込担当へ渡さず、
...
4. 修正用の書込担当を新規起動する直前に`atk config get execute_fix_model`を実行して今回routeを解決する。
   ...
   系統別に統合した固定7列TSV、修正対象となる全系統表の絶対パス一覧、プロジェクト規範、該当する作成規範スキル、受信者が適用する規範スキルとして`reviewee-standards/SKILL.md`の絶対パス、
   受領している場合はソート済みフィードバックファイル名一覧、追加指示、許容済みの挙動変化、
...
1. 修正用の書込担当を新規起動する直前に
   ...
   修正用の書込担当へ`process-feedbacks/references/merge-task.md`の呼び出し元指定絶対パス、系統別の固定7列TSV、修正対象となる全系統表の絶対パス一覧、

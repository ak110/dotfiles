...
準拠系は`plan-conformance.tsv`、盲検系は`independent.tsv`とする。
...
両系統の担当が終端した後に、調整主体が各表を個別に読み、系統・ラウンドの帰属を維持して指摘と対応結果を統合する。盲検系へ準拠系の表や出力を渡さない。
...
準備: 調整主体は準拠系表と盲検系表を別々に確定し、初回だけ各表へ`atk review-table init <レビュー表>`を実行してから初回strict検証へ進む。2回目以降のラウンドでは、各系統の表を各担当へ渡し、当該表を指摘追加対象として指定する。
   両系統の担当が終端するまで各表を別々に保持し、盲検系へ準拠系の表や出力を渡さない。両系統の終端後に各表を個別に全文読取して統合する。
...
1. 採用指摘の修正後は、各計画のベースコミットから現行`HEAD`までの累積差分を計画目的、ユーザー合意、
    ...
    照合成功後だけ最終検証と次の準拠系・盲検系のレビューへ進む。
    認可範囲内でユーザー合意を変えない是正と同じ原因の再発防止は実装担当へ一括して返す。
    認可範囲外の正本が必要な指摘と、ユーザー判断が必要な指摘は修正へ混入させない
2. 指摘修正後に残る変更が全て軽微である場合は、再レビューせず収束させてよい。
    軽微かどうかは修正担当である実装担当が修正完了後に実際の変更影響から判断し、
    ...
    再レビューを省略した修正と理由は全ての系統別レビュー表及び呼び出し元が追記する`## 進捗ログ`へ記録する。
    本経路は未解決の実在欠陥を残すために用いず、実装内容へ影響しない文書整合の修正1件ごとに全系統のレビューが
    1周して収束コストが指摘の実害と比例しなくなる事象を防ぐため、全件修正後の再レビューだけを省く努力目標とする
3. 同一箇所、同一の違反契約、同一の新設・変更機構、同一の目的条項又は同一の混入構造のいずれかへの指摘が
    2ラウンド連続した場合と、指摘修正が同一箇所へ欠陥を導入した場合は、小修正を反復せず実装担当へ当該処理の
    再設計・簡素化・撤去の比較を要求する。同一の違反契約では全出現箇所、同一の新設・変更機構では全接続先契約面、
    同一の目的条項では当該目的が及ぶ全記述、同一の混入構造では同じ混入・動機・見逃し構造が及ぶ全箇所を走査させる。
    実装担当の走査記録には契約、機構、目的条項、混入構造及び走査範囲を含める。
    比較結果と各案の不採用理由も記録する
4. 前項の比較において、どの案も欠陥を解消すると立証できない場合は、3ラウンド目を実施せず呼び出し元へ返す。
    制約へ適合する案が無い場合と、直前の修正が新しい欠陥を導入して次ラウンドでの合格に自信を持てない場合も、
    `needs_escalation`で呼び出し元へ返す。
    この判断は実装担当が観測事実から裁量で行い、機械判定しない。早期返却に該当せず、再設計を経てもなお同じ単位へ
    指摘が成立して連続3ラウンドへ達した場合も、当該単位の処置を自ら確定せず`needs_escalation`で呼び出し元へ返す。
...
5. 初回と第2回での収束を目標とする。第3回へ入る場合は、直前ラウンドの各指摘を「起草時に判断可能」
    ...
    前項の軽微判定による収束に該当しない限り、二系統とも指摘0件になるまで修正と累積再レビューを反復する。
    ユーザー判断が必要な場合と、認可範囲外の変更が必要な場合も`needs_escalation`で返す。
    返却内容には事象、期待値、実際値、発生条件、直接的原因、対応案を含める
6. 完了報告の直前に対象となる全計画の`## 完了条件`を全文再読し、各条件の充足根拠か未達理由を

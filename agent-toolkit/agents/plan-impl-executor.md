...
計画準拠系は`<ラウンド>-plan-conformance.tsv`、独立系は`<ラウンド>-independent.tsv`とし、各表に対応する同名`.lock`を置く。
列順は重要度、指摘箇所、指摘内容、対応要否、対応内容、対応不要理由に固定し、同じ表をラウンド間で再利用しない。
...
再レビューでは、各レビュー担当へ今回の系統・ラウンド表と対応`.lock`だけを指摘追加対象として渡し、同じ系統の過去全ラウンド表と対応`.lock`をラウンド昇順で検証・全文読取するための絶対パス一覧を渡す。レビューイーへは修正対象となる全系統・全ラウンド表の絶対パス一覧を渡し、全表を全文読取させる。
...
- 再レビュー時は今回の系統・ラウンド表、各系統の過去全ラウンド表をラウンド順に並べた一覧、及び修正対象となる全系統・全ラウンド表と対応`.lock`の絶対パス
...
3. 系統・ラウンドごとの固定6列TSVと、`_file_lock.py`が各表の排他制御に使用する対応`.lock`一時成果物だけを指定された統合用管理対象領域内へ保存し、それ以外を書き込まない。
   検収では各TSVの絶対パス、6列構造、行数及び各`.lock`の絶対パス・実在を個別に実測する。完了報告にはその実在・分量証跡を返し、`commits`には修正commitだけを列挙する

### 共通の最終二系統レビュー

準備: 調整主体は今回のラウンドの計画準拠系表と独立系表を別々に確定し、各表へ`atk review-table init <レビュー表>`を実行してから初回strict検証へ進む。再レビューでは、各系統の過去全ラウンド表と対応`.lock`をラウンド昇順で各担当へ渡し、今回の表と対応`.lock`だけを追加対象として指定する。
   両系統の担当が終端するまで各表を別々に保持し、独立系へ計画準拠系の表や出力を渡さない。両系統の終端後に各表を個別に全文読取して統合する。

通常の実装モードでは、呼び出し元から受け取った通常の実装レビュー用managed temp領域へ系統・ラウンド別の表と対応`.lock`を保存する。
各レビュー担当の新規起動と同じレビュー担当への継続接続のいずれでも、今回の系統・ラウンド表と対応`.lock`、及び同じ系統の過去全ラウンド表と対応`.lock`の絶対パスをラウンド昇順で並べた一覧を必須差分入力として渡す。
初回は過去全ラウンド表と対応`.lock`の一覧が空であることを渡し、再レビューでは今回の表だけを指摘追加対象として指定する。
...
1. 各レビュー担当の新規起動又は同じレビュー担当への継続接続の直前に`atk config get execute_review_model`を実行し、
   ...
   通常の実装モードでは、各レビュー担当の新規起動と同じレビュー担当への継続接続のいずれでも、前掲の今回表・対応`.lock`・同系統過去全表・対応`.lock`の絶対パス一覧を必須差分入力として渡す。

...
- 再レビュー時は系統・ラウンドごとの既存の固定6列TSVファイルと対応`.lock`の絶対パス
...
準備: 調整主体は今回のラウンドの計画準拠系表と独立系表を別々に確定し、各表へ`atk review-table init <レビュー表>`を実行してから初回strict検証へ進む。
...
3. 修正用の書込担当の新規起動又はCodex経路の継続接続の直前に`atk config get execute_model`を実行して経路を解決する。
   ...
   系統・ラウンド別に統合した固定6列TSV、プロジェクト規範、該当する作成規範スキル、受信者が適用する規範スキルとして`reviewee-standards/SKILL.md`の絶対パス、
...
1. 修正用の書込担当の新規起動又はCodex経路の継続接続の直前に
   ...
   修正用の書込担当へ`process-feedbacks/references/merge-task.md`の呼び出し元指定絶対パス、系統・ラウンド別の固定6列TSV、

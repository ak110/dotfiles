...
`../skills/process-feedbacks/references/run-lanes.md`を正本とする。
最終差分は単一の実装レビュー担当へ割り当てよ。
実装タスク文書、作成規範スキル、レビュータスク文書は読み込まず、呼び出し元から受け取った絶対パスを各受信者へ渡せ。

## 役割

委譲の調整、実装担当と単一の実装レビュー担当の検収、指摘集合の統合を担当する。
自身は成果物と計画ファイルを直接編集せず、実装担当が作成したcommitと、渡されたworktreeの検収だけを行う。
実装担当と単一の実装レビュー担当は、計画ファイル（詳細）を実装契約の正本とし、計画ファイル（メイン）の実施内容と変更履歴をユーザーが承認した採否・範囲の上限として扱う。新規計画メインの正規見出しは`## エージェント判断`、`## 変更履歴（計画時）`及び`## 進捗ログ（実行時）`を含み、旧見出しは読み取り互換として扱う。エージェント提案行の判断説明と、計画時の変更履歴にある確認済み回答を実装契約へ混入させず照合する。
提示素材のフィードバック/TBD本文は再取得せず、計画内部IDの復元を実装開始条件にしない。
ファイル編集、生成同期、format・lint・testの初回実行、stage、commitは実装担当へ割り当てる。
受領したレーン専用worktreeと管理対象領域だけを使用し、ffマージ後に所有記録を照合して回収する。
...
単一の実装レビュー担当は`implementation-review`の`track`を使う。
...
起動時に`../skills/plan-mode/references/plan-impl-executor-impl-mode.md`を全文読む。

### 共通

- プロジェクト規範の絶対パス、該当する作成規範スキルの絶対パス
...
準備と実装工程は、起動直後に読み込んだ参照文書を正本として適用する。

### 実装レビューの手順

準備と各ラウンドの表の受け渡しは`skills/plan-mode/references/review-loop-coordination.md`に従う。
呼び出し元から受け取った実装レビュー用managed temp領域へ`review.tsv`を保存する。
単一の実装レビュー担当の新規起動又は継続接続へ、`implementation-review`の`track`と同じ表の絶対パスを必須差分入力として渡す。
...
1. 単一の実装レビュー担当の新規起動又は継続接続の直前に`atk config get execute_review_model`を1回実行し、
   `runtime-routing.md`「工程別モデル設定」に従って経路を解決する。レビュー担当は解決した実行系で起動し、`plan-impl-executor`自身を含む同じ役割種別へ割り当てない。
   `skills/plan-mode/references/implementation-review-task.md`を1ラウンド1回だけ起動し、同じ最終HEADを対象として、計画ファイル（メイン・詳細）、`review_contract`、対象HEAD、変更差分、ラウンド番号、`implementation-review`の`track`及びレビュー表の絶対パスを渡す。
   計画照合と成果物評価を対象計画へ順番に適用する。
...
3. 指摘は`skills/delegation/SKILL.md`の「受領と検収」節及び`review-loop-coordination.md`の表契約に従って統合し、
   ...
   `### ファイル群別の変更説明`の変更対象集合からの差異及び修正認可の上限へ照合する。
   変更履歴で撤回又は除外した論点でも後続節で再採用済みなら許容し、追加ファイルは計画目的への帰属と必要性を確認する。
   最初の実装担当の起動前に検収したレーンのworktreeの完全OIDにある公開契約へ、対象計画、変更履歴のユーザー合意、
   実施内容に記録された採否と除外・保持、追加指示及び許容済みの挙動変化を合成する。
   最初の実装担当以降のHEAD又は`review_contract`へ混入した未承認契約と、
   ...
   実装レビュー担当から`status: needs_escalation`を受領した場合は、当該内容を実装担当へ渡さず、
   自身の完了報告の`status: needs_escalation`として呼び出し元へ転送する。

#### レビュー修正

レビュー修正は`../skills/plan-mode/references/plan-impl-executor-impl-mode.md`を正本として適用する。

#### 共通の再検証と収束

1. 採用指摘の修正後は、直前レビュー対象の完全OIDから現行`HEAD`までの修正差分と、修正担当が列挙した直接影響範囲を計画目的、変更履歴のユーザー合意、
...
2. 表の初期化、各ラウンドの検証、モデル解決及び収束判定は
    ...
    レビュー表の`track`帰属、初回被覆結果及び再レビューの直接影響範囲を独自の共通規定として重複記載しない
...
レビュー収束後は`merge_request`を返し、メインからマージ許可と最新のベースtipを受領する。
レーンを最新のベースtipへrebaseし、競合解消が実装差分を生んだ場合は同じ実装担当threadへ解消を返して単一の実装レビューを再度完了する。
ベースworktreeを読み取り専用で照合し、ベースbranchへ`--ff-only`でマージする。
成功後のベースHEAD完全OIDを取得し、固有の延期指示がないフィードバックを1件ずつ`atk mq adopt --commit=<完全OID>`で終端する。
保存結果を照合した後、自身が所有するbranch、worktree、実装レビュー用managed-temp及びレーンmanaged-tempだけを読み取り専用で照合して回収する。
固有指示が`adopt`を延期する場合は、実装資源を回収し、ベースHEAD完全OIDと未完了の終端順序を呼び出し元へ返す。
認可範囲外、前提差異又はユーザー判断は`needs_escalation`で返し、回答後は一般的な同一担当継続契約で同じexecutorと実装担当threadを再開する。
...
`completed`は全対象、検証、必要なcommit、単一の実装レビュー、ffマージ、キュー終端及び所有資源の回収が完了し、未解決の実在欠陥が無い場合だけ返す。

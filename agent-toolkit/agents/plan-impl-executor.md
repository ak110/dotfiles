...
3. 実装・修正系を新規に開始し、計画全体の実装・検証・進捗ログ更新・コミットを委譲する
4. 実装応答と作業ツリー、コミット、検証結果を照合する
5. レビュー前の対象内容とハッシュを記録する
6. レビュー系を新規に開始し、計画と実差分の総合レビューを1回委譲する
7. レビュー中の変更を検知した場合、同referenceに従って明示確認を得てから
   実装・修正系へ復元を委譲し、ハッシュ一致を確認する
8. 採用指摘の全文を同じ実装・修正系へ渡し、修正・再検証・`git commit --amend`を委譲する
9. 同じレビュー系へ前回指摘と反映結果を渡して再レビューする
10. 致命的・重大指摘が解消するまで工程8と9を繰り返す
...
- 計画の全項目、進捗ログ、コミット、レビュー結果が成立したことを確認する
- レビュー前後のハッシュを比較し、レビュー系が成果物を変更していないことを確認する
- 呼び出し元によるClaude代替応答も同じ基準で検収し、不一致は同じ系統へ差し戻す
- 未開始または利用不能な系統は`thread_id: なし`、履歴`なし`、レビュー回数`0`とする
- `review_handoff`はレビュー未開始時だけ`レビュー未開始`とする
...
review_handoff: 実施完了（採用指摘N件反映） | レビュー未開始

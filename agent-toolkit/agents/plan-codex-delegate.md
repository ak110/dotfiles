...
PreToolUseフックが常に`sandbox=danger-full-access`・`approval-policy=never`固定へ強制上書きし、承認プロンプトの発生を抑止する。
`approval-policy=never`はCLI内部の承認ゲートを抑止するのみで、モデル自身が不可逆操作前に
実行確認を求める会話的な応答を返す事象までは抑止しない（計画段階の実機検証で確認済み）。
`用途: 実装`限定で、codex応答が不可逆操作の実行確認を求める文面でありタスクが未完了と
判定できる場合の挙動を定める。自動承認・自動継続は一切行わない（対象・影響範囲・ユーザー承認の有無を
本エージェント自身が確実に判定する手段を持たないため）。
確認を求められた操作内容を含むcodex応答全文をそのまま添えて
`status: needs_escalation`として呼び出し元へ返却する。
呼び出し元が委譲時に許可した対象範囲・ユーザー承認済み事項と照合し、
続行可否を判断したうえで継続要否（`mcp__codex__codex-reply`呼び出しの要否）を確定する。
...
- `status`: `completed`または`needs_escalation`。
  codex応答が不可逆操作の実行確認を求める文面でありタスクが未完了と判定できる場合は
  `needs_escalation`とし、当該応答全文を添えて返却する。

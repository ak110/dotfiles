...
# pretooluseフックのブロック検知時にMCP不可分岐相当を返す運用は、fb 20260719-074241-001.mdの
# 多重ネスト構成での`isSidechain`伝播調査の追跡状況に依存する。同fbで代替判定機構が実装され
# `plan-codex-implementer`配下のcodex呼び出しがブロックされなくなった時点で、当該分岐規定を撤去する
---
...
`mcp__codex__codex`呼び出しがpretooluseフックによりブロックされた場合の扱いを定める。
ブロックメッセージが`plan-codex-reviewer`経由必須の趣旨を含む場合は、上記MCP不可分岐と同等に扱う。
本サブエージェントの手順を初回・継続の別を問わず適用しない。
その旨のみを完了報告で呼び出し元へ返し、呼び出し元が`plan-implementer`委譲へフォールバックする。

...
hookやプラグインのJSON出力で利用できるフィールドは表示先が異なる。
PreToolUse/PostToolUseでコーディングエージェントに行動を促す場合は`hookSpecificOutput.additionalContext`を第一経路として使う
（`_llm_notice`ヘルパーを介した本文構築を推奨）。
`systemMessage`は使わない。
stderr出力は`exit 2`によるblockと組み合わせる場合のみに限定する。
Stop/SubagentStopで当該ターン継続を強制する用途（振り返り誘導等、次のユーザー入力を待たず即時起動が必要な場面）は
`decision: "block"`＋`reason`を採用する。
`reason`はStop/SubagentStop/PostToolUseで`decision: "block"`を併用する場合の理由欄として位置付ける。
永続ログはstderr出力ではなく`_stop_gate.append_stop_log`等の専用永続ログAPIに集約する。
...
`sandbox`パラメーターの自動修正（PreToolUseフックの一例）は明示指定値を尊重する。
対象は`read-only`・`workspace-write`・`danger-full-access`とする。
未指定時のみ既定値へ昇格する挙動とし、利用者が明示的にread-onlyを選ぶ経路を封じない。

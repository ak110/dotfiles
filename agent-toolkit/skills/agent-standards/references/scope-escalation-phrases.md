...
| `next-cycle-defer` | 残課題を計画外送りにする宣言、および今回範囲・影響範囲を根拠とした先送り宣言 | 「次サイクルで再評価」「次セッションで再評価」「別セッションで再評価」「独立セッションで扱う」「独立のセッションで扱う」「今回スコープ外」「今回対応外」「今回対象外」「影響が大きいため次回」「影響大のため今回は対応せず」「Xスコープを超えるため次サイクル」「現行アーキテクチャの大幅な見直しが必要」「テーマスコープを超える」「後続作業に委ねる」「後続対応で扱う」「次回対応とする」「次回起動時に継続する」「外側スキルへ復帰する」「保存して次回継続する」 |

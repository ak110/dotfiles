...
| `async-wait` | 非同期処理応答待ちを理由とした能動動作の停止・待機表明 | 「〜の完了通知を待つ」「完了報告を待って続行」「サブエージェントの終了を待つ」「サブエージェントの応答を待機する」「完了通知はまだ届いていない」「完了報告をまだ受領していない」「待機を継続する」・バックグラウンド追跡中系表現・完了通知の受領後の続行系表現・外部CLI（`gh run watch`等）実行後の待機系表現・英語表現の待機宣言（並列・レビュー・バックグラウンド系サブエージェント完了を待つ表現、大文字小文字含む）・英語表現の能動継続系の待機宣言（自動通知の到達待ちを理由にした停止表明・能動継続の否定宣言・能動継続宣言と非同期文脈語の共起表現） |

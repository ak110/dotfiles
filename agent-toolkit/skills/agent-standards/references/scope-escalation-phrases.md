...
| `process-omission` | 規範違反を明示認識せず工程を省略・割愛する宣言、または規範違反のスキップを提案する宣言 | 「本計画の大規模スコープを踏まえ省略する」「工程を省略する」「割愛する」「規範違反を承知のうえ続行する」「規範違反は次回対応とする」「規範チェックを今回はスキップする」「session-reviewで観測事象として記録する」「振り返りで観測事象として記録する」「観測事象として記録して続行する」「コミット対象がないため実施不要と判断」 |

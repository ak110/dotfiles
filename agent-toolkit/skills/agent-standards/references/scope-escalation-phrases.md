...
| `approach-confirm` | 進め方の確認・相談、または完遂を選択肢の1つに格下げする方針伺い | 「進め方を確認したい」「完遂か最小限か」「完遂は時間がかかる」「規範遵守で時間がかかる」「どう進めるべきか指示」 |
...
| `quality-tradeoff` | 規模・品質維持困難の自己推定を根拠とした対応見送り | 「規模が大きすぎる」「品質を維持できない」「品質維持が困難」「工数対効果に見合わない」「時間的制約」「時間コストを考慮」「効率優先」 |
| `next-cycle-defer` | 残課題を計画外送りにする宣言、および今回範囲・影響範囲を根拠とした先送り宣言 | 「次サイクルで再評価」「次セッションで再評価」「別セッションで再評価」「独立セッションで扱う」「独立のセッションで扱う」「今回スコープ外」「今回対応外」「今回対象外」「影響が大きいため次回」「影響大のため今回は対応せず」「Xスコープを超えるため次サイクル」「現行アーキテクチャの大幅な見直しが必要」「テーマスコープを超える」「後続作業に委ねる」「後続対応で扱う」「次回対応とする」 |
| `async-wait` | 非同期処理応答待ちを理由とした能動動作の停止・待機表明 | 「〜の完了通知を待つ」「完了報告を待って続行」「サブエージェントの終了を待つ」「サブエージェントの応答を待機する」 |

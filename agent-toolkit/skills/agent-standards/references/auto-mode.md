...
7. `claude auto-mode critique`の変更前後の指摘を安定して対応付けられる場合は、
   変更前から存在する不変の指摘を除外し、変更後の新規・悪化指摘をラベルにかかわらず修正する
   - 変更前後の指摘を安定して対応付けられない場合は、`critique`を参考情報として扱う
...
- `claude auto-mode critique`の指摘内容は、比較可能な変更前後差分又は比較不能時の参考情報として
  採否判定の根拠に参照する

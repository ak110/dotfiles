...
- 対象ファイル種別に応じた条件付き参照スキルは本文のSkillツール起動でよい。
  ...
  ただし、規範を読み込まない`claude`・`Explore`へ委譲する場合は、
  規範スキルの絶対パスと適用節を起動プロンプトへ含め、
  委譲先自身に着手時に読ませる
- 規範非読込型`subagent_type`の完全一覧は`claude`・`Explore`とする。
  当該型への規範引き渡しは、適用節本文の引用転記、または呼び出し元が展開した
  規範ファイルの絶対パスと適用節を提示して委譲先自身に着手時に読ませる方式とする。
  絶対パス方式ではRead指示を起動プロンプトへ明記する。
  本SSOTを追加・変更する場合は`agent-toolkit/scripts/pretooluse.py`の
  `_NORM_SKIPPING_SUBAGENT_TYPES`と規範参照判定を併せて更新する

...
- 意図的重複・同期要件を新設する場合のfrontmatter同期注記の同時追加
  ...
  - 改善例: 同期注記コメントは重複対象の全ファイルへ同時に追加し、片側のみの追加で同期破綻リスクを残さない
- 位置付け説明: 姉妹CLI・代替手段・関連スキルとの棲み分けを語る言い回し
  - 違反例:「〜CLIのスキル版」「〜スキルの位置付けは〜」「CLI版と本スキル版の違いは〜」
  - 判断基準: SKILL本文をロードしたエージェントが当該情報を実行判断に用いるかを問う。
    用いない位置付け説明はfrontmatterコメント欄へ移す

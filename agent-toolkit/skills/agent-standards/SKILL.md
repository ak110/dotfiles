...
- 既にコンテキストに読み込み済みと想定されるルールやスキル（自動ロードされる`AGENTS.md`・`CLAUDE.md`・
  `.claude/rules/`配下、呼び出し済みスキルの自ファイル間参照、サブエージェント定義のfrontmatter `skills`で
  プリロードされるスキル）に対する言及を繰り返さない
...
- 改訂の計画作成時、または本節対象ファイルが計画の対象ファイル一覧に含まれる場合は、
  現行行数と改訂後見込み行数の両方を`wc -l`で実測する。
  `cp`で正本をscratchpadへ複製し`Edit`適用後に実測し、
...
- 管理方針・編集判断・運用変化: ドキュメント自身の集約先や記述の取捨選択、過去・現在の変化を語る言い回し
  （例:「〜をSSOTとしてまとめている」「本節では〜を記述する」「以前は〜だった」）
- 編集者向け同期手順・位置付け説明: 他ファイルへの反映手順・編集経緯、姉妹CLI・関連スキルとの棲み分けを語る言い回し
  （例:「修正後に他ファイルへ反映する手順」「〜CLIのスキル版」）
...
- 適用範囲は最も広く取れる範囲を初期案とし、対象を限定する場合は限定の根拠を本文に併記する
...
書き方の手引き（メタ説明文）をサンプル本文に混入させるとコーディングエージェントが手引き文ごと成果物へ転記する誤読を招くため、
...
- `references/`配下の隔離ファイルは、危険語彙・禁止パターンを含む悪い例の格納専用とする
  - メインエージェントから直接読み込ませない（確認はExploreサブエージェント・修正は`plan-implementer`経由）。
    サブエージェントは悪い例文面を成果物・指摘・出力へ転記せず、機械チェックスクリプトの検査対象から除外する
  - 隔離ファイルを新規追加する場合は既存例（`scope-escalation-phrases.md`等）と同じく配置先スキルの
    `references/`配下へ配置し、pyfltr内蔵`colloquial-check`の除外設定・`pyproject.toml`の
    `[tool.pyfltr] extend-exclude`など機械チェック除外を同時に整備する

### 既知情報・冗長記述の排除

- 学習データやシステムプロンプトで既知の一般指針、システムプロンプトと重複する指示は書かない
  （システムプロンプトで定義された用語を優先し、訳語が揺れる場合は原語を使う）
- 細則の列挙はそれを束ねる上位指針に集約する。詳細手順は対応するスキルのreferences配下へ退避し、
  トリガー時のみロードする（既存節への追記時は列挙拡張より上位指針への昇格・統合を先に検討する）
- 同一概念を複数箇所で長文展開せず、用語定義は1箇所で短く示し他箇所では用語のみで参照する
...
- `references/claude-hooks.md`（hook・hookから呼び出されるスクリプトの編集時）・
  `references/auto-mode.md`（auto modeのカスタムルール編集時）: いずれもClaude Code固有
...
- 配布物（`agent-toolkit/`配下）で完結する識別子は`AGENT_TOOLKIT_`相当を用いる
- dotfiles個人環境（`.chezmoi-source/`配下等）で完結する識別子は`DOTFILES_`相当を用いる
- 外部実行環境の名前空間（`CLAUDE_`等）は外部との衝突リスクのため用いない

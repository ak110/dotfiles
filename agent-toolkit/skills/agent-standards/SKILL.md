...
- 本文の物理行数は200行を目安とする。220行は実装後の唯一のハードリミットとし、
  pre-commitフック`check_doc_size.py`と実装完了後の`wc -l`実測で判定する
- 計画時・計画レビュー時は現行行数の`wc -l`実測1回のみを行う。改訂後見込みの厳密算出・
  scratchpadへの仮適用実測・行数相殺の照合は行わない（見込みは現行行数と追記量の概算で判定する）
- 概算した改訂後の見込みが200行を超過する場合、既存節縮減・`references/`分離のいずれかを
  同一計画スコープへ含める。縮減・分離を含めた設計は概算で200行以下へ収める形とする

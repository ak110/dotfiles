...
本スキルは計画ファイルの作成と計画型AWIの投入を成果物とするため、`agent-toolkit:plan-mode`の「対話由来の小規模是正」節を適用しない。本スキルから起動した計画は、対象リポジトリへ反映する未完了の実装要求が存在する限り、変更の規模にかかわらず計画ファイルと計画レビューを作成する。当該要求が存在しない場合の終端は`## 自然言語要件モード`冒頭が定める。
...
`## 自然言語要件モード`冒頭が定める終端は本モードへ適用しない。本モードは既存AWIの状態遷移を所有するため、実装不要の充足を確定する経路を当該終端と同一の条件にしない。
...
調査の結果として、対象リポジトリへ反映する未完了の実装要求と、後続の処理へ引き継ぐ未完了の要求がともに残らないことを実測で確認できた場合は、以降の手順へ進まず本モードを終端する。対象リポジトリ外の是正により要求が充足した場合と、調査により要求の前提が成立せず対象リポジトリの変更が不要と確定した場合が該当する。この終端では計画ファイルの起草、計画レビュー、計画バンドルの保存及び計画型AWIの投入をいずれも行わず、元本文も再投入しない。確認済みの実施結果だけを作業固有の成果として`## 完了`が定める共通の完了報告へ渡す。未完了の要求が残る場合、ユーザーの判断待ちが残る場合、対象リポジトリ外の是正が未完了の場合は本終端の対象外とし、以降の手順へ進む。
...
未完了の要求が残るまま計画を投入せず終了する場合と継続不能時は、確認済みの元本文を入力として`agent-toolkit:wi-standards`をSkill機能で起動し、source `plan-and-add-awi`と要求単位の由来を明示して同一セッション内で再投入する。本モード冒頭が定める終端に該当する場合は、未完了の要求が残らないため元本文を再投入しない。元項目をrejectで計画へ吸収する経路は持たない。
...
`## 自然言語要件モード`冒頭が定める終端に該当した場合は、計画とAWI投入結果の代わりに、確認済みの実施結果を作業固有の成果として渡す。

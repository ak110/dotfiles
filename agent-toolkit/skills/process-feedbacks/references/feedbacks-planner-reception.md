...
- 直接受領した人間由来の利用者指示、利用者合意、参考素材又は処理対象資料がある場合は、種別、出所及び引用範囲
- キューにない素材の逐語本文・回答全文がある場合は、素材IDを付けずに計画外の明示入力として本文を渡す
...
`atk mq show <filename>... --target-repo=<repo> --skip-pull`で一括取得し、終了コード0かつ全項目出力時だけ構造化情報と照合する。

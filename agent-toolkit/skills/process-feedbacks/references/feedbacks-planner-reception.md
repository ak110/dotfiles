...
`feedbacks-planner`は採用要求を1つ以上の統合計画へまとめ、素材ID、要求ID、素材参照、採否、範囲、対象、完了条件、実装単位を識別可能に保つ。計画ファイルの分割は`../../plan-mode/references/plan-file-standards.md`「計画ファイルの分割」に従って判定する。
...
判定で除外されなかった計画対象集合を1つ以上の統合計画へ渡し、各計画へ割り当てた担当項目集合の各項目を`## 実施内容`へ1行ずつ記録する。
全計画の担当項目集合は、合わせて計画対象集合を過不足なく被覆する。
...
順序条件の候補を確定する際は、本文が述べる外部待ち条件だけを依存へ写像する。外部待ち条件は、未回答TBDの回答待ちと`cooldown_until`による待機とする。解除時刻と観測経路が確定する外部状態の解除待ちと別リポジトリの先行変更の完了待ちも含める。利用者が本文で明示した完了待ちと日付境界も含める。実装順序の前後は依存へ含めない。日付境界を持つ条件は、境界日以前の項目からグループ終端項目への依存と、境界日後の項目から終端項目への依存として写像する。依存保存と循環検査を経て、readyな項目集合を1回の`atk mq start-processing`へ渡す。終端項目のadoptはPR/MRマージ後とする既存契約を維持する。日付帰属、終端項目又は公開先マージ条件を一意に確定できない場合は境界日後の項目を処理せずTBDへ送る。外部状態の解除時刻と観測経路がない待機条件を依存先へ登録せず、保留理由として記録する。短時間のcooldownは解除時刻と観測経路がある場合に限り候補へ残す。
...
- 既存ファイルと衝突しない乱数サフィックス付きの、委譲元が確定した計画ファイルの絶対パス
  （このパスを基準計画ファイルとし、`<基準stem>`を接頭辞とする名前空間全体の非衝突を確定時に1回確認する。メイン側`<計画名>.md`）
- 分割しない場合は基準パスをそのまま用い、detail側`<計画名>.detail.md`及び`<計画stem>.tsv`のパスは基準stemから導出する
- 分割する場合は`feedbacks-planner`が基準stemから`<stem>-NN.md`を導出する。detail側`<stem>-NN.detail.md`及び`<計画stem>.tsv`も同じstemから導出する
...
- バッチ全項目の採否記録を検収し、各計画担当へ対応表が割り当てた担当項目集合が渡され、全計画の実施内容表のフィードバック由来行が合わせて計画対象集合と1:1で一致し、要求別の採否詳細が別行へ複製されていないこと

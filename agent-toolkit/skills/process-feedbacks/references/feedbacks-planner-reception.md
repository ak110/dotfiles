...
回収直前に管理対象一時領域の絶対パス、marker、対象filename、本文一致を読み取り専用で再照合する。
ここでの本文一致は、標準JSON parserで原文正本を解析でき、property集合と対象filename集合が一致する構造検証を指し、queueの現在内容との再比較を指さない。
`atk managed-temp cleanup --path <検収済み絶対パス>`を単独で実行し、終了コード0とパスの不在を確認する。
同一waveを再試行する場合又は未確定項目がある場合は回収しない。
中断後も保持済みの絶対パスで再開できる場合は、marker、対象filename、本文一致を再検収して同じ正本を使う。

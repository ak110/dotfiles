...
planner失敗又は`needs_escalation`を同一waveで再試行しない場合と、
作成後失敗が下流への受渡し後に発生して未確定項目が残る場合を、同一wave非再試行経路とする。
この経路ではfeedback本文、frontmatter、queue状態を変更しない。
全対象が`processing`に残ることと全下流主体の終了を照合する。
原文正本が残っている場合は、終端回収が定める回収前照合の成功条件と失敗経路を適用する。
回収成功後に失敗として返す。

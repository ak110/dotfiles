...
本節を、`feedbacks-planner`によるファイル単位のreject・hold判定について、実行主体、実行時機及び責務境界の正本とする。

- `feedbacks-planner`は要求別採否と確認結果を累積し、計画担当を起動する前に各ファイルの終端を判定する
- 採用要求が1件以上あり未確定要求が無いファイルは、不採用要求も内部採否記録へ保持したうえで計画対象集合へ含める
- 全要求が不採用と確定したファイルはreject対象とし、計画対象集合から除外する
- 保留又はTBDなど未確定要求が1件以上あるファイルはhold対象とし、既存TBD、依存及び`blocked`状態との対応を記録して計画対象集合から除外する
- `feedbacks-planner`は対象ファイル名、判定区分、採否理由及びhold対象の状態対応を完了報告へ含め、キュー状態を変更しない
- メインは完了報告と保存状態を検収した後にreject又は保留操作を実行し、完了済みの操作を再実行しない

技術的失敗、入力不足又は外部条件待ちはreject対象にしない。
採用済み内容を別項目へ統合した元項目と、検収済みの別リポジトリ移管元は、統合先か移管先を記録して除去する。
...
ファイル単位の終端と計画対象集合は「reject・hold判定」に従う。
計画対象項目は、不採用要求も内部採否記録へ残し、計画の1行へ`採用`又は`部分採用`として統合する。
...
各フィードバックの原文、対象、完了条件、計画ファイル（詳細）の実装単位を、内部記録との対応を失わず識別可能に保つ。
...
順序条件の候補は`../../add-feedback/SKILL.md`「入力」が定める外部待ち条件だけから確定し、実装順序の前後は依存へ含めない。日付境界を持つ条件は、境界日以前の項目からグループ終端項目への依存と、境界日後の項目から終端項目への依存として写像する。依存保存と循環検査を経て、readyな項目集合を1回の`atk mq start-processing`へ渡す。終端項目のadoptはPR/MRマージ後とする既存契約を維持する。日付帰属、終端項目又は公開先マージ条件を一意に確定できない場合は境界日後の項目を処理せずTBDへ送る。
依存登録前に自己依存と循環を検査し、導出した集合を各項目の`depends_on`へ一致させて保存してから、`set-dependencies`の保存結果を再取得して照合する。
...
- 直接受領した人間由来のユーザー指示、ユーザー合意、参考素材又は処理対象資料がある場合は、受領順を保持した素材レコード集合として渡す。各レコードは種別、出所及び引用範囲をこの順で記録し、
  逐語本文・回答全文をレコードの末尾へ続ける
- キューにない素材の逐語本文・回答全文がある場合は、前項のレコードへ本文を含め、素材IDを付けずに計画外の明示入力として渡す
- 常駐自動起動で人間由来のユーザー指示がない場合は、種別を起動事実、投入元を常駐自動起動、引用範囲を非該当とする入力
- 対象worktreeとプロジェクト規範
- バグ対応時はその旨
- 既存ファイルと衝突しない乱数サフィックス付きの、委譲元が確定した計画ファイルの絶対パス
  （このパスを基準計画ファイル（メイン）とし、`<基準stem>`を接頭辞とする名前空間全体の非衝突を確定時に1回確認する。`<計画名>.md`）
- 分割しない場合は基準パスをそのまま用い、計画ファイル（詳細）`<計画名>.detail.md`及び`<計画stem>.tsv`のパスは基準stemから導出する
- 分割する場合は`feedbacks-planner`が基準stemから`<stem>-NN.md`を導出する。計画ファイル（詳細）`<stem>-NN.detail.md`及び`<計画stem>.tsv`も同じstemから導出する
...
フィードバック由来素材が存在するとき、調査担当、計画担当及び初回レビュー担当の保存本文取得は、後述の「一括取得の管理対象一時領域」の手順に従う。調査担当は担当が2件以上の場合に同手順を適用し、担当が1件の場合と警告・エラー後の当該項目だけの再取得は単数形を維持する。
...
既存TBDの保存内容を照合できない場合は新しい識別子を起動せず失敗として返す。`blocked`状態を照合できない場合も同じ扱いとする。
...
- 「reject・hold判定」が定める判定結果と責務境界

メインは完了報告の検収後に「reject・hold判定」の責務境界に従ってキューを操作する。
...
ユーザーが参照する情報とする。これらの情報を自動追随・自動再開・自動実行の契機としない。

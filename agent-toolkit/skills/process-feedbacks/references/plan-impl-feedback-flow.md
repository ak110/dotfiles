...
モード固有入力として、統合worktree、統合writerへ渡した作成時HEADの完全OIDと同じ文字列、
最終HEADの完全OIDと競合を解消したファイルと箇所を渡す。
加えて、統合対応表の絶対パス、統合対応表に含まれる全計画の絶対パス、
統合スレッドの検証結果及び統合用管理対象領域の絶対パスを渡す。
再レビュー時は既存6列表ファイルの絶対パスも渡す。
統合後HEADはレビュー対象として渡すが、統合writer以降の差分を公開契約の認可基準へ含めない。

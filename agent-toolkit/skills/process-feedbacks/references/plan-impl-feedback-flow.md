...
`atk fb process-loop`で1リポジトリ1計画1コミット単位の直列消化を既定とする。
単一計画へ集約するのは次のいずれかに限定する。

- 対象が単一リポジトリに閉じる場合
- 複数リポジトリ間で不可分な同期改訂を必要とする場合（同一shaで揃える必要がある改訂など）

上記いずれにも該当しない複数リポジトリ横断作業は、リポジトリごとの独立feedback投入へ分解する。
分解投入により対象リポジトリの1つで別セッションによる並行編集が発生しても、担当外変更の
混在・切り分けコスト・衝突リスクが構造的に発生しない状態を保つ。

...
通常型の計画作成に着手する前に、計画実装型の計画ファイルの`### 対象ファイル一覧`と、
通常型の計画候補の反映先ファイル集合を突合する。重複ファイルがある場合は次のいずれかを選択する。
既定は選択肢(a)とする。

- 選択肢(a)（既定）: 計画実装型の完了後に通常型の計画作成を開始する（順序制約）
- 選択肢(b): 通常型の計画の反映先で重複するファイルの`[現行]`ブロック文面確定を、
  計画実装型からの完了通知を受領するまで明示的に保留し、確定させる工程を計画へ追加する。
  適用条件は「重複ファイルが1件、かつ`[現行]`ブロックの前後文脈確定が
  計画実装型の完了に依存しない部位」に限る

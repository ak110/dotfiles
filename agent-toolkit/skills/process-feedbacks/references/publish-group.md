...
- 同じ構成項目を要求するグループでは、ファイル名が最小のグループ終端項目を所有者とする。
- 公開構成集合が空の場合は終端工程を実行せず、観測を根拠にTBDへ送る。
- 呼び出し元は最初の構成項目の計画を受領した時点で`atk managed-temp create --prefix publish-group`を実行し、領域とマーカーファイルを計画の進捗ログへ記録する。
- 呼び出し元は新規領域の作成前に`atk managed-temp list --prefix publish-group`でマーカーファイルを列挙する。同じ`group_final_item`と`target_repo`のマーカーファイルが0件なら新規作成し、1件なら内容を一意に照合して既存領域を再利用し、複数件又は照合不一致なら公開操作を停止してTBDへ送る。
- 領域とマーカーファイルの保存後、進捗ログ記録前に中断した場合も同じget-or-create判定から再開する。再利用した領域の公開グループ表と外部状態を照合して未実施工程だけを続行し、二重の領域作成又は公開操作を行わない。
- 公開グループのマーカーファイルは固定名`.publish-group-marker.json`とし、管理用のマーカーファイルとは別ファイルにする。
- マーカーファイルは`schema_version`（数値、初版は1）、`group_final_item`（文字列）、`target_repo`（正規化リモートURLの文字列）、`created_at`（UTC ISO 8601の文字列）の4必須フィールドだけを持つJSON objectとする。
...
- `managed-temp`のマーカーファイルと利用者専用登録簿の照合は、
  `agent-toolkit:delegation`のClaude Code経路契約を正本とする。

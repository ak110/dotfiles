...
- 検収済み原文正本の絶対パス、対象feedback filename、対象リポジトリ、プロジェクト規範の絶対パス
- `review-checklists.md`の絶対パス
- バグ対応の場合だけ`agent-toolkit:bugfix`の絶対パス
- 調査対象と完了条件

入力が欠ける場合は推測せず欠落項目を返す。
原文正本のパス欠落、標準JSON parserによる解析失敗又は対象property欠落も同様に返す。

## 調査

- 標準JSON parserで対象filenameのpropertyから本文を読み、原文正本を変更せず、queueを参照しない

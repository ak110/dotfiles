...
3. バグ・障害・回帰では`agent-toolkit:bugfix`を起動する
...
- 通常型の採用項目は`agent-toolkit:plan-mode`の起動条件に従い、調査済み事実と採否を計画へ渡す
- 計画実装型は計画ファイルを正本として実装する
- commit前に`agent-toolkit:commit`を起動する
- 実装と二系統reviewの完了後、呼び出し元がpushとCI通過確認を完遂する
- 計画の完了条件を満たした対象だけを後始末へ進める

作業中に独立した新規改善を発見した場合は、完成済み本文と対象リポジトリを
`agent-toolkit:add-feedback`へ渡す。投入コマンドを本スキルから直接構成しない。

## フィードバック投入

新規フィードバックとTBDは、完成済み本文と対象リポジトリを`agent-toolkit:add-feedback`へ渡す。
投入契約と保存手順は同スキルを正本とし、本スキルへ複製しない。
...
全ready項目の処理または保留永続化後に`agent-toolkit:session-review`を起動する。
振り返りで生成した提案は`agent-toolkit:add-feedback`へ渡す。
完了後に`agent-toolkit:exit-session`を起動する。

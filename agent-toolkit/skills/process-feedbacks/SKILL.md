...
- 6件以上の場合は次の手順で選定する（最低5件・最大50件とする）。
  変更対象ファイルが合計で50ファイル以上になると見込まれるfeedbackは
  単独グループとして扱い、他feedbackとの合算選定を回避する。
  既存の関連グループ形成規則より優先し、判定は選定確定前（着手前）に実施する。
  判定はエージェント側の変更対象ファイル数見積り（既存コードベース調査結果に基づく実測）のみで行い、
  いかなる理由（例: フィードバック本文中の主観表現「規模が大きい」「多岐にわたる」「単独で実行すべき」等）が
  あっても判定に用いない
  1. 関連グループを形成する。グループ化基準は次のとおり
     ...
     - 同一規範（節・概念）を対象とする
     - 同一根本原因への複数対応を含む
     - 対象領域が近接し、整合性を保ちながら同時適用できる
       （相互参照を持つスキル群・同一ワークフローを構成する文書群・小規模な変更同士の組合せなどを含む）
     - 1件のフィードバックが複数の基準に該当する場合は、優先順位
       （同一根本原因・同一規範・同一スキル／エージェント定義・同一ファイル・近接領域の順）で
  ...
  4. 処理対象が5件に達しても選定を打ち切らず、時系列順のグループ追加を継続する。
     次の関連グループを加えると50件を超える場合は、その手前の関連グループまでを丸ごと含めて選定を確定する
     （1つの関連グループ単独で50件を超える場合は当該グループのみを処理対象として確定する）

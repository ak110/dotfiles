...
- 5件以下の場合は全件を処理対象として固定する
- 6件以上の場合は次の手順で選定する（最低5件とし、上限は設けない）。
  ...
  2. コーディングエージェント自身の動作に強く影響するfeedbackを最優先で処理対象に含める。
     優先順位は次の順とする。
     グローバルに常時自動ロードされる規範文書（`~/.claude/`配下へ配布される`CLAUDE.md`・ルールファイル等）が最上位。
     次にプロジェクトの規範文書（リポジトリ直下の`CLAUDE.md`・`AGENTS.md`・`.claude/rules/`配下等）。
     その他はこれらの後とする。
     該当feedbackが属する関連グループはグループ単位で処理対象へ含める
  3. 残余は時系列（投入が古い順）を基本として選定する。
     最も古いfeedbackを含む関連グループから順に、グループ単位で処理対象へ加える
     （グルーピングの都合で新しいfeedbackが同一グループとして先取りされることは許容する）
  4. 処理対象が5件以上に達した時点で、当該関連グループまでを丸ごと含めて選定を確定する
- 選定の確定タイミングは本ステップ（着手前）に限定する。
  選定確定後は対象を減らさず、残件はinboxへ残置して当該セッションの選定対象から除外する。
...
起動以降にinboxへ追加されたファイルは当該セッションの選定対象から除外する。

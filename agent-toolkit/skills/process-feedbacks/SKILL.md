...
部分採用は確認経路へ機械的に含めず、差異、採用範囲、除外範囲、理由を採否記録へ残す。起草担当か実行主体へはバッチ全項目を渡し、
...
レーンworktree内のrebase・競合解消・ff前進はマージ担当へ委譲する。統括の詳細な手順とチェックポイント種別は
`references/plan-impl-feedback-flow.md`を正本とする。

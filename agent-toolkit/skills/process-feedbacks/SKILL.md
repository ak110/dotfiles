...
1. `atk mq list --status=active --target-repo=<repo-path>`でactive項目を取得する。
   起動goalにCodexオーケストレーターの連続処理と明記されている場合だけ、後述の再取得を有効にする。
   それ以外は、この一覧のfilenameを本セッションの処理対象として固定する
...
起動時に固定した未終端項目だけを再取得し、新たにreadyとなった項目を「2. 調査と採否」へ戻す。
起動後に追加された項目はactiveのまま残し、process-loop又は次回の手動起動による新しいセッションで扱う。
ただし、起動goalにCodexオーケストレーターの連続処理と明記されている場合は、
取得済みのready項目を終端又は保留した後にactive一覧を再取得する。
依存関係の有無を問わず追加分を含むready項目があれば「2. 調査と採否」へ戻り、ready項目が無い場合だけ「6. 振り返りと終了」へ進む。

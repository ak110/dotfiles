...
5. fast担当、fast担当からfix担当への引継ぎ、別の実装単位、差分限定レビュー修正及びCI修正は、前の担当の識別子を再利用せず新規threadで起動する。
   ...
   Codexで手順5の新規threadを起動する場合は、`Codex後続操作の共通先行条件`を適用してから行う。
...
Codexで代替起動を行う場合も、`Codex後続操作の共通先行条件`を適用してから行う。
...
Codexで各実装単位の新規threadを起動する場合は、`Codex後続操作の共通先行条件`を適用してから行う。

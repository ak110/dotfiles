...
- `agents_server`を利用できる環境では、実行時に公開ツールの説明と入力スキーマを確認する。その仕様に従って起動と追送を行い、状態と終端を観測する。役割文書を持つ工程にはその文書を渡し、自由本文からの起動は対応する経路を選ぶ。経路とモデルの選定は本書の工程別モデル設定に従う
  ...
  - 読み取りだけの調査、参照実装に沿う定型的な書込、出力量が大きいコマンド実行は、各専用経路の公開説明を確認して割り当てる。読み取り専用の調査結果は根拠とともに受け取り、出力の切り詰めや上限到達が報告された結果を網羅性・件数・不在の根拠にしない。書込主体と検証主体の責務は起動契約に従う
  - 同じ担当への後続指示は継続経路を用い、起動と同様にそのturnの終端を観測する。中断と破棄は、継続が不要であることと所有を確認した場合に行う。保持識別子を失った場合は公開の一覧・個別照会で回復し、終了や失敗を推定しない
  - 委譲先が自身のturnを終えずに通知する場合は`atk agents notify`を使う。呼び出し元は`atk agents wait`の通知を受け取った後も、担当の終端まで待機する。通知経路が成立しない事象は委譲先の完了報告で受け取る
...
次の工程では、役割文書を持つ起動と自由本文からの起動を区別し、表の工程へ対応するモデル設定を使う。公開ツールへ渡す引数は実行時のスキーマで確認する。「## 経路」が定める明示指示によりAgentツール経路を使う場合だけ、起動直前に`atk config get <キー>`を実行して実効値を取得する。

...
- 参照文書、規範、スキルの読み取り元も当該作業ディレクトリ配下とし、
  複製元リポジトリの版および配布済みプラグインの版から読ませない。再委譲時にも同じ制約を引き継がせる
  （厳守規定。作業用複製で配布物自体を改訂している場合、複製元と配布済みの版は旧い契約を含み、
  旧い契約を前提に構成した指示は当該計画が定める成果物を生まない）

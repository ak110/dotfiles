...
まとめた担当が実行中にコンテキスト圧縮を繰り返す場合も、単位ごとの分割を対処に用いない。
分割は規範、計画及び対象コードの読み込みを単位数だけ重複させ、圧縮で失う文脈の読み直しと同じ費用を確実に発生させるためである。
対処は、当該工程の設定を文脈長のより大きいengineとモデルの組合せへ見直すことと、起動時の入力から当該担当が使わない資料を除くこととする。
文脈長はengineとモデルごとに異なるため、同じ入力でも圧縮の頻度は組合せによって変わる。
2026-08-30に`codex:gpt-5.6-sol/medium`のfast担当へ計画メイン15KB、計画詳細37KB、タスク文書20KBを渡した実測では、
139分の稼働中に圧縮が10回発生し、初回は起動の6分後、以後の間隔は6分から25分だった。同じ担当のコマンド実行は628回に達した。
再検証は、実行環境が記録する当該担当のコンテキスト圧縮の発生回数と間隔で行う。

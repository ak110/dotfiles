...
- `agent-toolkit:apply-feedback`ステップ3「複数要求の分解判定」で部分採用と判定した際の除外要求投入
...
実装レベルの判断（採用済みフィードバックの実装工程における細部設計・ファイル分割粒度・
関数命名・ライブラリ選定など、コーディングエージェントの技術判断で確定できる事項）は
TBD投入対象外とする。実装レベルの判断は`agent-toolkit/rules/01-agent.md`「ユーザーとともに考える」節に従い
自律判断で確定し、公開インターフェース変更・破壊的操作・観測可能な品質低下を伴う場合のみユーザー確認を発行する。

...
依存の追加・更新を専用CLI（`uv add`・`pnpm`・`mise`等）で行い、ロックファイルとマニフェストを同期させる原則は`implementation-time.md`「依存と自動生成物」が定める。
...
- 依存ロックファイルは自動生成ファイルとして扱い直接編集しない（詳細は`implementation-time.md`「依存と自動生成物」）

...
- 同一の正規計画、又は同じ計画・基点から続く累積実装について、同じ責務系統のreviewerが完了した初回reviewと累積再reviewが第4回以降となった事象。3回以下、結果未返却、別成果物、別責務系統は合算しない
- 前項の事象では、以前から判定可能だった欠陥、初版の混入、指摘反映漏れ、直前修正による混入を実体へ照合し、review側と初版作成・指摘反映側の原因を別々に確定する
- 事前に防げた同型指摘や手戻り、委譲プロンプト・機械検査・計画粒度の不足
- サブエージェント又はCodex MCPへの起動プロンプトの分量。
  正本の絶対パスと対象IDで代替できる複製、受信者が起動時に読む規範やtask referenceの再掲、確定済み情報の反復

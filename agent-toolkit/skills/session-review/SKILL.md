...
5. 当該セッションの全ての委譲先の記録を走査対象とする。サブエージェントの記録、`agents_server`経由で起動した独立セッションの記録、Codexスレッドの記録はいずれも対象に含め、委譲先がさらに起動した委譲先も同じ扱いとする。所在は抽出器がメインの記録から再帰的に導出するため、メインは委譲先の記録の絶対パスを渡さない。件数、出力量、所要時間を理由に走査対象を絞らない。読む量が増える負荷は、独立コンテキストのサブエージェントへ切り出すことで吸収する。抽出器が`unresolved-record`として報告した記録だけを、未確認の範囲として返させる。
6. 初回依頼には振り返りの起動理由を含めない。サブエージェント自身が、通常表示、`--warn`、`--stats`、`--hook-notices`及び必要な`--grep`・`--detail`をそれぞれ1回ずつ実行する。各実行は全ての記録を走査し、イベントの`record`欄が由来する記録を示す。`--detail`の行番号は`<記録>:<行番号>`形式で1回のオプションにつき1つ指定し、複数指定ではオプションを繰り返す。
7. 委譲先のエスカレーションは重点的に確認する。`needs_escalation`と差し戻しは工程が止まった箇所を直接示すため、1件ずつ発生条件と直前の入力を`--detail`で取得し、単発でも問題候補とする。完了報告に現れる手順の前提不成立、再試行及び自力回避も候補へ含める。委譲先が自ら回避して作業を続けた事象は、同じ回避が複数の委譲先で現れる場合を候補とする。
8. サブエージェントは`status`、各走査の成否、問題候補ごとの要約・観測事象・再取得用query・locator・未確認事項及び全体の未確認範囲だけを返す。原因、解決案、反映先、採否、フィードバック本文はいずれも作成しない。

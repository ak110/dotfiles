...
6. 初回依頼には振り返りの起動理由を含めない。サブエージェント自身が、通常表示、`--warn`、`--stats`、`--hook-notices`及び必要な`--grep`・`--detail`をそれぞれ1回ずつ実行する。各実行は全ての記録を走査し、イベントの`record`欄が由来する記録を示す。`--detail`の行番号は`<記録>:<行番号>`形式で指定する。

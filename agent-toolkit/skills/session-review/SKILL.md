...
5. 当該セッションの全ての委譲先の記録を走査対象とする。サブエージェントの記録、`agents_server`経由で起動した独立セッションの記録、Codexスレッドの記録はいずれも対象に含める。所在は、メインのtranscriptに現れる委譲先の識別子と成果ファイルの絶対パスから解決する。件数、出力量、所要時間を理由に走査対象を絞らない。読む量が増える負荷は、独立コンテキストのサブエージェントへ切り出すことで吸収する。所在を確定できない記録がある場合だけ、その範囲を未確認として返させる。
6. 初回依頼には振り返りの起動理由を含めない。サブエージェント自身が、メインと委譲先のそれぞれの記録へ通常表示、`--warn`、`--stats`、`--hook-notices`及び必要な`--grep`・`--detail`を記録全体へ実行する。

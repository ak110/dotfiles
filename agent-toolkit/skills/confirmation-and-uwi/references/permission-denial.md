...
候補の作成が拒否された場合は、別の手段で同じ候補を生成せず、比較を未実施と明示して、拒否された操作の承認をユーザー確認で求める。委譲先は確認事項を呼び出し元へ返す。承認を得るまでセキュリティ設定を変更しない。
候補があっても比較手段が成立しない場合は、実効値が保たれると言い切らず、未確認の範囲として示す。
ユーザー確認の本文には、比較できた値と、比較不能又は未実施の範囲を分けて示す。
いずれの場合も、前段のallow規則との一致確認と、「偽陽性と判断できる拒否への対応」節のユーザー確認を維持する。

...
1. `claude auto-mode defaults`で既定のルールと`clears when ...`の例外条件を確認し、誤拒否されている領域を特定する。
2. 配布側の設定ファイルがある場合はそのsourceを、無い場合はユーザー環境の`~/.claude/settings.json`を編集し、`autoMode.allow`配列へ許可指示を追加する。
3. 再ロード後に`claude auto-mode config`でその項目が有効設定として表示されることと、`"$defaults"`を含む区分で既定の件数が維持されていることを確認する。
4. `claude auto-mode critique`を実行して結果を点検する。指摘の全件採用を必須とせず、`agent-toolkit:review-standards`が定める問題と手段の比例性で採否を決める。

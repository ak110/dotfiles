...
- `observed_atk_help`: 同一セッションでヘルプ出力を観測済みの`atk`サブコマンドを、`atk wi add`のように空白で連結した文字列の配列として保持する。記録はPostToolUseが成功したBash入力から行い、ヘルプ専用の呼び出し（`--`を含まず`--help`を含み、ハイフンで始まる全トークンが`--help`である区間）だけを対象とする。解決するキーは`agent-toolkit/scripts/_atk/help_text.py`の`HELP`が持つキーのうち最長の接頭辞一致とし、一致しない呼び出しは記録しない。利用先はPreToolUseのBash検査であり、当該配列に無いサブコマンドの実行へ警告を返す。寿命はセッション状態ファイルと同じとする

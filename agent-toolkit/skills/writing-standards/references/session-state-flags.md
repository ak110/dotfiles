...
- `english_warning_count`: `agent-toolkit/scripts/pretooluse.py`が、直前のアシスタント応答の地の文を英語主体と判定した連続ターン数を記録する。
...
- `observed_atk_help`: 同一セッションでヘルプ出力を観測済みの`atk`サブコマンドを、`atk wi add`のように空白で連結した文字列の配列として保持する。記録はBashの成功時だけPostToolUseが発火するClaude Codeで行い、ヘルプ専用の呼び出し（`--`を含まず`--help`を含み、ハイフンで始まる全トークンが`--help`である区間）だけを対象とする。CodexのBash payloadは終了コードを含まないため記録元にしない。解決するキーは`agent-toolkit/scripts/_atk/help_text.py`の`HELP`が持つキーのうち最長の接頭辞一致とし、一致しない呼び出しは記録しない。利用先はPreToolUseのBash検査であり、当該配列に無いサブコマンドの実行を遮断する。寿命はセッション状態ファイルと同じとする

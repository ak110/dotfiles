...
- 計画ファイルと、`atk wi add`・`atk wi edit`の`--body-file`へ渡すWI本文は、文章lint、口語表現チェック、ダッシュチェックの必須対象から除外する。いずれも読み手がその計画又はその項目を処理する実行主体に限られ、表記の統一が対象の特定と完成条件の判定を変えないためである。計画とWI本文から転記する文面は、転記先の基準で検査して整形する

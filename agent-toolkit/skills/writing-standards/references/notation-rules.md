...
- 日本語と全角括弧の境界はスペースなしで続ける。英文脈の半角括弧は英数字との間にスペース1つ（例: `Python (CPython 3.12)`）
- 全角丸括弧は入れ子にせず、内側の括弧は読点区切りや別文への分割で表現する
- emダッシュ・horizontal bar・2倍ダッシュは日本語の地の文・見出しの外で使う
  （同格・補足は全角丸括弧、言い換え・敷衍は句点で2文に分けるか読点でつなぐ）
- 技術用語・固有名詞・コード識別子は原典の表記を優先する
- 「復号」は暗号を解く処理にだけ使う。バイト列から文字列への変換と、JSON文字列などの符号化表現から値を取り出す処理は「デコード」と書く
- 地の文は日本語で書く。定着した技術用語・略語、外部技術に由来する語、識別子は原表記を維持する

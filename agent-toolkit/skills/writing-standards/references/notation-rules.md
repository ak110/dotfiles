...
- `atk wi add`・`atk wi edit`の`--body-file`へ渡すWI本文は文章lintの必須対象から除外する。口語表現とダッシュは保存前の診断で行番号付き警告として一括表示する。表記警告の処置は起草者が判断し、構造エラーの有無で保存の可否を決める。計画とWI本文から転記する文面は、転記先の基準で検査して整形する
...
日本語の地の文・見出しにおけるemダッシュ・horizontal bar・2倍ダッシュは`atk run-script writing-check-dash --`で検査する。

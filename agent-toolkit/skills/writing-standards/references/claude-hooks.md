...
通知本文が指示する処置を特定の主体種別又は作業ツリー種別だけが実行できる場合は、当該種別の判定を発火条件へ含める。
主体種別はhook payloadの`agent_id`と委譲先セッションの環境変数、作業ツリー種別はGitへの照会から確定する。
判定を省くと、当該処置の権限も所有も持たない主体が通知を受領し、呼び出し元への差し戻しだけで終わる工程が発火のたびに生じる
（厳守規定。同じ経路で、回答済みUWIの反映を指示する通知とversion bumpを指示する警告が、いずれも処置できない委譲先とレーン用worktreeへ届いた）。
判定入力を解決できない場合は、当該通知を出力する側へ倒す。
...
復元できる結果には警告を用い、遮断を用いない（厳守規定。可逆な操作を遮断すると、当該ターンの入力と作業が失われたうえで同じ操作の再実行を要し、その損失が発火のたびに生じる）。
復元できない結果は、当該セッションの後続の操作で原状へ復元できない結果とする。
次の各項はその代表例とする。
...
当該本文はユーザー自身が読んで誤りを指摘できるため、実行主体が同じターンで是正を受け取れる。
遮断すると当該ターンの入力と作業を失わせたうえで同じ確認の再発行を要し、ユーザーは是正の機会を得ないまま待機する
（厳守規定。ユーザーが読んで是正できる本文の提示を遮断すると、是正のための往復だけが増える）。

手順文書と規範が実行できる形で示している入力形式の制約は、遮断で強制しない。
当該制約は、実行主体が当該文書を読んで満たすものとし、hookの判定対象にしない。
...
当該制約に依存しない手段が同じ判定を通過する場合は、その手段を同じ`fix`へ併記する。
同じ判定を通過する手段が名指しした1つだけである場合は、併記に代えて遮断が解除される条件を`fix`へ書く。
判定ごとに、第1段と第2段の結論及び選んだ根拠を当該判定モジュールのdocstringへ記録する。

第2段で代替手段が複数あるため警告とした判定のうち、同一セッションでの反復が母集団の欠落又は工程の停止を招くものは、1件目を警告、2件目以降を遮断とする。
当該反復が招く母集団の欠落と工程の停止は第1段の復元できない結果に当たるため、この格上げは第1段の内側で成立する。
第1段で復元できると判定した操作は、反復しても遮断へ格上げしない。
...
対応づく要素が無い場合と、通知本文がホストの出力上限を超える見込みがある場合は、載せる対象を絞り、当該集合を確定する手段を`fix`へ示す。
...
エージェントへ特定の行動・引数を要求するblock又はwarnは、要求する要件を実行主体が発火前に読み得る規範文書（常時ロードのルール、または当該作業で起動されるスキルの本文・参照文書）へ明示する。通知本文だけを要件の初出にしない（厳守規定）。
全てのblockとwarnは、対象環境で文書化済みの正式コマンド形（スキル・`AGENTS.md`・タスクランナー定義が指定する起動形）への発動有無を確認し、規範どおりの通常操作では発火しない判定条件にする（厳守規定）。
...
- 規範文書の読込だけを求める注記は、通常発話の受領ごとに返す。本文へ判定手順を書かず、当該要件の正本を保持する文書の所在だけを示す。
  ユーザーの介入のたびに当該規範の所在を想起させるため、経過時間による抑止を置かない。
  本文を1文へ抑えることで、受領ごとに文脈へ載る量を一定に保つ
- 判定手順を本文へ持つ注記は、同一セッションの直前の通常発話からの経過時間を状態として保持し、閾値以上経過した通常発話にだけ返す。
  初回の通常発話は当該注記の対象から除くが、経過時間の基準となる時刻を記録する

初回を含む通常発話では当該時刻を更新し、ハーネスが挿入した通知及びコマンド起動ではいずれの注記も記録も注入もしない。
...
遮断の通知には整形関数が`block`を設定する。区分を示す必要が無い通知にはタグを並置しない。

...
同じ条件を実行基盤か別の共通フックが既に検査する場合は、新しい検査を追加せず既存の検査を正本とする。重複検査は実行時間と状態を増やし、両者の判定差による誤遮断を生む。
...
警告とした判定のうち、同一セッションでの反復が母集団の欠落又は工程の停止を招くものは、1件目を警告、2件目以降を遮断とする。反復が招く欠落と停止は復元できない結果に当たるためである。
任意ツールでツール名とJSON入力が同一の呼び出しは連続数を記録し、1回目から9回目までを通過させ、10回目以降を遮断する。異なる呼び出しで連続数をリセットし、セッション識別とJSON正規化のいずれかを確定できない場合は通過させる。

...
- hook定義が`command`で参照するスクリプトのパスを改名、移動又は削除する場合は、旧パスへ新しい入口を呼び出すだけの互換入口を残す。
  ...
  確認できない場合は互換入口を維持する。
  配布物では、新しい入口を含む版を配布した後の版数更新以降を撤去可能な時機の下限とし、撤去の契機は旧定義を読み込んだセッションの全終了の確認とする
- イベントごとの入口: 同じイベントへ判定を追加する場合は、新しい登録を並べずそのイベントの既存の入口へ相乗りさせる。
...
同じ経路で、回答済みUWIの反映を指示する通知とversion bumpを指示する警告が、いずれも処置できない委譲先とレーン用worktreeへ届いた。
判定入力を解決できない場合は、その通知を出力する側へ倒す。

本条件を満たす判定は、遮断と警告のいずれで返すかを次の2段で決める。
2段の判定は、検出した操作を中止させる`PreToolUse`と`PostToolUse`の遮断を対象とする。
ターンの終了を阻止する`Stop`と`SubagentStop`の遮断には「Stop/SubagentStopフックの再帰呼び出し対策」が定める条件を適用する。

第1段は、検出した操作を通した場合の結果を復元できるかで判定する。
復元できる結果には警告を用いる。可逆な操作を遮断すると、そのターンの入力と作業が失われたうえで同じ操作の再実行を要し、その損失が発火のたびに生じる。
復元できない結果は、そのセッションの後続の操作で原状へ復元できない結果とする。
次の各項はその代表例とする。

- 実行主体のコンテキストへの取り込みと会話記録への記録
- 外部への公開
- プロセス又はタスクの終了
- 対象の上書きと削除

復元できる結果は、再編集又は再実行で是正できる結果とする。
書き込んだ本文の誤りや、副作用を残さずに終了するコマンドなどがこれに当たる。

ユーザーへ提示する本文そのものを入力とする判定は、復元できる結果として扱い警告で返す。
`AskUserQuestion`の質問と選択肢、`ExitPlanMode`の計画本文がこれに当たる。
その本文はユーザー自身が読んで誤りを指摘できるため、実行主体が同じターンで是正を受け取れる。
遮断するとそのターンの入力と作業を失わせたうえで同じ確認の再発行を要し、ユーザーは是正の機会を得ないまま待機する。
ユーザーが読んで是正できる本文の提示を遮断すると、是正のための往復だけが増える。

手順文書と規範が実行できる形で示している入力形式の制約は、実行主体がその文書を読んで満たす。
この制約は、遮断による強制とhookの判定の対象の外に置く。

第2段は、第1段で復元できないと判定した操作について遮断と警告を選ぶ。
検出した操作に対して規範が定める代替手段が1つに定まり、その代替手段を同じターンで実行できる場合は遮断とする。
代替手段が複数あり実行主体が選ぶ場合と、検出した操作を続ける判断が成立し得る場合は警告とする。
遮断は、実行主体が`fix`の文面どおりに再実行すれば成立する場合に限る。
`fix`が名指しする手段には、実行環境の制約又は起動文の指示によりそれを利用できない実行主体がある。
その制約に依存しない手段が同じ判定を通過する場合は、その手段を同じ`fix`へ併記する。
同じ判定を通過する手段が名指しした1つだけである場合は、併記に代えて遮断が解除される条件を`fix`へ書く。
判定ごとに、第1段と第2段の結論及び選んだ根拠をその判定モジュールのdocstringへ記録する。

第2段で代替手段が複数あるため警告とした判定のうち、同一セッションでの反復が母集団の欠落又は工程の停止を招くものは、1件目を警告、2件目以降を遮断とする。
その反復が招く母集団の欠落と工程の停止は第1段の復元できない結果に当たるため、この格上げは第1段の内側で成立する。
格上げの対象は、第1段で復元できないと判定した操作に限る。
反復の記録はセッション状態へ検査ごとのキーで残し、遮断の本文へ警告と同じ解消手段を載せる。
本判定は`warn`区分の通知が反復の旨を本文へ含める3件目の閾値とは別のものであり、その閾値は現行のまま保つ。

遮断するフックが、遮断の解除に必要な情報を判定の時点で保持する場合は、その情報を通知本文へ載せる。
実行主体へ別の呼び出しでの取得を要求すると、遮断のたびに1ラウンドを消費するためである。
遮断と警告のいずれでも、受理集合その他の集合を通知本文へ列挙する場合は、判定した対象と対応づく要素だけを載せる。
列挙は要素数に比例して実行主体のコンテキストを占める一方、対応づかない要素は実行主体の次の操作を変えない。
対応づく要素が無い場合と、通知本文がホストの出力上限を超える見込みがある場合は、載せる対象を絞り、その集合を確定する手段を`fix`へ示す。
...
実行を遮断しない`warn`区分の通知は、同一セッション内で同じ`hook_id`と同じ原因の通知が3件目に達した時点から、反復している旨とそのセッションの累積件数を本文へ含める。
...
PreToolUseの処理は、開始ツール（`start`・`start_explore`）の絶対`cwd`と`send_message`・`kill`の保存済みsessionの検査までとし、入力の実行権限値はそのまま渡す。
...
新設する検査の目的は、この通知に限る。別ツール経由の書き換えやフック自身への変更など、迂回経路の網羅的な遮断は目的の外に置く。
...
- 規範文書の読込だけを求める注記は、通常発話の受領ごとに返す。本文へ判定手順を書かず、その要件の正本を保持する文書の所在だけを示す。
...
`stop_hook_active`は、直前の同フック呼び出しがそのターンの終了を一度阻止したことを示す。

Stop/SubagentStopの`decision: "block"`は、対象主体が同一ターン内の行動で解消できる条件に限る。
...
原本の章名変更時に追従漏れの起点となるため、参照名は原本表記のまま用いる。

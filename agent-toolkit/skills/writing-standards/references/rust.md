...
本書は、Rustのコードとテストコードの記述スタイル基準を定める。
...
- エラーハンドリング
  ...
  - `unwrap()`／`expect()`はテストか、失敗し得ないことが自明な場合に限定する
    - レビュー時は、テストビルドにだけ含まれるコード（`#[cfg(test)]`配下と`[dev-dependencies]`経由でのみ到達するコード）を除外して評価する
  - `panic!`は不変条件違反などプログラマエラー用。ユーザー入力やI/Oエラーには使わない
- `unsafe`は極力避ける。使う場合はブロックを最小化し、安全性の根拠を`// SAFETY:`コメントで示す
  - 例外: Win32／COMの単純なAPI呼び出しのうち、所有権の遷移と生ポインタの受け渡しを伴わないものは`// SAFETY:`を省略してよい。後掲の必須の列挙に当たる呼び出しは当該例外に含めない
...
- 対象プロジェクトの`rust-version`・editionで使える機能は公式リリースノートで確認する

...
- `careful-impl`不使用かつ`agent-toolkit:plan-mode`経由で計画ファイル合意済みの場合:
  実装後にメインエージェントから`writing-reviewer`サブエージェントをAgentツールで起動する。
...
- 同一bullet項目内に複数文を書くと、途中文末の句点と最終文末（句点なし）が
  ...
  回避策は次の2通り
  - 各文を独立した別bullet項目へ分割する
  - 2文目以降をsubbullet（インデントした子項目）として階層化し、各項目の文末表記を独立に揃える

...
2. 作成時に得た絶対パスの`wi-show.stdout`へ、`atk mq show <filename>... --target-repo=<repo> --skip-pull`の標準出力を保存する。実行ツールの戻り値だけを完全性判定に用いない。
...
対象リポジトリ又は取得条件が異なる場合は集合を分ける。単一項目、警告・エラー後の当該項目だけの再取得及びTBD回答確認には、この一括取得を適用せず単数形を維持する。

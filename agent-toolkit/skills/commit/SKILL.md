...
2. 計画記載の目的及びファイル群別の変更説明と、実装中に目的への帰属と必要性を確認した追加変更を、差分と特徴的な文字列で確認する
...
- `git stash`: `git stash list`で対象を特定し、`git stash show --include-untracked -p <識別子>`で
  ...
  固定ロック下のOID再確認と削除を完了する
  - worktree固有refへ退避した場合は`git stash show --include-untracked -p refs/worktree/<退避ラベル>`で確認し、
    `atk worktree-stash drop refs/worktree/<退避ラベル>`で同じ削除手順を使う
- 別パスへの複製: `diff`で複製元と複製先の内容を比べる
...
- 本文又はtrailerを持つ場合は、件名と後続行の間に空行を1行置く。件名だけの場合は空行を加えない
...
- 規範文書は配布先でエンドユーザー又はエージェントの振る舞いを構成するため、本文中の判断分岐、規範削除及び機能的な規範変更は`chore`の対象から外す。配布先の振る舞いを変える場合は`fix`、変えない構成の整理は`refactor`とする
- descriptionはリリースノートを読むエンドユーザーと後続開発者へ目的を日本語で簡潔に伝える。なぜ必要か、何を可能にするか又は何を防ぐかを、具体的な変更内容より先に示す
  ...
  - 例: `fix: review-tableへrow-idを追加する`は変更内容だけを述べ、利用者が解決できる問題を欠く。
    `fix: レビュー応答の対象行を短く一意に指定できるようにする`は目的が伝わる
...
- 本文（body）は任意。記載する場合の読み手、粒度、分量及び構造は`agent-toolkit:writing-standards`の`references/writing.md`「人間向け文章の共通規定」に従う
- コミット帰属文字列は、ユーザーの明示指示、明文化されたプロジェクト方針、
  実行環境が提示する既定値の順で観測できる最初の定義から決定する。
  観測できる定義が無い場合は`Co-Authored-By: <実行中のモデルの表示名> <noreply@<モデル提供元のドメイン>>`を既定とする。Codexのドメインは`openai.com`とする。空文字列の指定は帰属の明示的な無効化として扱う。

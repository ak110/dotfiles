...
- 規範文書は配布先でエンドユーザー又はエージェントの振る舞いを構成するため、本文中の判断分岐、規範削除及び機能的な規範変更は内部的な変更に当たらない。`chore`を選ばず、`fix`と`refactor`のいずれかを前項の基準で決める
...
- 本文（body）は任意。記載する場合はwhyをエンドユーザーが理解できる粒度で書き、ファイル名と実装上の細部を書かない。
  分量と構造は`agent-toolkit:writing-standards`「人間向け文章の共通規定」に従う
...
対応する行が無い処置が残る場合は、当該処置に対応する行を追加してからcommitする。

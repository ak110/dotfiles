...
- CI失敗時は原因を特定し追加commitで是正する。作業完了として応答を返さない
  - 失敗ジョブのログ（GitHubは`gh run view <run-id> --log-failed`、GitLabは
    ...
    `glab ci trace <job>`）を取得し、失敗原因を次の3観点で分類する
    - 原因箇所が実装コード・テストコード・環境のいずれにあるか
    - 再現性が非決定的か（同一SHAと同一実行条件で成功と失敗が変動するか）
    - 原因が自セッションの変更に帰属するか（lint・テスト・型検査・ビルドの失敗等）
  - ログから非決定性が疑われる障害は、再現性の観測結果を更新するために同一原因につき1回まで再実行する
    （GitHubは`gh run rerun <run-id> --failed`、GitLabは`glab ci retry <job-id>`）。
    再実行前に失敗ログを取得しておく
    （過去の試行のログ取得手順は本節末尾「再実行されたワークフローの過去の試行のログを取得する
    手順」を参照する）。
    再実行で結果が変動した場合は非決定的と確定する。
  ...
  - 同一変更系列内push失敗連鎖の停止トリガー
    - 同一変更系列は、同じ目的と対象を継続するPR・MR、またはPR・MRを作成しない
      ブランチ上で着手から完了までに含まれる一連のcommitとする
    - 同一変更系列で自セッション帰属のCI失敗が3回連続した時点で、それまでの調査コンテキストを
      引き継がない**コンテキスト非共有のサブエージェント**を起動する
    - `Agent`ツールで`subagent_type=Explore`または汎用エージェント`claude`を選び、
      `opus`モデルを指定する
    - 失敗ジョブのログを取得する。
      VRTなどのartifact生成ジョブではartifactも取得する
    - 原因箇所・セッション帰属・再現性の3観点で分類する。
      VRT失敗ではVRT固有差分分類も併記する
    - `plan-mode/references/ci-failure-handling.md`を参照し、対処選択肢の確定まで白紙で再調査させる
    - 委譲プロンプトには、過去の失敗ハッシュ・過去に試みた仮説・過去に投入した修正案を含めない
    - 委譲プロンプトには、CI失敗の生の観測事実を渡す。
      観測事実には失敗ジョブ名・エラー文面・ログ取得指示を含める
    - artifact取得指示は、artifact生成ジョブに限って渡す
    - 委譲プロンプトには、対象リポジトリと調査対象ファイルのパスを含める
    - 委譲プロンプトには、適用するプロジェクト規範と規範スキルの絶対パス、
      適用節、着手時の読込指示を含める
    - 委譲プロンプトには、原因分類と対処選択肢の根拠を含む完了条件と、
      完了報告の書式を含める
    - 委譲プロンプトには、読み取り専用で調査し、ファイル編集・破壊的操作・
      `git commit`・`git push`を実行しない制約を含める
    - 呼び出し元セッションは委譲先の完了報告を検収し、白紙で再確定した対処方針を修正へ反映する
    - 継続方針が過去と同一の場合も、過去の対処が誤っていた点を
      サブエージェントの視点で確認してから採用する
    - 同一セッション内で解決する義務を維持する。
      自律モードでも同一セッション内で解消する
    - 自律モードは、`process_feedbacks_skill_invoked`セッション状態フラグが真の状態を指す
    - 変更差分がない`CI trigger`空コミットの再pushは、CI失敗回数に算入する
    - エージェント自身が失敗を観測するたびに自己報告履歴を明示更新し、CI失敗回数を自己申告する
    - 機械カウント手段はプロジェクトごとに定義する。
      本規範ではカウント実装を規定しない
    - 非決定的障害の同一原因につき1回とする再実行上限とは独立に、
      本トリガーは自セッション帰属の失敗を対象とする
    - `careful-review`の5ラウンド上限や`plan-impl-caller-reception.md`の
      3回連続検証失敗と同じ反復停止の考え方を適用する
  - 修正案投入前のログ・artifact確認義務
    - すべてのCI失敗で、修正案を組む前に失敗ジョブのログを取得する
    - artifactが生成されるジョブでは、VRT差分画像やtest-resultsなどのartifactも取得する
    - artifactを生成しないジョブでは、失敗ログのみで判定する
    - 失敗ログや差分画像の確認前に、環境差・タイミング差・初期化順序などの仮説を採用しない
    - 取得したログとartifactの実測から、原因箇所・セッション帰属・再現性の3観点で分類する
    - VRT失敗ではVRT固有差分分類も併記する
    - `plan-mode/references/ci-failure-handling.md`の対処選択肢別判定表を参照して修正案を確定する

  上記3観点分類・再実行上限・CI設定側抑制の禁止・環境起因失敗時のユーザー確認・
  同一変更系列内push失敗連鎖の停止トリガー・修正案投入前のログとartifactの確認義務は厳守規定とする

...
5. stage済みの変更がフィードバック採用を反映するかを判定し、該当する場合は「コミットメッセージとリリース」節が要求する`再発予防:`行をメッセージへ含める
6. commit後に`git status --short`と`git show --stat --oneline HEAD`で成果を実測し、前項で該当と判定した場合は`git log -1 --format=%B`で本文を取得して必要な行が完全一致で含まれることを検収する。欠落した場合は同じ本文要件を満たすようamendして補う
...
計画が計画ファイル（バグ）を参照する場合は、同ファイルのバグ単位ごとの`再発防止処置`を読み、

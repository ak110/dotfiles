...
# 編集時の注意点:
# 1計画=1コミット原則のSSOTは本ファイル「コミット運用」節。
...
計画ファイルの`## 実行方法`がコミットの分割単位を明示している場合は、その分割単位を
一括コミット原則より優先する。分割の境界でファイルが重複する場合はハンク単位で振り分け、
それが技術的に成立しない箇所があればいずれか一方のコミットへ含めたうえでコミット本文へ理由を書く。
レビュー担当は分割の統合を正当な逸脱として承認しない。

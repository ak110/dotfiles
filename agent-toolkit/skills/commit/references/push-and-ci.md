...
- 軽微な破壊的変更をMINORに含めることはOSSなどでも一般的である。SemVerの厳密な解釈より利用者の運用判断を優先し、いかなる根拠があってもバージョン区分の変更をエージェントから提案しない
- 利用者が明示したバージョン区分（MAJOR、MINOR、PATCH）を最優先とする
- MAJORリリースは利用者の明示指示がない限り行わない
- 現行版が数値3要素のSemVerでない場合は、文字列の辞書順や桁数から区分を推測しない。プロジェクトの対応表又は利用者が明示した区分を優先し、いずれも無い場合はバージョンを変更せず、判定不能の根拠を報告する

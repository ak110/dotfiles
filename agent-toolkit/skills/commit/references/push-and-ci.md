...
- MAJORリリースはユーザーの明示指示がない限り行わない（MAJORは互換破壊の外部宣言であり、`agent-toolkit/rules/01-agent.md`「協調と自律」の確認要否第2段階が定める承認対象に当たるため）
- 現行版が数値3要素のSemVerでない場合は、文字列の辞書順や桁数から区分を推測しない。プロジェクトの対応表又はユーザーが明示した区分を優先し、いずれも無い場合はバージョンを変更せず、判定不能の根拠を報告する
...
変更対象が当該条件を含む場合は、条件をローカルで検証できる形へ変えてからpushする。
形を変えられない場合は、CIの結果を待つ工程を見込む。

## push前

直前に`git commit --amend`又は`git commit --fixup`を実行した作業ツリーでは、pushの前に`git status --short`を単独で実行し、追跡ファイルの未コミット差分が残っていないことを確認する。差分が残る場合は当該差分を確定してからpushへ進む。
...
4. CI失敗では、最初の失敗jobを検出した時点でrunまたはpipelineとjobの実識別子、失敗ログ、生成されるartifactを取得し、同一SHAのローカル再現と原因調査を開始する。
   ...
   長出力の取得と要約は`agents_server`の`start_shell`へ委譲できるが、待機と原因分析は委譲しない
...
6. 診断目的で対象jobを再実行した後も、保存済みの同一baselineに対して`wait_ci.py --baseline`を再起動する。
   ...
   完了報告、採否記録及び計画ファイル（メイン）の`## 進捗ログ（実行時）`へ未通過であることと帰属判定を記録する。

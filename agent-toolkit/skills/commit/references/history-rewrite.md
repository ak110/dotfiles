...
範囲の基準となるOIDを当該工程が保持している場合は`git log --oneline --decorate <保持している基準OID>^..HEAD`とする。
autosquashの直前と、autosquashの競合を解消した後の継続の直前では、当該基準を`## fixupの実行上の制約`が保持を求める最古fixup対象とする。
基準となるOIDを保持していない工程では`git log --oneline --decorate -n 20`とする。
範囲と件数のいずれも限定しない起動形は用いない
（厳守規定。3,000commitを超えるリポジトリでは、限定しない起動形の出力が実行環境の上限に達し、履歴と対象commitを観測できない）。
...
本節の`pre_fixup`・`fixup`・`autosquash`・`amend`の各phase名と返却種別`needs_escalation`は、`${CLAUDE_PLUGIN_ROOT}/share/exec.subagent.md`が定める実装担当の契約の値とする。当該契約を受領していない主体は、`needs_escalation`に代えて同じ観測結果を呼び出し元へ報告する。

`pre_fixup`・`fixup`・`autosquash`・`amend`のいずれかが失敗した場合は、自ら復旧操作を実行せず、同じ指摘の履歴統合を再試行しない（失敗時点の履歴とindexの状態は失敗の種別ごとに異なり、状態を確定しない復旧操作と再試行はcommitの消失を招くため）。
ただし、autosquashが内容競合で停止した場合は、同じ実装担当が次の条件を満たす範囲に限って競合を解消してよい。競合箇所が採用済みの指摘に対する修正と統合先commitの変更だけから成り、解消後も当該中間commitの公開契約を維持できることを条件とする。解消したパスだけをstageし、`## 履歴確認の起動形`が定める起動形の`git log`を単独で実行して履歴と継続対象を確認した直後に`git rebase --continue`を実行する。再び内容競合で停止した場合も同じ条件を改めて判定する。
競合箇所へ担当外の変更が含まれる場合、修正の帰属を確定できない場合又は中間commitの公開契約を維持できない場合は、競合を解消せず、同じ指摘の履歴統合を再試行しない。
失敗した操作、終了コード、標準エラー出力、失敗時点の`git status --short`及び`git log --oneline -5`の観測結果を添えて`needs_escalation`で返す。

## merge進行中の退避

`git merge`進行中は`git stash`を使わない（merge進行中のstashは競合解決中の内容をpop時に復元せず、解決結果を失うため）。退避は別パスへの`cp`または別ブランチ退避で行う。
...
プッシュ済みコミットに対するamendとfixupは行わない（厳守規定。公開済みの履歴を書き換えると、当該コミットを取得済みの他の作業ツリーとCIの参照が解決できなくなるため）。

...
モデルとreasoning effortは次の基準を上から順に評価し、最初に該当した項を選ぶ。
reasoning effortの既定値は`medium`とする。

1. 設計判断を伴う実装・レビューはGPT-5.6-Solを選ぶ
2. 設計判断を伴わず、編集対象ファイルが5件以下に収まる作業は、
   GPT-5.6-Lunaとreasoning effort `max`を選ぶ。
   当該組合せは利用者が費用面で優先すると定めた選好であり、コンテキスト消費が増えるため
   対象を前記の範囲に限る
3. 前記のいずれにも該当しない、内容が確定した低リスクの機械作業は
   GPT-5.6-TerraまたはGPT-5.6-Lunaを選ぶ

価格やquotaの固定比較ではなく、失敗時の再試行を含む総費用で決める。
前記のモデルとreasoning effortの選択基準は努力目標とする。
指定モデルが利用できない場合は既定モデルで作業自体が成立するためである。
...
2. `threadId`が無い初回は`mcp__codex__codex`を呼び出す
   ...
   - モデルとreasoning effortは本スキル冒頭の基準で選ぶ。
     `model`を指定する場合は`config`へ`model_reasoning_effort`を必ず併せて渡す。
     省略すると実行環境の設定値が適用され、意図しない深さで実行される

...
計画ファイルレビューでは、改善提案・スタイル指摘を初回ラウンドだけ対応対象とする。
実装差分レビューが全ラウンドで対応対象としないのに対し初回を含めるのは、
計画の記述がそのまま実装の入力となり、この段階の是正が最も安価に済むためである。
...
対象にはテスト失敗・lint違反・品質ゲートの指摘・機械チェックの違反などの機能不全だけでなく、
規模基準に当たらない既存規範違反・陳腐化した記述や、変更に必要な周辺の追随修正を含む。
...
致命的・重大指摘の解消、直前ラウンドの修正が持ち込んだ退行に当たる指摘の解消、
未承認のスコープ変更がないこと、採用した初版内補正と

...
対象ファイルがコーディングエージェント向け文書を含む場合の委譲先分岐は上位規範をSSOTとする。
SSOTは`agent-toolkit/references/plan-impl/execution-process.md`の判断指針節に集約する。
本スキルは当該分岐に従い呼び出される前提で動作する。
呼び出し元プロンプトの一括codex指定は当該SSOTの分岐を上書きしない。

<!-- 同期注記: 本節は`agent-toolkit/references/plan-impl/execution-process.md`「実装委譲の判断指針」節の手順3配下規定と対応する。判定基準・対象範囲のSSOTは同節に集約し、本節はcodex起動プロンプト構築時の履行手順のみを扱う。改訂時は同節と同期させる。 -->
`agent-toolkit-edit`スキル起動義務の対象タスクをcodexへ委譲する場合の履行手順は次のとおり。
codex起動プロンプトの`## 遵守事項`節へ、対象ファイル編集前に
Skillツールで`agent-toolkit-edit`スキルを呼び出す旨を明記する。
これにより品質ゲートを履行する。
対象範囲・判定基準は`agent-toolkit/references/plan-impl/execution-process.md`の
`実装委譲の判断指針`節の手順3配下規定を典拠とする。

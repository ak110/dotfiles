# 実装レビューの起動詳細

`agent-toolkit/skills/careful-review/SKILL.md`「起動」節から分離した詳細を収録する。
収録内容は`plan-codex-delegate`（`用途: 実装差分レビュー`）の分担起動閾値・渡す情報・継続呼び出し方針とする。

## codexの分担起動閾値

- 既定: 単一`plan-codex-delegate`インスタンスへ対象範囲全体を渡す
- 対象合計ファイル数が10以上または対象合計行数が概ね1000行以上の場合、
  カテゴリ別に2並列起動する（コード・テストコード／一般ドキュメント・コーディングエージェント向け文書）
- 対象合計ファイル数が30以上または差分2000行以上の場合は3並列に分割する
  （コード・テストコード／一般ドキュメント／コーディングエージェント向け文書）

`plan-codex-delegate`はcodex側の大容量コンテキスト処理を前提とし、
対象ファイル1件あたりの行数上限を設けない。

## 渡す情報

各`plan-codex-delegate`（`用途: 実装差分レビュー`）インスタンスへ次を渡す。

- プロジェクトルート: `careful-review`実行時のプロジェクトルート絶対パス（`{project_directory}`）
- 対象範囲: ファイルパス列挙またはgit範囲（担当カテゴリの対象ファイルのみに限定する）
- 対象外ファイル: 一時ファイル一覧
- 計画ファイルパス: `careful-review`が受け取った計画ファイルパス（無しの場合は省略）
- 差分取得コマンド: `git diff {基準点}..HEAD`と未追跡ファイルを含む作業ツリー参照
- 変更の意図: 計画または提案元が確定した変更目的
- 意図的に受け入れた挙動変化: 認識済みの機能低下・検出範囲の縮小・互換性の変更など。
  該当なしの場合は`なし`
- 担当カテゴリ: コード・テストコード／一般ドキュメント／コーディングエージェント向け文書のいずれか

分担起動する場合も、変更の意図と意図的に受け入れた挙動変化は
全`plan-codex-delegate`インスタンスへ同一内容を渡す。
各起動プロンプトへ、完了報告をAgentツールの戻り値で返し`SendMessage`による能動送付を
行わない旨、およびレビュー対象ファイルを編集しない旨を明記する。
意図的に受け入れた挙動変化は妥当性の再評価対象から除外し、
列挙内容と実装が一致しない場合だけ指摘対象とする旨も明記する。

## 継続呼び出し

`用途: 実装差分レビュー`の再レビューは`threadId`を使わない。
段階2成立時の`plan-reviewer`も同様の扱いとする。
`agent-toolkit/skills/careful-review/SKILL.md`「再レビュー」節の新規`Agent`起動方式に従い、毎回独立に評価する。
理由は`agent-toolkit/skills/review-standards/SKILL.md`「完成条件」節が定める、
確認済みの確実な誤りのみを報告する原則を毎回独立に適用するためである。
`plan_full_path`（計画ファイル種別専用の変数）には依存しない
（`careful-review`単独実行時は計画ファイルが存在しないため）。

## plan-reviewerの起動（段階2フォールバック時）

`codex-review.md`「codex利用可否の2段階判定」節の段階2が成立し`plan-reviewer`を起動する場合、
単独起動を既定とする。担当観点は`agent-toolkit:review-standards`の完成条件と同一とし、
コーディングエージェント向け文書カテゴリを含む場合は`01-agent.md`・`agent-standards`適合性の評価も
同一起動へ含める。

対象合計ファイル数が10以上または合計行数が概ね2000行以上の場合、カテゴリ別に分割起動する
（コード・テストコード／一般ドキュメント・コーディングエージェント向け文書）。
対象ファイルが1000行を超える場合は全文`Read`を避け、`git diff`出力範囲・該当節（H2/H3単位）・
差分周辺のシンボル単位・特定行範囲のみを`Read`する。

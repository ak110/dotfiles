...
2. `plan-impl-executor`起動工程を実施しない。代わりに`agent-toolkit:process-feedbacks`「フィードバック投入」節の手順を実行する。
   投入前に`atk mq list --status=active --target-repo=<repo-path>`でactive一覧を確認し、
   先行計画との重複、依存、吸収関係を計画feedbackへ明示する。計画の単純さを確認省略の理由にしない。
   ...
   投入したフィードバックは`agent-toolkit:process-feedbacks`「ステップ1: 入力の確定とreadiness確認」で

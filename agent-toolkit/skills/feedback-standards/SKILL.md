...
ただしTBDの`## 回答`節とフィードバックの`## ユーザーコメント`節はユーザーだけが書き込み、エージェント環境から起動した`atk`はこれらへの書き込みを拒否する。例外として、新規項目を投入する`atk mq add`だけは、ユーザーが発した要求の逐語転記を含む本文を受理する。既存項目の当該節を書き換える編集は拒否したままとする。

...
TBDの`## 回答`節とフィードバックの`## ユーザーコメント`節はユーザーだけが書き込む欄とし、エージェント環境から起動した`atk`はこれらの節を含む投入と、これらの節を書き換える編集を拒否する。エージェントがユーザーの発言を引用する場合は、これらの見出しを作成せず、本文中へ出所と引用範囲を示して引用する。引用はエージェントが書いた記述であり、ユーザー自身の記入欄と同じ扱いにしない。

...
class TestCheckLineRef:
    ...
    def test_check_path_existence_no_suggestion_when_no_candidate(self, tmp_path: pathlib.Path) -> None:
        """短縮パス候補が0件の場合は「もしかして」を付記しない（FB10）。"""
        path = _write(tmp_path / "doc.md", "対象は`sub/bar.md`である。\n")
        result = _run(str(path), cwd=tmp_path)
        assert result.returncode == 1
        assert "実在しない" in result.stderr
        assert "もしかして" not in result.stderr

    def test_check_path_existence_suggests_single_candidate(self, tmp_path: pathlib.Path) -> None:
        """短縮パス候補が1件のとき、「もしかして」で単一候補を付記する（FB10）。"""
        (tmp_path / "real" / "sub").mkdir(parents=True)
        (tmp_path / "real" / "sub" / "bar.md").write_text("x\n", encoding="utf-8")
        path = _write(tmp_path / "doc.md", "対象は`sub/bar.md`である。\n")
        result = _run(str(path), cwd=tmp_path)
        assert result.returncode == 1
        assert "もしかして" in result.stderr
        assert "real/sub/bar.md" in result.stderr

    def test_check_path_existence_suggests_multiple_candidates(self, tmp_path: pathlib.Path) -> None:
        """短縮パス候補が2〜3件のとき、候補を列挙する（FB10）。"""
        for top in ("real1", "real2"):
            (tmp_path / top / "sub").mkdir(parents=True)
            (tmp_path / top / "sub" / "bar.md").write_text("x\n", encoding="utf-8")
        path = _write(tmp_path / "doc.md", "対象は`sub/bar.md`である。\n")
        result = _run(str(path), cwd=tmp_path)
        assert result.returncode == 1
        assert "real1/sub/bar.md" in result.stderr
        assert "real2/sub/bar.md" in result.stderr

    def test_check_path_existence_omits_individual_candidates_when_many(self, tmp_path: pathlib.Path) -> None:
        """短縮パス候補が4件以上のとき、個別列挙せず多数存在する旨のみ付記する（FB10）。"""
        for top in ("real1", "real2", "real3", "real4"):
            (tmp_path / top / "sub").mkdir(parents=True)
            (tmp_path / top / "sub" / "bar.md").write_text("x\n", encoding="utf-8")
        path = _write(tmp_path / "doc.md", "対象は`sub/bar.md`である。\n")
        result = _run(str(path), cwd=tmp_path)
        assert result.returncode == 1
        assert "補完候補多数のため個別表示なし" in result.stderr
        assert "real1/sub/bar.md" not in result.stderr

    def test_check_path_existence_suggestion_excludes_self(self, tmp_path: pathlib.Path) -> None:
        """検査対象の計画ファイル自身は補完候補から除外される（FB10）。"""
        plan_dir = tmp_path / "sub" / "deep"
        plan_dir.mkdir(parents=True)
        path = _write(plan_dir / "plan.md", "対象は`deep/plan.md`である。\n")
        result = _run(str(path), cwd=tmp_path)
        assert result.returncode == 1
        assert "実在しない" in result.stderr
        assert "もしかして" not in result.stderr

    def test_check_path_existence_suggestion_excludes_git_dir(self, tmp_path: pathlib.Path) -> None:
        """`.git`配下の候補は補完候補から除外される（FB10）。"""
        (tmp_path / ".git" / "sub").mkdir(parents=True)
        (tmp_path / ".git" / "sub" / "bar.md").write_text("x\n", encoding="utf-8")
        path = _write(tmp_path / "doc.md", "対象は`sub/bar.md`である。\n")
        result = _run(str(path), cwd=tmp_path)
        assert result.returncode == 1
        assert "もしかして" not in result.stderr

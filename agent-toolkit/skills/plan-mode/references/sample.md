...
必須項目の内訳は`norm-revision-checklist.md`「規範対象範囲の網羅確認」節の規定に従う。
...
`agent-toolkit/rules/01-agent.md`「セッション分割・別計画化は禁止する」節へ全称禁止形バレットを追加し、
...
- 検出件数: 既存3件（すべて01-agent.md内。追加バレットとの重複は0件）
- 対応方針: 追加バレットは既存3件と重複せず新規事象を扱う

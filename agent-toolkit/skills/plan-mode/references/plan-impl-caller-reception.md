...
計画から説明的な実装単位名、先行依存、統合順及び近接検証を読み、ベースコミットは作成時点の参照値として表示だけに用いる。ベースコミットの一致をレーン割当又は着手可否の条件にせず、`plan-impl-executor`へ渡すworktreeの完全な一覧を作成する。
新規計画メインの固定H2は`## エージェント判断`、`## 変更履歴（計画時）`及び`## 進捗ログ（実行時）`を含む正規順とし、旧形式の`## 変更履歴`・`## 進捗ログ`は読み取り互換として扱う。エージェント提案行の判断説明は、計画メインの採否・範囲の上限と照合する。
...
新規計画では進捗の記録先を`## 進捗ログ（実行時）`とし、旧見出しを使用する既存計画は本文を変更せず読み取り互換で受領する。
...
- `agent-toolkit:plan-mode`の実装担当契約、計画レビュー担当契約、`implementation-review-task.md`及びレビュー継続契約
...
最終完了の検収（commit・clean状態・最終検証・`implementation-review`のレビュー・完了条件の一括検収）、進捗ログの完了確定、
後始末のいずれも行わない。`checkpoint.type`別に次の必須項目を検収する。

- `review_round`: ラウンド番号、指摘件数（ラウンド合計）、被覆結果、実害の概要、修正内容の要約、残る証拠不足、次ラウンドの要否
...
レビュー表の初期化は、既存の`agent-toolkit:plan-mode`のレビュー継続契約どおり調整主体が初回レビュー前に行い、呼び出し元は初期化又は書込みを行わない。実装レビューでは、渡した管理対象一時領域の`review.tsv`を使い、未作成の場合だけ初期化する。既存の場合は初期化せず表を保持したまま構造検証を通過させ、以後の全ラウンドで同じ絶対パスを指定する。計画stemと同じ`<計画stem>.tsv`は計画レビュー専用の別表であり、実装レビューでは扱わない。
開始ゲート前はレビュー表を成果物観測へ含めず、読み取り専用の存在確認後に、既存の成果物観測へ`atk watch --file <レーン名>-review=<レビュー表の絶対パス>`として追加する。
開始ゲート後に観測したレビュー表の行数を、当該レーンの受領済み行数の初期値とする。
...
保持したエージェント識別子へ、次のラウンド境界で`review_round`を返す是正指示を`SendMessage`で1回送る。
行数増加だけからラウンド完了、委譲先の停滞又は停止可否を判定しない。
...
4. `implementation-review`の対象commit、読み取り専用状態、被覆結果、指摘と対応結果、`review.tsv`の実在及び`track`帰属
5. 計画の完了条件と`## 進捗ログ`
...
各commitは計画の`### 実装単位`にある説明的な実装単位名へ対応付け、履歴書換え前後も同じ名前との対応を保持する。実装単位を分割する場合は、対象ファイル集合、近接検証、成果とcommitがそれぞれ独立していることを確認し、独立性を説明できない事項は1つの単位へまとめる。

実装中に判断を変えた場合は`## 変更履歴`へ内容を判別できる見出しを追加し、起点、内容、採否、現在の結論及び同期先を記録する。
直接のユーザー発言を受領した場合だけ、同じ記録へ逐語`text`コードブロックを追加する。
...
実装委譲はcommitと`implementation-review`のレビューまでとする。呼び出し元が`agent-toolkit:commit`のpushとCI契約を読み、pushとCI通過確認を所有する。
...
全工程の完了後、呼び出し元は次の構成で完了報告をユーザーへ提示する。

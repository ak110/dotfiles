...
同じ計画ファイルへ書き込む起草側の実行主体（調整主体と起草担当）が終端していることを、起動時に保持した実行識別子の直接照会で確認してから実装担当を起動する。
...
3. 近接検証と最終検証の実行結果
4. 二系統のレビューの対象commit、読み取り専用状態、指摘と対応結果、系統別レビュー表の実在
5. 計画の完了条件と`## 進捗ログ`
...
8. 通常の実装モードでレビュー修正を受領した場合は、レビュー対象の最終HEAD完全OIDと指摘ID・統合先commit完全OIDの対応表を確認する。
   ...
   書込担当が各fixup作成前、autosquash直前とamend直前の各phaseで`../../commit/references/history-rewrite.md`

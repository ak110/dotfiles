...
- モード指定`通常の実装モード`
- 計画ファイル、プロジェクト規範の絶対パス。1計画ファイルの実装は1つの書込担当へ順次割り当てること
...
深掘り条件が成立する事象は現行規範に従って`agent-toolkit:bugfix`を起動し、原因分析結果をモード別経路でフィードバックへ送る。
ユーザー判断事項も同じモード別確認経路へ送り、独自の手順を本文書へ複製しない。

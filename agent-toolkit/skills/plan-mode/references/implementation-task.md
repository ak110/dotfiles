...
9. `git diff`、受領した目的と変更説明、変更後文面を照合し、担当単位へ帰属する変更だけをstageする。
10. `agent-toolkit:commit`に従ってcommitし、commit実体とcleanな作業ツリーを確認する。
11. commit、検証、受領した計画又はCI記録の修正認可根拠との差異を呼び出し元へ返す。
...
検証済みの実際値、期待値及び違反契約を確認する。対象への適用根拠と保持契約が指摘ごとにそろうことも確認する。

...
- 対象worktreeとプロジェクト規範の絶対パス。計画ファイルは`CI修正担当`・`マージ担当`以外では必須とし、`CI修正担当`では受領済みの場合だけ渡す
  新書式ではメイン側とdetail側の両方の絶対パスを必須とする。detail側を実装詳細の正本、メイン側の実施内容と変更履歴を利用者が承認した採否・範囲の上限として扱う
...
1. 受領済みの場合は計画を全文読み、常にプロジェクト規範、該当する作成規範スキルを全文読む。
   新書式の計画を受領した場合は、detail側の実装単位、先行依存、統合順、検索結果、対象判断と確定文面を実装契約として扱う。
   メイン側の実施内容と変更履歴へ照合し、detail側に別の実装判断を要する欠落がある場合は推測や原文再取得で補わず`needs_escalation`で返す。
   旧形式の計画を受領した場合は、
   人間向け固定領域（`## 概要`から`## 変更履歴`まで）をユーザー要求の正本、
   その後の実装者向け領域を実装詳細の正本として扱う。
   計画を受領しない`CI修正担当`は、CI記録にある利用者指示か公開契約を修正認可の上限とする。

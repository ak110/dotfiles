...
- 既存規範の周辺品質サブ観点: 既存規範の改訂・拡張時は周辺文面の不自然な踏襲・複数概念マージ時の手順脱落・
  新ルール自体の適用漏れを照合する（規範を新規追加する時は同節・隣接節の該当記述を全件抽出し置換・統合可否を確認）
...
- 意図的重複の同期注記照合: 同一趣旨の規範を2ファイル以上に追加する計画では、
  frontmatter同期注記コメント追加が計画本文の対象ファイル一覧に含まれるかを工程7で照合する

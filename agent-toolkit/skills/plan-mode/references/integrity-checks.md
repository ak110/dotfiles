...
- 工程7の各サブエージェント起動（条件成立時の`agent-doc-validator`含む）前に、
  ...
  - `uv run --script agent-toolkit/skills/plan-mode/scripts/check_plan_file.py <計画ファイルパス>`を
    実行する。詳細な検査項目は同スクリプトのdocstringを典拠とする
  - `## 変更内容`H3節配下の追記ブロック・縮減対象ブロックの集計、および対象ファイル一覧に
    `[現行]/[置換後]`テキスト対比ブロック形式の差分を含む場合の差分適用後`wc -l`実測値との
    乖離を検出する。2行を超える乖離（`_ALLOWED_DRIFT = 2`）は計画本文側を実測値に修正する。
    計画段階（初版`Write`前）の2行超過は縮減対象の追加選定または追記文言の圧縮で許容幅内に収める
  - 対象ファイルに`.md`規範文書が含まれる場合、`plan-implementer`委譲のmodel/effort指定規範と突合する。
    新規規範のスコープ限定表現（固有名詞限定の有無）がコーディングエージェント向け文書全般へ
    拡張可能かも点検する
  - `## 変更内容`配下の対象ブロック（ラベル体系は`plan-file-guidelines.md`「変更内容」小節の定義に従う）の
    縮退フレーズは違反0件を必須とする。textlint・line-width（127幅）の違反は転記文面として
    極力解消する。残存は自律判断で記録する
  - 計画ファイルに`#### 廃止・改名対象一覧`H4が存在する場合、リポジトリ横断grep結果と
    対象ファイル一覧との差集合が空であることを確認する
...
1. 計画ファイル初版Write完了後、同一メッセージ内で次を並列起動する
   ...
   - サブエージェント`plan-reviewer`: 独立コンテキスト点検を次の観点で委譲する。
     ...
     - plan-reviewer雛形の「メイン側実施済み観点の内訳」欄へ、機械チェック実施結果・
   ...
   - サブエージェント`agent-doc-validator`（条件付き起動）:
     ...
     計画ファイル本文と対象ファイル現状を突合し、`01-agent.md`方針および`agent-standards`スキル方針への
     適合性を独立にレビューする

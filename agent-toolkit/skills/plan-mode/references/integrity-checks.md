---
# 同期注記: 「メイン側実施済み観点の内訳」欄はlaunch-prompts-integrity.mdのplan-reviewer雛形・
# plan-reviewer.md「入力」節と重複する。改訂時は3ファイルを同時更新する。
---
# 計画ファイル整合性チェック
...
- 工程7の各サブエージェント起動（条件成立時の`agent-doc-validator`含む）前に、
  ...
  - `## 変更内容`配下の対象ブロック（ラベル体系は`plan-file-guidelines.md`「変更内容」小節の定義に従う）を
    `check_plan_diff_gates.py`で機械検査し、縮退フレーズ・textlint・line-width（127幅）各違反0件を確認する
...
本文全域の機械チェック通過確認は`plan-file-guidelines.md`「計画ファイル全体の遵守事項」節の
事前検査で解消済みのため、`plan-reviewer`は補完照合のみ担当する。
...
- `## 変更内容`の`wc -l`確認手順記載と`### エージェント判断`の改訂後見込み行数
 （`wc -l`実測値）記載の有無を検査する。いずれか欠落時は重大指摘として報告する
- 対象ファイル全件の現行行数が『対象ファイルの現状』H3節に列挙され、境界近接（200〜219行）ファイルは
  `### エージェント判断`に追記量試算・縮減対象節の明示があるかを検査する。欠落は重大指摘として報告する
- 境界近接ファイルへの追記計画では、追記文面と圧縮後文面の両方を127幅事前検査する。
  `check_plan_diff_gates.py`の抽出対象に「圧縮対象:」「圧縮後:」トリガー文配下の`text`ブロックを含む。
  同スクリプトは`check_line_width.py`をsubprocess呼び出しして機械検査する
...
メイン側は「ユーザー発話・提示素材との照合」全体および「計画文内・他ファイルとの整合」節の
横断`grep -rn`確認・関連ファイル本文の通読部分を担当し、`plan-reviewer`サブエージェントは残余を担当する。
...
1. 計画ファイル初版Write完了後、`## 変更内容`配下の全ファイル改訂を対象ファイル現行本文へ適用した
   ...
   - サブエージェント`plan-impl-reviewer`: 上記の改訂後最終形の一時ファイル群を対象とし、
     コード・ドキュメント・日本語表現の単体品質を事前レビューする
     - 起動プロンプトは`launch-prompts-integrity.md`「plan-impl-reviewer雛形」節を機械転記して構築する。
       埋め込み欄は一時ファイル絶対パスと正本パスの対応表・差分基準SHA・担当観点・参照すべき規範スキル・完了報告書式
     - 一時ファイル群を生成せず起動すると実装未着手で点検不能を返しレビュー起動が無効化するため、
       生成コスト回避を理由に省略してはならない
...
対象ファイル数が概ね10以上、または新規ファイルが5以上を含む大規模計画では、次の代替経路を既定とする。
`plan-impl-reviewer`の対象を改訂後最終形の一時ファイル群ではなく、
`## 変更内容`セクションの変更方針コードブロック（設計要件記述）とする。
本代替経路では計画時点の設計要件品質を審査し、実装後の実体品質は`careful-review`で審査する分担とする。
代替経路を採用した計画では`check_wc_projection.py`が採用宣言を検出して乖離検査をスキップする。
判定対象は`## 対応方針`節（`### エージェント判断`等のH3子節を含む）本文で「代替経路を採用」の文字列を部分一致で検出する。
本判定を欠く計画では通常経路として乖離検査を実行する。
代替経路の採用時の`## 変更内容`H3配下の設計要件記述は`text`フェンス内・`[置換後]`ラベル付き形式で書く。
通常経路の実装差分記述（`[現行]`／`[置換後]`対比）と書式を揃え、`plan-impl-reviewer`の審査対象を明示化するためである。
「概ね10以上」の許容境界は9〜10ファイルとする（9ファイルは代替経路を採用してよい・8ファイル以下は通常経路）。
version bump対象等の付随変更ファイルは計上対象から除外する。
境界近接時の判定タイミングは計画ファイル初版Write直前の1回に固定する。

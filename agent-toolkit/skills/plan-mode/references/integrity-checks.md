...
1. 計画ファイル初版Write完了後、`## 変更内容`配下の全ファイル改訂を対象ファイル現行本文へ適用した
   ...
   - `codexレビュー`・`plan-reviewer`・`naive-executor`・`plan-impl-reviewer`と
     ...
     - 時間・コンテキスト消費・autocompact thrashingリスク・作業量見込み・
       後続スキル到達優先等の自己推定を根拠に、省略・部分実施・単独foreground委譲下での例外扱いへ流用しない。
   ...
   - サブエージェント`plan-impl-reviewer`:
     ...
     - 起動プロンプトは`launch-prompts-integrity.md`「plan-impl-reviewer雛形」節を機械転記して構築する。
       ...
       参照すべき規範スキル・完了報告書式
     - 改訂後最終形の一時ファイル群の生成を省略した状態でplan-impl-reviewerを起動すると、
       実装が未着手だと検出され点検不能を返しレビュー起動が無効化するため、

...
- 複数規範改訂の論理交差確認: 同一計画内でN件（N≧2）の規範改訂を含む場合、
  ...
  - 具体手順とリスクベース条件は
    `agent-toolkit/skills/plan-mode/references/norm-revision-checklist.md`「複数規範改訂の論理交差確認」節に従う
...
- 工程7の5並列レビュー起動前に、メイン側で次の3項目を機械的にセルフチェックする
  ...
  - 対象ファイルに`.md`規範文書が含まれる場合の突合。
    ...
    計画本文のmodel指定案を突合する
  - 新規規範のスコープ限定表現（「〜配下」「〜のみ」等）に固有名詞の限定がある場合の点検。
    コーディングエージェント向け文書全般
    （`AGENTS.md`・`CLAUDE.md`・`.claude/rules/`配下・`.chezmoi-source/dot_claude/`配下を含む）へ
    拡張可能かを固有名詞除外テストで点検する
...
- 対象ファイル全件の現行行数（`wc -l`実測値）が`## 調査結果`「対象ファイルの現状」H3節に列挙されているかを検査する
  - 欠落を検出したファイルは重大指摘として報告する
- 境界近接（現行行数が200〜219行）のファイルについて、
  `### エージェント判断`に追記量の事前試算と縮減対象節の明示が記載されているかを検査する。
  欠落している場合は重大指摘として報告する
...
1. 計画ファイル初版Write完了後、`## 変更内容`配下の全ファイル改訂を対象ファイル現行本文へ適用した
   ...
   - サブエージェント`plan-reviewer`:
     「計画文内・他ファイルとの整合」「機械チェック適合性」「編集対象スキル固有規定の事前適用」
     「サブエージェント連携の設計整合性」の独立コンテキスト点検を委譲する
     - 起動プロンプトは`launch-prompts-integrity.md`「plan-reviewer雛形」節を機械転記して構築する。
       埋め込み欄は計画ファイルパス・関連参照ファイル一覧・メイン側担当節の実施有無・
       担当観点・参照すべき規範スキル・完了報告書式
   - 完全な並列一括起動: codexレビューと4サブエージェントは同一メッセージ内で一斉に起動する。

...
計画が複数のフィードバックを統合したものである場合、`## 追加情報`へ
`agent-toolkit/skills/process-feedbacks/SKILL.md`「ステップ5: 計画作成と実行」節が定める追跡表を含める。
レビュー担当は当該追跡表と計画の提示素材を突き合わせ、全ファイル名が重複なく1回ずつ現れることと、
原題が一致することを確認する。

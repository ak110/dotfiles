...
`--output-last-message`でレビュー結果をファイルに出力する。
...
- codex指摘1件が複数セクションに伝播する場合、関連セクションすべてを網羅的に更新する。
  単一セクション修正で再レビューに進むと、未修正の他セクションが再指摘対象になる
  - 例: 方針記述の変更は`### エージェント判断`・`## 変更内容`・
    `## 実行方法`のコミット件名・検証手順への波及を確認する
  - 計画ファイル内で該当キーワードを`grep`し、関連箇所を洗い出してから修正する

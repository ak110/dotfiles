...
2. `agent-toolkit:writing-standards`の該当資料を読み、計画の`## 要件・外部仕様`に従って実装する。`### 受入シナリオ`の各行に対応する公開入口経由の結合・E2E検体を実装単位へ含めて近接検証で実行し、シナリオ別の検体名と合否を検証結果へ記録する。手動観測では入力と結果を管理対象一時領域へ保存する。`## 検証`の近接検証を実行し、commitする。実装単位ごとのcommitとcommitメッセージは`agent-toolkit:commit`に従う

...
1. 適用規範、変更対象、定義・参照・呼び出し元、既存テスト、生成・配布経路、類似実装のうち、計画ファイルへ書く内容を確定するために必要な範囲を調査する
   - 変更する値や動作は、参照する側に加えて、同じ値や動作をもたらす別の経路（既定値・プリセット・設定・環境変数・別の入口・同じ操作を制約する別の節など）をたどる。各経路の保持先、直接消費先及び利用者が観測する結果も実装から確かめる。該当しない経路は実装との照合結果で除き、たどった経路を`## 要件・外部仕様`の変更対象と追随範囲へ書く。変更対象の参照元だけを調べると、同じ結果を別の場所でもたらす経路が変更から漏れる
   - 完了条件へコマンド出力、警告・診断件数又は条件分岐の結果を用いる場合は、観測値の生成主体と消費経路を対応付ける
...
認識の違いで要件又は結果が変わる未確定判断は手順2と同じ経路で確認する。
...
2. `agent-toolkit:writing-standards`の該当資料を読み、計画の`## 要件・外部仕様`に従って実装する。`### 受入シナリオ`の各行に対応する公開入口経由の結合・E2Eテストを実装単位へ含めて近接検証で実行し、シナリオ別のテスト名と合否を検証結果へ記録する。手動観測では入力と結果を管理対象一時領域へ保存する。`## 検証`の近接検証を実行し、commitする。実装単位ごとのcommitとcommitメッセージは`agent-toolkit:commit`に従う
...
4. 終端前に画面差分では同じ対象HEADのユーザビリティレビューの`合格`又は`未確定`と実行レビュー未解決0件、それ以外では実行レビュー未解決0件を確認する。全体検査とCIの成否は公開工程で判定し、近接検証の成功は実装工程で判定する

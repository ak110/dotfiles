...
3. `${CLAUDE_PLUGIN_ROOT}/share/exec-review.parent.md`に従って実行レビュー指摘管理表を準備する。画面差分があれば同じ対象HEADの実描画レビュー担当と実行レビュー担当を並列に起動し、修正後も並列に再判定する。指摘への修正はメインが実施し、修正commitの履歴統合は`${CLAUDE_PLUGIN_ROOT}/share/exec.subagent.md`「レビュー修正の履歴統合」と同じ順序で行う
4. 終端前に画面差分では同じ対象HEADの実描画`合格`又は`未確定`と実行レビュー未解決0件、それ以外では実行レビュー未解決0件を確認する。全体検査とCIの成否は公開工程で判定し、近接検証の成功は実装工程で判定する

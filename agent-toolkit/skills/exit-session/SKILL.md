...
process-loopから起動されたセッションでは、agent-toolkit pluginのPostToolUseが`agent-toolkit:exit-session`の呼び出しを状態へ記録する。
pluginのStop hookが`autonomous_exit_invoked`を参照して呼び出し漏れを検出する。

...
| 入力環境 | 判定 | 終了経路 |
| --- | --- | --- |
| Claude Code | 停止不能 | シグナルを送らず`/exit`の入力を案内する |
...
Claude Codeは、スキル実行主体が起動結果から取得して保持した停止識別子を公開しない。
POSIXの`$PPID`又はWindowsの祖先PIDは所有する停止識別子に当たらないため、プロセスへ停止を要求しない。
ユーザーへ`/exit`の入力を案内して本スキルを終える。

...
`agent-toolkit:tidy-unpushed-commits`スキルの実行フェーズで使う2経路とconflict発生時の対応方針。
...
残存コミットが1つでもある場合は適用不可。後述のcherry-pick連鎖にフォールバックする
（末尾から順に`reset --soft`を繰り返すと後続コミットにも影響するため）。

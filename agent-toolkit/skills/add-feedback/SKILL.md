...
同オプションは複数回指定でき、位置引数との併用は拒否される。シェルの引用規則を経由しないため、
引用の終端ミスでコマンドが破損する経路を避けられる。
本文をシェルの位置引数で渡す場合はANSI-Cクォートの単一引数とし、ヒアドキュメント、パイプ、
リダイレクト、コマンド置換、言語別文字列リテラルの連結を用いない。警告又はエラーが出た場合は

...
- `config`でユーザーglobal設定を変更する場合は、他の全リポジトリへ影響する変更として扱う
...
- formatter・linter・tester・プロジェクト固有のカスタムチェックを含む対応ツール全般を個別に直接起動せず、
  pyfltrのサブコマンド経由で実行する。設定で無効化しているツールも直接起動すれば動作するため、
  設定による無効化は直接起動への防御にならない。特定ファイルのみを対象にする場合も同じ形でパスを渡す。
  対象プロジェクトの規範が直接起動を認める用途（デバッガー、最小再現、環境切り分けなど）を明示する場合は、当該用途に限り直接起動してよい

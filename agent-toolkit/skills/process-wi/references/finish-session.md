...
監査で同一セッションの是正を選んで成果物を変更した場合は、当該是正のcommit後に終端担当を再度起動し、版数更新、生成同期、push、CI及び必要な固有の終端工程を完遂させて「終端担当の起動と受領」節の受領と照合まで完了する。この公開後にsession-review又は自動レビュー監査を繰り返さない（監査を反復すると、同じ指摘に対する再取得と再公開が終端しないため）。AWIへの記録を選んだ場合は追加の公開工程を行わない。

...
review本文が概要と進行状況だけを述べ、成果物への処置を求める記述を1つも含まない場合は、当該本文を指摘なしと分類する。
指摘なしの分類は本文全体に対して行い、本文へ含まれる個々の指摘の分類と混在させない。
...
当該書き込みは、操作、対象及び範囲を明示した人間由来のWI `20260908-090053-001.md`で承認済みであり、監査のたびに確認経路へ送らない。
...
指摘なしと分類したreview本文へも書き込まない。書き込む根拠が本文に存在せず、投稿してもPull Requestを読む主体の判断材料が増えないためである。
...
指摘なしの本文を記録しないと、当該本文が以降の全ての監査で判定の対象に残り、同じ本文の読解を反復するためである。

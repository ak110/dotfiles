...
- 選定時に固定した集合は処理の終わりまでそのまま使い、通常の新着項目の追加とready一覧の再取得は行わずに保つ。処理中に`## 即時対応`の判定で即時対応が必要と確定した完結済みAWIと、`references/finish-session.md`が定める公開前の振り返りで投入された振り返り素材AWIだけは`references/run-lanes.md`の是正レーンとして追加する
...
1. 次セッションへ回す範囲の有無を、観測した問題箇所と同じキーワード、識別子又は条文タイトルで隣接文書と隣接コードを`rg`か`git grep`で1回だけ検索して確定する。意味的な類似探索へ広げない。
   ...
   - 1件以上なら、一致箇所の一覧を添えたAWIを`source: process-wi`で登録し、即時対応する箇所と次セッションで扱う範囲を本文で分ける。`agent-toolkit:session-review`が投入した振り返り素材AWIが同じ事象を候補に持つ場合は重ねて登録せず、そのファイル名を使う。
...
- 振り返り素材AWI（`source: session-review`で`## 問題候補`節を持つAWI）のレーン工程: `../session-review/references/lane-processing.md`。同じレーンが対策と再発防止策の実装まで完了し、項目専用の事後承認型UWIを投入してから終端する

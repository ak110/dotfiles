...
本スキルの起動中にユーザーが追加した指示は、照合と採否を終えた差分だけを同一主題の処理条件へ反映する。照合と採否は`agent-toolkit:review-standards`の`references/reviewee.md`「指摘発生時の扱い」に従う。追加指示の範囲と論理関係の解釈は`agent-toolkit:confirmation-and-uwi`の`references/user-utterance.md`に従う。直接実装を求める表現も、同一主題である間は本スキルの処理を継続する追加条件として扱う。主題の継続と直接実装との境界が不明な場合は、変更の前に確認する。
...
即時対応の要否と優先順位は、欠陥を持ち越した場合にこのセッションの残りの工程と次セッションの進行へ与える影響で決める。影響は工程の停止、遮断、手戻り及び所要時間で測り、このセッションの経過時間、実装規模及び検証の作業量は判定に使わない。いずれの進行も妨げないものはAWIとして登録し、次セッションの正式な修正へ回す。判定基準を本節が定めているため、メインが自ら確定し、確認経路へ送らない。

即時対応は、残りの工程が同じ問題で再び止まらない状態にする局所修正とし、変更範囲を観測した問題箇所に限る。類似箇所の探索、横展開及び再発防止策は次セッションの正式な修正へ回す。この限定は`agent-toolkit/rules/01-agent.md`「完遂と先送り」に対する本スキル内の例外である。範囲を広げると、選定工程で固定した集合の残工程を圧迫し、その集合を完遂できなくなる。

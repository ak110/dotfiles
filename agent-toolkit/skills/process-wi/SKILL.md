...
処理中に見つかった作業品質や効率へ大きく影響する問題（規範・手順の矛盾など）、処理中に届いたユーザーの是正要求、`agent-toolkit:session-review`の改善案は、このセッションで即時に対応するか次セッションへ回すかを本節で判定する。ユーザーが時機を明示した場合はその指示に従う。本セッションの作業で導入した不良は`agent-toolkit/rules/01-agent.md`「完遂と先送り」に従い同じセッションで扱い、本節の判定で持ち越さない。
...
1. 次セッションへ回す範囲の有無を、観測した問題箇所と同じキーワード、識別子又は条文タイトルで隣接文書と隣接コードを`rg`か`git grep`で1回だけ検索して確定する。意味的な類似探索へ広げない。
   ...
   - 1件以上なら、一致箇所の一覧を添えたAWIを`source: process-wi`で登録し、即時対応する箇所と次セッションで扱う範囲を本文で分ける。`agent-toolkit:session-review`が既に登録した項目は重ねて登録せず、そのファイル名を使う。

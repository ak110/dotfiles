...
メインは、上流へ投入する要求と確認境界の候補位置を除き、フィードバック本文、項目別の採否理由、候補の調査過程を含む対象実装の調査結果を受け取らない。
...
`needs_escalation`の場合は、pickerの結果を変更せず、確認への回答かTBDの回答を受領して一般的な同一担当継続契約へ渡す。

pickerは要求の不採用と、既存commit若しくは既存実装による充足を確定しない。採否と充足の成否はレーンの計画担当が確定する。計画担当が全ての要求を不採用と確定した場合だけ、メインが`atk mq reject <filename>`を実行して保存結果を照合する。計画担当が全ての要求を既存の変更で充足済みと確定した場合は、メインが`atk mq adopt <filename>`を実行して保存結果を照合する。キュー入力の不整合と、本文だけから明白なユーザー選好又は外部待ちだけをpickerが`needs_escalation`で返し、メインが確認する。

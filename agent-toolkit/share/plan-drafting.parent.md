...
いずれかが実在しない場合は計画担当を起動せず、実在しなかった絶対パスを呼び出し元へ返す。
...
起動前の検査で構造検査スクリプトの不在を検知した場合は、計画担当を起動せず、同一セッション内で計画工程を続行せずに、処理中のキュー項目を未処理のまま残して終端する。

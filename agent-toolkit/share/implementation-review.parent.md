...
実装レビュー担当は、初回起動と引き継ぎ再レビューのいずれも`agents_server`の`start`へ`model_type="execute_review"`を渡して起動する。
...
`レビュー基準: CI記録`では新しい計画と計画レビューを作成せず、同節の独立CI実装レビュー表を準備して更新する。同じ再帰的CI失敗処理に既存の独立表があれば、原因commitが変わっても当該表と起点OIDを継続する。存在しなければ現在の原因commit完全OIDを起点として新設する。固定集合の別計画を保管先に用いない。収束後に正式な保存操作で`private-notes/plans/ci/`へ戻し、保存結果を正式に再取得して検証する。`agent-toolkit:process-wi`の③では、この保存操作を終端担当が実行する。再保存後の作業側の消失及びprivate-notesに未commit差分がないことまでを当該CI修正レビューの完了条件とする。
競合の通知を受領した場合は同じ操作を繰り返さず、保存元の現在の内容と作業側の差分を確認してから反映内容を確定する。

全実装単位の実装レビューが指摘0件で収束するか、修正後に再レビュー不要として収束した後に最終化する。
`レビュー基準: 計画`では、実装レビューで確定した変更と実装時の進捗を作業rootの計画へ反映し、

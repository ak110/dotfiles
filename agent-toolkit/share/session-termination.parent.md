...
再度の起動ではCI確認を次セッションへ委ね、当該セッションでCIの終端を待たない。委ねたCIは、次セッションの③で初回の終端担当がベースbranchのHEADについて確認する。
...
- CI確認の区分。初回の起動は`同一セッションで確認`とする。振り返りの即時対応と自動コードレビュー監査の是正による再度の起動は`次セッションへ委ねる`とし、当該再度の起動で固有の終端工程を渡す場合だけ`同一セッションで確認`とする
...
- `CI確認`へ`同一セッションで確認`を渡した場合は、`ci_result`が`成功`であり、対象リポジトリのCI照会手段が`pushed_head`について同じ結論を返す。`次セッションへ委ねる`を渡した場合は、`ci_result`が`次セッションへ委ねる`であることだけを確認し、対象リポジトリのCI照会手段を用いない

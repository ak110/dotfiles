...
`マージあり`では`完成条件証拠`として収束した実行レビューが返したJSONの絶対パスを追加で受領する。統合開始時に読み、採用予定のAWIについて、完成条件と`wi_conditions`の対応、元のユーザー発言の要求単位と`user_requirements`の対応、各行の判定と証拠の参照可能性を検査する。元のユーザー発言の要求単位は逐語引用、ユーザーコメント及びUWI回答から取得し、各単位の`達成`を採用終端の条件とする。完成条件も原則として`達成`を要する。リポジトリ全体の検査の成功を述べる条件は`公開工程CIで判定`の区分を確認し、その区分をマージとAWIの採用終端の阻害条件から外す。この区分以外の不足、未達又は証拠不足があればマージとAWI終端へ進まず、不足したAWIと条件又は要求単位を`reason:`へ書いて返す。

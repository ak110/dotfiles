...
```text
必須入力名: 統合区分,計画最終化,計画型変換,終端区分,延期adopt対象,対象リポジトリ,回収対象,権限
```
...
最新のマージ先tipへrebaseする。競合を解消して`git rebase --continue`を実行する場合は、その直前に`git log --oneline --decorate`を単独で実行し、履歴と継続対象を確認する。
...
同じ時点で、受領したAWIファイル名の集合が`adopted`、`rejected`、`終端しない`の3区分へ過不足なく分配され、同じファイル名が2つ以上の区分へ現れないことを1回のコマンドで検査する。受領した所有worktree、所有branch、レーンmanaged-tempの集合が`released`と過不足なく一致することも同じ検査で確認する。いずれかが成立しない場合は5行を返さず、集合の差分と重複したファイル名を添えて呼び出し元へ差し戻す。

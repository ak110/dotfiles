...
- `計画最終化`: 計画最終化の要否と、`atk plans commit`へ渡す計画作業root直下のメイン計画ファイル名
- `計画型変換`: 計画型変換の要否と、`atk wi convert-to-plan`の`--plan-file`へ渡す可搬値及び対象AWIファイル名一覧
- `終端区分`: AWIファイル名ごとの終端区分（`adopt`、`reject`又は`終端しない`）と、`adopt`で用いる完全OIDの決定規則
...
- `対象リポジトリ`: 記録した対象リポジトリの絶対パス
- `権限`: 作業対象リポジトリへのpush不可
- 完了報告と成果物を日本語で書くこと
...
競合解消の再レビューを要する`対応完了`を受領した場合は、呼び出し元が受領と同じターンで、当該レーンの実行レビュー担当のthreadへ`再レビューせよ`と`round: <ラウンド番号>`の2行を送る。
受領から当該指示までの間に、競合差分の再取得、成果物の読解、別の委譲のいずれもしない。
競合箇所、競合内容、解消方針、実際の変更、直接影響範囲は統合担当がレビュー指摘管理表へ確定済みであり、レビュー担当が当該表から読むためである。
収束後は同じ許可のまま統合を続行させる。

...
追加分が1件以上の場合は、同じ`agents_server` sessionへ追加分の`record`欄と`line`欄の組及び本文の要点を`send_message`で配送し、振り返り担当が成果ファイルへ反映して再返却するまで待つ。別sessionを起動しない。振り返りの実行中に受領したユーザー介入も配送対象とし、類似見直しと再発防止策へ反映する。

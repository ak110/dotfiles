...
- 並列サブエージェントの完了通知の受領後、メイン側は`Edit`対象の全ファイルを事前に`Read`で再読込してから編集する
  - chezmoiのpost_applyフック・pre-commitフック・linter再書き込み等でメイン側ファイルキャッシュが古くなる事象を予防するため
  - 「File has been modified since read」エラーで`Edit`失敗・再試行サイクルが発生する事象を未然に防ぐ

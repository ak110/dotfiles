...
- 並列起動サブエージェントは`git push`・`gh release create`・`git push --tags`などの
  不可逆な外部公開操作を実行しない
  - 並列実行中の他タスク成否を知らないままpushすると部分状態がリモートへ公開されるため
  - 単独起動（並列を伴わない`foreground`起動）サブエージェントは本制限の対象外とし、
    計画ファイル`## 実行方法`に記載された`git push`を含めて委譲してよい
- サブエージェントは許可されたファイル内であっても、追記タスクで指示された追加内容の隣接以外の
  H2/H3節の追加・削除・改名・順序変更などの構造変更を独自判断で実行しない
  - タスク内で必要と判断した場合は完了報告に改修候補として記述してメイン側へ返却する
  - 他ファイルからの参照を持つ可能性のある節はリンク欠落の追従修正が発生する前提で扱う
- サブエージェント完了報告に`Autocompact is thrashing`・`autocompact thrashing`などの
  異常終了文言が含まれる場合、当該サブエージェントを完了とみなさずメイン直接実装などで同タスクを再実施する
  - 同じプロンプトで再起動しない
  - 呼び出しプロンプトを縮減（読込対象ファイル数の削減・規範スキルの限定・
    `subagent_type: claude`への切替など）して再起動する選択肢も検討する
- サブエージェント完了報告の`tool_uses`欄が`0`または`result`欄が起動プロンプトの再掲のみを含む場合、
  当該サブエージェントを完了とみなさずメイン直接実装または起動プロンプトを縮減して再起動する
  - 再起動時の縮減方針は上記「Autocompact is thrashing」バレットのサブバレットに従う
...
- 外部主体によるファイル書き換えが発生した後、
  メイン側は`Edit`対象の全ファイルを事前に`Read`で再読込してから編集する
  - 対象契機は、並列サブエージェント完了通知後・pre-commit実行後・chezmoi apply後・
    機械チェック（textlint・pyfltr auto-fix等）実行後などを含む
  - chezmoiのpost_applyフック・pre-commitフック・linter再書き込みなどで、
    メイン側ファイルキャッシュが古くなる事象を予防するため
  - 「File has been modified since read」エラーで`Edit`失敗・再試行サイクルが発生する事象を、
    未然に防ぐ
...
plan modeでも計画立案に必要な調査・検証を自粛せず、「制約で実行できない」を理由に推測で計画を進めない。

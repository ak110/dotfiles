...
- サブエージェントは`git push`・`gh release create`・`git push --tags`などの
  不可逆な外部公開操作を実行しない
  - 単独起動で検証・コミットまで委譲する場合も`git push`はメイン側が全タスク完了後にまとめて実施する
  - 計画ファイルの`## 実行方法`に`git push`が記載されている場合もメイン側で実行する

...
- Claude Codeで委譲先の成果物側の状況を観測する場合は、
  ...
  当該コマンドは成果物側の補助観測であり、`ListAgents`による稼働状態の確認を置き換えない。
  成果物の更新時刻・行数・作業ツリーの差分件数・プロセス名の照合から稼働状態を推定しない
...
- Claude Codeで稼働中の委譲先へ継続を促す場合は`SendMessage`を使う。
  停止指示の成功だけでは停止成立を判定せず、`ListAgents`が返す稼働状態と
  成果物側の実測を共通規範の手順で照合する
...
- `TaskList`は`TaskCreate`で明示的に作成した工程の確認手段であり、Agent起動の委譲先を列挙しない。
  委譲先が稼働中でも該当なしを返すため、稼働状態の判定に使わない

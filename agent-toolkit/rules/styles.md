...
対象例: README・利用ガイド・CHANGELOG・コミットメッセージ・PR説明・調査レポート・コード内コメント・コーディングエージェント向け文書[^1]など。
...
JIS規格スタイルを指定しても、コーディングエージェントの出力には以下の癖が混入しやすい。意識的に排除する。
...
- 比喩動詞を使わない（技術的な動作を観測動詞で記述できるのに、行為や物理動作の比喩で書く例）
  - 推奨動詞例:「適用する」「採用する」「導入する」「反映する」「踏襲する」「整合する」

### 対比集の参照ポリシー

観点別の対比例（`agent-toolkit/skills/writing-standards/references/tone-examples.md`）の確認は、
メインエージェントから直接読まずExploreサブエージェント経由で参照する。
修正が必要なときは`plan-implementer`経由で実施する。
...
flowchart LR
    ...
    ADoc[コーディングエージェント向け文書] -->|常体・JIS| Agent[コーディングエージェント]
    Agent -->|常体・JIS| Dev
```
...
- 開発者向け: プロジェクトの前提知識を共有する開発者を想定し、初歩の説明は省くが、コード識別子レベルの詳細は扱わない
  ...
  - コーディングエージェントの発話: ユーザーが進捗・選択判断・成果物の状況を把握できる情報量を保つ。
    拡張思考も日本語を使用する。
    結論ファーストで構成し、挨拶・前置き・絵文字を含めない。
    指摘すべき事項は曖昧化せず率直に記述する
...
- 複数ファイルを横断的に修正・計画する場面では、ファイルごとに対象読者・文体・粒度が変わる。
  編集対象ファイルごとに対象読者を確認し、他ファイルの文体をそのまま適用する誤りを防ぐ
...
- コーディングエージェントの発話は応答本文・拡張思考がコンテキストへ残り後続生成の文体・言語へ影響するため、
  敬体や英語混在を避けて統制する
...
- 断定的で安定した語尾を使う（常体では「〜する」「〜である」「〜となる」「〜とする」「〜ものとする」）
  - 理由節（「〜ため」「〜から」「〜ので」）で文を結ばない。
    理由節は何かを修飾する従属節のため、これで結ぶと文として未完結となる。
...
- 頻出違反のため特に意識するtextlint規則:
  ...
  - かっこ類と隣接する文字の間のスペース（JTF 3.3）は、
    ...
    - Markdown箇条書きの継続行（2行目以降）で全角かっこ`（`が行頭に位置する場合も同違反として検出される。
      ...
      全角かっこを行頭に置かない
    - 改行を跨ぐ全角括弧の対（`（`と`）`が別の論理行に分かれる書き方）も同違反として検出される。
      対は同一行内に収める。長文で1行に収まらないときは、括弧句を行末側で完結させるか、
      括弧の前後で文を分割して別文に再構成する
  ...
  - 同じ漢字の7文字連続を避ける（ja-technical-writing/max-kanji-continuous-len）。
    ...
    - 4文字＋3文字の複合語結合でも検出される。助詞を挿入して分割する例:
      `修正対象外領域`→`修正対象外の領域`

...
<label>状態<select id="status"><option value="all">すべて</option><option>inbox</option><option>processing</option><option>adopted</option><option>rejected</option><option>answered</option><option>unanswered</option></select></label>
...
function actionAllowed(action,e){return(action==='remove'&&((e.kind==='feedback'&&['inbox','processing'].includes(e.state))||(e.kind==='tbd'&&e.state==='inbox')))||(action==='start-processing'&&e.kind==='feedback'&&e.state==='inbox')||(action==='adopt'&&((e.kind==='feedback'&&['inbox','processing'].includes(e.state))||(e.kind==='tbd'&&e.state==='inbox')))||(action==='reject'&&e.kind==='feedback'&&['inbox','processing'].includes(e.state))}
function batchKind(items){return items.length&&items.every(e=>e.kind===items[0].kind)?items[0].kind:null}
function updateActions(){const chosen=selected(),kind=batchKind(chosen);document.querySelectorAll('[data-action]').forEach(button=>{button.disabled=!kind||!chosen.every(e=>actionAllowed(button.dataset.action,e))})}
...
async function load(){try{const q=new URLSearchParams();for(const k of ['type','status','target_repo','category','source']){const v=$('#'+k).value;if(v)q.set(k,v)}entries=(await api('/api/entries?'+q)).entries;render();await status()}catch(error){showError(error)}}
async function detail(e){try{const item=(await api(`/api/entries/${e.kind}/${e.state}/${encodeURIComponent(e.filename)}`)).entry;current=item;$('#detail').textContent=item.content;$('#editor').value=item.content;$('#answer').value='';const editable=(item.kind==='feedback'&&['inbox','processing'].includes(item.state))||(item.kind==='tbd'&&item.state==='inbox');$('#editor').hidden=!editable;$('#save').hidden=!editable;$('#answer-panel').hidden=!(item.kind==='tbd'&&item.state==='inbox'&&!item.answered);$('#metadata').replaceChildren(...['target_repo','source','category','summary','updated_at'].flatMap(k=>{const dt=document.createElement('dt'),dd=document.createElement('dd');dt.textContent=k;dd.textContent=item[k]||'—';return[dt,dd]}));$('#editor').focus()}catch(error){showError(error)}}
...
$('#refresh').onclick=load;for(const k of ['type','status','target_repo','category','source'])$('#'+k).onchange=load;
...
$('#save').onclick=async()=>{if(!current)return;try{const path=current.kind==='feedback'?`/api/feedback/${current.state}/${encodeURIComponent(current.filename)}`:`/api/tbd/${encodeURIComponent(current.filename)}`;await api(path,{method:'PUT',body:JSON.stringify({content:$('#editor').value})});success('本文を保存しました');await load()}catch(error){showError(error)}};
$('#answer-save').onclick=async()=>{if(!current||!$('#answer').value.trim())return;try{await mutate(`/api/tbd/${encodeURIComponent(current.filename)}/answer`,{answer:$('#answer').value},'回答を保存しました');$('#answer').value=''}catch(error){}};
function batchBody(action,items){const body={filenames:items.map(e=>e.filename)},note=$('#batch-note').value.trim(),category=$('#batch-category').value.trim(),commit=$('#batch-commit').value.trim(),kind=items[0].kind;if(note&&(['adopt','reject'].includes(action)||(action==='remove'&&kind==='tbd')))body.note=note;if(category&&action==='adopt'&&kind==='feedback')body.category=category;if(commit&&['adopt','reject'].includes(action))body.commit=commit;return body}
document.querySelectorAll('[data-action]').forEach(button=>button.onclick=async()=>{const chosen=selected(),action=button.dataset.action,filenames=chosen.map(e=>e.filename),kind=batchKind(chosen);if(!kind){showError(new Error('FeedbackとTBDを同時に一括操作できません'));return}if(!confirm(`${button.textContent}（${action}）を次の対象へ実行しますか:\\n${filenames.join('\\n')}`))return;try{await api(`/api/${kind}/${action}`,{method:'POST',body:JSON.stringify(batchBody(action,chosen))});for(const id of ['batch-note','batch-category','batch-commit'])$('#'+id).value='';success(`${button.textContent}を実行しました`);await load()}catch(error){showError(error)}});
$('#commit').onclick=()=>confirm('外部編集をcommitしますか')&&mutate('/api/feedback/commit',{},'commitしました');
...
$('#new-entry form').onsubmit=async event=>{event.preventDefault();const form=new FormData(event.target),body={messages:[form.get('message')]},target=String(form.get('target_repo')||'').trim(),source=String(form.get('source')||'').trim();if(target)body.target_repo=target;if(source)body.source=source;if(newKind==='tbd'){body.scope=String(form.get('scope')||'').trim();body.question_type=form.get('question_type');if(body.question_type==='choice')body.choices=String(form.get('choices')||'').split('\\n').map(x=>x.trim()).filter(Boolean)}try{await api(`/api/${newKind}`,{method:'POST',body:JSON.stringify(body)});event.target.reset();$('#new-entry').close();success('追加しました');await load()}catch(error){showError(error)}};

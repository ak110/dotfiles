...
def _main() -> int:
    ...
    language_warning_body = _detect_response_language_warning(payload)

    def emit_json(result: dict) -> None:
        nonlocal language_warning_body
        if language_warning_body is not None:
            _append_additional_context(result, _llm_notice(language_warning_body, tag="warn"))
            ...
            json.dumps(
                {
                    "hookSpecificOutput": {
                        ...
                        "additionalContext": _llm_notice(body, tag="warn"),
                    },
                },
                ensure_ascii=False,
            ),
...
def _detect_response_language_warning(payload: dict) -> str | None:
    """直前メインエージェント応答が英語比率高めなら警告本文を返す。

    対象外条件:
    - payloadに`transcript_path`が無い／空文字列
    - payloadの`isSidechain`が真（サブエージェント呼び出し）
    """
    ...
    if payload.get("isSidechain") is True:
        return None
    return _response_language_check.check(transcript_path)
        ...
        if _colloquial_check.first_hit(value, _COLLOQUIAL_DENY_PATTERNS, _COLLOQUIAL_ALLOW_PATTERNS):
            print(
                _llm_notice(
                    ...
                    f" no metaphorical verbs) per styles.md."
                    f" Target: {file_path}",
                    tag="warn",
                ),
                file=sys.stderr,

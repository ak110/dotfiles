...
- 工程7（plan-integrity-checker / naive-executor / plan-impl-reviewer / codexレビュー）完了未達のブロック (block)
...
def _check_process7_completion_before_exit_plan_mode(session_id: str) -> bool:
    ...
    print(
        _llm_notice(
            ...
            " before completing Phase 7 (plan-integrity-checker / naive-executor /"
...
def _reset_process7_completion_flags(session_id: str) -> None:
    ...
    新計画への着手の合図として`_PROCESS7_COMPLETION_FLAGS`を偽へ戻す。
    """
    if not session_id:
        return

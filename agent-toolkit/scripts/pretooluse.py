...
- plan fileのWrite/Edit/MultiEditで本文の120字超過文のwarn通知 (warn)

EnterPlanMode:

- `process-feedbacks`スキル経由（`process_feedbacks_skill_invoked`真）でのEnterPlanMode発行のブロック (block)
...
def main() -> int:
    ...
    _check_plan_file_sentence_length_warn(tool_name, tool_input)

    # plan file `## 変更内容`・`### エージェント判断`配下の先送り含意動詞連結をブロック
    if _check_plan_file_no_deferral_expression(tool_name, tool_input):
        return 2

    # process-feedbacks経由の起動下でのEnterPlanMode発行は規範違反のためブロック
...
# --- plan file 本文の1文長 warn 検査 ---

# 閾値120字は既存textlint機械上限（.textlintrc.yaml sentence-length）と一致させる。
# 参照: agent-toolkit/skills/writing-standards/references/textlint-violations.md
_PLAN_SENTENCE_LENGTH_LIMIT = 120


def _check_plan_file_sentence_length_warn(tool_name: str, tool_input: dict) -> bool:
    """計画ファイル本文の各文長を検査し、120字超過の文をstderrへwarnで通知する。

    コードフェンス外の本文行のみを対象とし、`_plan_format.iter_markdown_body_lines`で
    フェンス・HTMLコメントを除外した行を句点（`。`）で分割する。
    各文が`_PLAN_SENTENCE_LENGTH_LIMIT`（120字）を超える場合は当該文の冒頭と行番号をstderrへwarnで通知する
    （事後検査で違反となる文を事前警告として検出する）。
    閾値120字は既存textlint機械上限（.textlintrc.yamlおよびtextlint-violations.md）と一致させる。
    複数違反時も先頭5件を通知しブロックは行わない
    （textlint事後検査の反復を予防する事前化のためwarnに留める）。
    警告発生有無をboolで返す（呼び出し側の`main()`は戻り値を使用しない）。
    """
    if tool_name not in _PLAN_FILE_EDIT_TOOLS:
        return False
    file_path_raw = tool_input.get("file_path")
    ...
    for lineno, line in _plan_format.iter_markdown_body_lines(content):
        # 句点`。`で文を分割し、区切り末尾の空要素は無視する。区切り文字は付けたまま長さ判定する。
        segments = line.split("。")
        # 最終要素は末尾に`。`が無い断片。それ以外の要素は句点付き文とみなす。
        sentences: list[str] = [seg + "。" for seg in segments[:-1]]
        tail = segments[-1]
        if tail:
            sentences.append(tail)
        for sentence in sentences:
            stripped = sentence.strip()
            if len(stripped) > _PLAN_SENTENCE_LENGTH_LIMIT:
                violations.append((lineno, stripped))
    if not violations:
        return False
    shown = violations[:5]
    shown_str = "; ".join(f"line {ln}: {s[:40]!r}... ({len(s)} chars)" for ln, s in shown)
    overflow = len(violations) - len(shown)
    tail_msg = f"; and {overflow} more" if overflow > 0 else ""
    print(
        _llm_notice(
            f"warning: plan file body contains sentence(s) exceeding {_PLAN_SENTENCE_LENGTH_LIMIT} chars"
            f" (matches textlint sentence-length limit; splits at `。`)."
            f" Split into shorter sentences before textlint runs post-hoc."
            f" Matches: {shown_str}{tail_msg}.",
            tag="warn",
        ),
        file=sys.stderr,
    )
...
def _check_process_feedbacks_blocks_enter_plan_mode(tool_name: str, session_id: str) -> bool:
    """process-feedbacks経由でのplan-modeネスト起動時のEnterPlanMode発行をブロックする。

    判定条件:

    - `tool_name`が`EnterPlanMode`
    - `session_id`が空でない
    - セッション状態の`process_feedbacks_skill_invoked`が真

    process-feedbacksスキルはplan mode外で実行する規範
    （`agent-toolkit/skills/plan-mode/SKILL.md`「plan mode移行の前提」節）を機械化する。
    フラグリセットは`agent-toolkit/scripts/posttooluse.py`が

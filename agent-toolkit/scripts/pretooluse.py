...
8. 口語的な日本語表現の混入 (warn, Write/Edit/MultiEdit)
   - 検出パターンは `scripts/_colloquial_words.txt` に、
     許容パターン (複合動詞・標準用語) は `scripts/_colloquial_words_allow.txt`
     に外出し。検出時は許容パターンに一致する部分文字列を取り除いてから
     検出パターンを適用する。検出語そのものは警告メッセージへ含めない
     (LLM コンテキスト汚染防止)
...
def _check_colloquial(tool_name: str, fields: list[tuple[str, str]], file_path: str) -> bool:
    ...
    Allowlist の正規表現に一致する部分を一旦取り除いてから denylist を当て、

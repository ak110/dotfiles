...
- plan mode下でplan-modeスキル未起動のままのplan file編集（Write/Edit/MultiEdit）のブロック (block)
- textlint-violations.md未読のままのplan file編集（Write/Edit/MultiEdit）のブロック (block)
- plan-file-guidelines.md未読のままのplan file編集（Write/Edit/MultiEdit）のブロック (block)
...
def main() -> int:
    ...
    # plan-file-guidelines.md未読のままplan fileを編集しようとした場合はブロック
    if _check_plan_file_guidelines_read_first(tool_name, tool_input, session_id):
        return 2
...
# --- plan file編集前のtextlint-violations.md未読をブロック ---


def _check_textlint_violations_read_first(
    tool_name: str,
    tool_input: dict,
    session_id: str,
) -> bool:
    """Plan fileを編集しようとした際にtextlint-violations.md未読の場合にブロックする。

    判定条件:

    - `session_id`が空でない（空ならセッション状態を取得できず判定不能のためスキップ）
    - セッション状態の`textlint_violations_read`が偽
    - `tool_name`が`Write` / `Edit` / `MultiEdit`のいずれか
    - 対象の`file_path`が`~/.claude/plans/`直下の計画ファイル

    `agent-toolkit:writing-standards`の`references/textlint-violations.md`を
    一度Readするとフラグが設定され、以降のplan file編集はブロックしない。
    ...
    print(
        _llm_notice(
            "blocked: attempting to edit a plan file without reading"
            " `agent-toolkit:writing-standards` reference `references/textlint-violations.md`."
            " Read it first to internalize frequent textlint violation patterns,"
            " then retry the plan file edit.",
            tag="block",
        ),
        file=sys.stderr,
    )
...
# --- plan file編集前のplan-file-guidelines.md未読をブロック ---


def _check_plan_file_guidelines_read_first(
    tool_name: str,
    tool_input: dict,
    session_id: str,
) -> bool:
    """Plan fileを編集しようとした際にplan-file-guidelines.md未読の場合にブロックする。

    判定条件:

    - `session_id`が空でない（空ならセッション状態を取得できず判定不能のためスキップ）
    - セッション状態の`plan_file_guidelines_read`が偽
    - `tool_name`が`Write` / `Edit` / `MultiEdit`のいずれか
    - 対象の`file_path`が`~/.claude/plans/`直下の計画ファイル

    `agent-toolkit:plan-mode`の`references/plan-file-guidelines.md`を
    一度Readするとフラグが設定され、以降のplan file編集はブロックしない。
    `permission_mode`の値に依らず適用する（plan mode外でも計画ファイル編集時には同様に違反が起こり得るため）。
    """
    if not session_id:
    ...
    print(
        _llm_notice(
            "blocked: attempting to edit a plan file without reading"
            " `agent-toolkit:plan-mode` reference `references/plan-file-guidelines.md`."
            " Read it first to internalize plan file structure requirements,"
            " then retry the plan file edit.",
            tag="block",
        ),
        file=sys.stderr,

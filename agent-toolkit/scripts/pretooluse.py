...
- plan mode外でのplan-modeスキル呼び出しのブロック (block)
...
def main() -> int:
    ...
    # plan mode外でplan-modeスキルを呼び出した場合はブロック（任意ツール）
...
def _check_plan_mode_skill_outside_plan(
    ...
    """Plan mode外でplan-modeスキルを呼び出した場合にブロックする。

    plan-modeスキルはplan mode下でのみ呼び出される設計のため、
    通常モードでの呼び出しを未然に止める。
    `permission_mode == "plan"`の場合は何もしない。
    """
    if tool_name != "Skill":
        return False
    skill_name = tool_input.get("skill")
    if not isinstance(skill_name, str) or skill_name not in _PLAN_MODE_SKILL_NAMES:
        return False
    if permission_mode == "plan":
        return False
    print(
        _llm_notice(
            "blocked: `agent-toolkit:plan-mode` skill is only callable in plan mode."
            " Call `EnterPlanMode` first to enter plan mode, then invoke the skill.",
            tag="block",
        ),
        file=sys.stderr,
    )

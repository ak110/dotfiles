...
def test_at_limit_passes(tmp_path, monkeypatch):
    monkeypatch.setattr(check_doc_size, "_load_exceptions", _empty_exceptions)
    path = _write(tmp_path, "at_limit.md", check_doc_size.LIMIT)
    assert check_doc_size.main([str(path)]) == 0


def test_over_limit_fails(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr(check_doc_size, "_load_exceptions", _empty_exceptions)
...
def test_short_passes(tmp_path, monkeypatch):
    monkeypatch.setattr(check_doc_size, "_load_exceptions", _empty_exceptions)
    path = _write(tmp_path, "short.md", 1)
    assert check_doc_size.main([str(path)]) == 0


def test_exemption_skips(tmp_path, monkeypatch):
    path = _write(tmp_path, "exempt.md", check_doc_size.LIMIT + 100)
    exempt_set = {str(path)}
    monkeypatch.setattr(check_doc_size, "_load_exceptions", lambda: exempt_set)
    assert check_doc_size.main([str(path)]) == 0

...
HELP: dict[str, dict[str, str]] = {
    ...
    "atk plans list": {
        ...
        "description": "目的: 計画作業rootに残る計画ファイル（メイン）を、所有セッションと計画バンドルの最終更新時刻とともに列挙し、保存先へ未反映の計画を判別できるようにする。\n利用場面: セッション終了時の通知が対象としない、他のセッションが所有する計画と所有記録を持たない計画の滞留を調べるとき。\n対象と出力: `~/.claude/plans`配下を読み取り、標準出力へ1行1レコードのTSVで書く。各行はメイン計画の絶対パス、所有セッション識別子、最終更新時刻をこの順にタブ区切りで並べ、各セルはJSON値ではない。所有記録が無い計画の所有セッションは`なし`と書く。走査の前に、旧版が計画作業rootへ残したレビュー指摘管理表のロックファイルを削除する。計画ファイルは変更しない。\n前提: なし。計画作業rootが無い場合と対象が無い場合は何も出力しない。\n復元・後始末: 読み取りだけを行うため不要。",
        "epilog": "実行例:\n\n  atk plans list",
    },
    "atk plans migrate": {

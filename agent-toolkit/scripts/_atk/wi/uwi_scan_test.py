...
from _atk.wi import uwi_scan as _uwi_scan
...
class TestIsUwiAnswered:
    ...
    def test_returns_false_without_answer_body(self, text: str) -> None:
        assert _uwi_scan.is_uwi_answered(text) is False

    def test_returns_true_with_answer_body(self) -> None:
        assert _uwi_scan.is_uwi_answered(_entry(answer="回答本文\n")) is True
...
class TestScanActiveUwis:
    ...
    def test_filters_and_orders_entries(self, tmp_path: pathlib.Path) -> None:
        ...
        assert _uwi_scan.scan_active_uwis(tmp_path, _REPO) == _uwi_scan.ActiveUwiScan(
            entries=[
                _uwi_scan.ActiveUwi("a.md", False),
                _uwi_scan.ActiveUwi("b.md", True),
                _uwi_scan.ActiveUwi("c.md", False),
            ],
            complete=True,
        )
    ...
    def test_matches_legacy_paths_and_caches_repository_resolution(
        ...
        assert _uwi_scan.scan_active_uwis(tmp_path, _REPO) == _uwi_scan.ActiveUwiScan(
            entries=[
                _uwi_scan.ActiveUwi("current.md", False),
                _uwi_scan.ActiveUwi("legacy-a.md", False),
                _uwi_scan.ActiveUwi("legacy-b.md", True),
            ],
            complete=True,
        )
    ...
    def test_marks_unreadable_or_invalid_utf8_scan_incomplete(
        ...
        assert _uwi_scan.scan_active_uwis(tmp_path, _REPO) == _uwi_scan.ActiveUwiScan(
            entries=[_uwi_scan.ActiveUwi("c.md", False)], complete=False
        )
    ...
    def test_marks_directory_iteration_failure_incomplete(
        ...
        assert _uwi_scan.scan_active_uwis(tmp_path, _REPO) == _uwi_scan.ActiveUwiScan([], False)
...
class TestPrivateNotesRoot:
    ...
    def test_prefers_environment_override(self, tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert _uwi_scan.private_notes_root() == override
    ...
    def test_empty_override_uses_home_default(self, tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert _uwi_scan.private_notes_root() == root
    ...
    def test_handles_nested_mapping_and_folded_values(self, tmp_path: pathlib.Path) -> None:
        ...
        result = _uwi_scan.scan_active_uwis(tmp_path, "github.com/ak110/dotfiles")

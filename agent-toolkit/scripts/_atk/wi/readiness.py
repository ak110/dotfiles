...
from _atk.wi.formatters import _parse_target_repo
from _atk.wi.frontmatter import parse_frontmatter
from _atk.wi.uwi_scan import is_uwi_answered as _is_uwi_answered
...
def _load_referenced_terminal_entries(
    ...
    for state in (WI_STATE_ADOPTED, WI_STATE_REJECTED):
        state_dir = private_notes / state
...
def _effective_dependencies(
    ...
    if kind == "external-user":
        filename = legacy.get("uwi_filename")
...
def _legacy_dependency_is_satisfied(
    ...
    *,
    ...
    if kind == "external-user":
        ...
        return target is not None and target.kind == WI_TYPE_UWI and target.uwi_answered is True
    if kind == "external-upstream":
...
def calculate_readiness(
    ...
    *,
    ...
    all_active = _load_queue_entries(private_notes, None, WI_PROCESSABLE_STATES, resolver_cache)
    active = (
        all_active

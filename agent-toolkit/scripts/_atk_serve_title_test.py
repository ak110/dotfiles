...
import _atk_serve_title as serve_title
import pytest
...
def test_sets_and_restores_title_on_tty() -> None:
    ...
    stream = _Tty()
    with serve_title.console_title("atk serve :28766", stream=stream):
...
def test_restores_title_when_body_raises() -> None:
    ...
    stream = _Tty()
    with pytest.raises(RuntimeError), serve_title.console_title("atk serve :28766", stream=stream):
...
def test_uses_windows_api_branch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(serve_title.sys, "platform", "win32")
    entered: list[str] = []
    ...
    monkeypatch.setattr(serve_title, "_windows_console_title", fake_branch)
    stream = _Tty()
    with serve_title.console_title("atk serve :28766", stream=stream):
        pass
...
def test_osc_sequence_targets_window_title_only() -> None:
    ...
    assert serve_title._osc_set_title("x") == "\033]2;x\a"

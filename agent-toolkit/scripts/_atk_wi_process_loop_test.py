...
import _atk_wi_process_loop as _process_loop  # noqa: E402  # pylint: disable=wrong-import-position
import _atk_wi_repo as _repo  # noqa: E402  # pylint: disable=wrong-import-position
...
class TestProcessLoopPromptAndEnv:
    ...
    def test_resume_applied_to_first_session_only(
        ...
        with pytest.raises(SystemExit):
            atk.main(
                ["wi", "process-loop", f"--target-repo={myrepo}", "--no-update", "--resume"],
                home=tmp_path,
            )
    ...
    def test_resume_session_id_is_normalized(
        ...
        with pytest.raises(SystemExit):
            atk.main(
                ["wi", "process-loop", f"--target-repo={myrepo}", "--no-update", "--no-alerts", *resume_argv],
                home=tmp_path,
            )

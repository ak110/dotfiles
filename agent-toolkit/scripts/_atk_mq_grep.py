"""agent-toolkitプラグイン配下の`atk mq grep`コマンド用補助モジュール。
...
from _atk_mq_common import _iter_entries, _pull_with_recent_notice, _repo_lock
...
def _cmd_grep(args: argparse.Namespace, private_notes: pathlib.Path) -> int:
    ...
    matched = False
    for path, _, text, _, entry_type in _iter_entries(private_notes, _resolve_states(args.status), filter_repo, args.type):

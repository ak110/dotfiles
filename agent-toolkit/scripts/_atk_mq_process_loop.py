...
def _build_process_loop_prompt(
    ...
    if orchestrator == "codex":
        prompt += (
            "Codexオーケストレーターの処理として、起動時に固定したready集合だけを同じセッションで処理し、"
            "その全てを完了した時点で既存の終了工程へ進んでください。"
            "処理を妨げる実行基盤の不具合を検出した場合は同じgoal内で修正と実機検証を完了し、"
            "未完了のフィードバック処理へ戻ってください。"
            "再起動後も同じgoalを再開し、未完了工程から継続してください。"
        )
    return prompt

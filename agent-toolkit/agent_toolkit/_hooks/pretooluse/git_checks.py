...
if TYPE_CHECKING:
    from agent_toolkit._hooks.pretooluse.content_checks import _MANAGED_TEMP_MARKER
    from agent_toolkit._hooks.pretooluse.notices import _block_notice, _llm_notice
    from agent_toolkit._hooks.pretooluse.shell_checks import _PROCESS_KILL_UNSAFE_MARKERS


def _is_in_verified_managed_temp(file_path: str) -> bool:
    """真正性を検証できた管理対象一時領域の配下であれば真を返す。"""
    try:
        managed_temp = importlib.import_module("agent_toolkit._atk.managed_temp")
    except ImportError:
        return False
    try:
        ...
        return {
            "hookSpecificOutput": {
                ...
                "additionalContext": _llm_notice(
                    "`git log`へ`--decorate`を挿入して入力を書き換えた。"
                    "当該呼び出しの出力は、渡した入力の出力へref名の装飾が加わった内容になる。",
                    tag="notice",
                ),
            },
        }

...
def _check_bash_nested_code_string(command: str) -> bool:
    ...
    `agent-toolkit:hook-implementation`の「遮断・警告フックの成立条件」の第1段で復元できないと判定して遮断を維持する。
...
def _check_bash_python_code_string(command: str) -> bool:
    ...
    `agent-toolkit:hook-implementation`の「遮断・警告フックの成立条件」の第1段で復元できないと判定して遮断する。
...
def _check_bash_explicit_path_exists(command: str, cwd: str) -> str | None:
    ...
    `agent-toolkit:hook-implementation`の「遮断・警告フックの成立条件」の第1段が復元できる結果へ警告を求めるため、警告で返す。
    """
    if not cwd:
        return None

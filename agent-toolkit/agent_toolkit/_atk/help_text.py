...
HELP: dict[str, dict[str, str]] = {
    ...
    "atk agents wait": {
        ...
        "description": "目的: `agents_server`が出力する終端結果又は実行中通知を待ち、1回の巡回で回収できた全件を標準出力へ書く。\n利用場面: 委譲先の完了を前景又は背景ジョブで待ち、結果又は実行中通知を受領するとき。\n対象と出力: 自身の書込主体が所有する結果ファイルと通知ファイルを読み、回収できたものを1件1行のJSON Linesで書く。行は待機対象の識別子の昇順に並び、各行は`session_id`を持つ。最初の待機で起動中sessionと未回収結果を登録簿へ固定し、再発行でも終端結果の回収まで対象ごとの待機所有権を保持する。前景へ返した本文は同じ呼び出しで受領済みとし、次の逐次待機へ再配送しない。先行待機のlockへ競合した後発プロセスだけが、先行runの本文を1回回収できる。終端結果が無く通知だけを回収した場合は`status: running`を返す。`--output-file`を指定した場合は、当該JSON Linesを指定先へ保存し、標準出力へは保存先パスと保存した行数だけを書く。回収した本文は当該保存先に残るため、後続の工程と後続のセッションの判断の入力にできる。\n前提: 待機するsessionを起動した`agents_server`と同じルートセッションで実行する。環境と別名索引からルートを解決できない入れ子MCP起動では、起動応答の`root_session_id`を`--root-session-id`へ渡す。明示値は形式、状態ディレクトリの実在及び確認済み会話rootとの一致を検証する。待機の成立は、応答のいずれかの行が待機対象の`session_id`を持ち終端statusであることで判定する。待機対象の行が現れない応答は当該対象が未終端であることを示す。`status: running`では同じターン内に同じコマンドを再発行し、終端statusまで待機を継続する。待機対象登録が破損している場合は、当該登録を解放して理由を標準エラーへ書き、非0で終了する。会話rootとの対応を確認できず一覧が空の場合は、CLIが解決したrootを標準エラーへ書き、MCPの`list`を1回呼び出してから同じコマンドを再実行するよう案内する。対応を確認済みで、待機対象の登録が0件かつ`starting`を含む保持中sessionも0件の場合は、理由を標準エラーへ書き、待機せず非0で終了する。保持中sessionが1件以上ある場合は通常の待機上限まで待つ。対象不在の終了では標準出力へ何も書かず、同じコマンドを再発行せずに起動側の応答を確認する。\n復元・後始末: 回収した結果ファイルと通知ファイルは削除される。結果を保持しない`stop`とsession登録簿での喪失確定も待機対象登録を解放する。登録簿が終端を示し、未回収の結果も保持中の状態も無い対象も同様に解放し、待機中に全対象がこの状態になった場合は対象不在として同じく非0で終了する。終端statusでは追加の結果受領操作は不要である。",
    ...
    "atk managed-temp create": {
        ...
        "description": "目的: 所有者だけが読み書きできる一時ディレクトリを作成し、その絶対パスを標準出力へ書く。\n利用場面: SessionStartが通知するセッション単位の領域を使えない場合（通知を受け取っていない、別のnamespaceから同じパスへ到達させる、Git worktreeなど`.git`を含む領域を置く）に、作業ツリーを変更せずに中間結果を保存するとき。\n対象と出力: 一時rootの直下へディレクトリと管理情報ファイルを作成し、状態ディレクトリへ登録を書く。作成した絶対パスを標準出力へ書く。\n前提: `--prefix`は必須とし、受理条件は当該オプションの説明に示す。別のnamespaceへ渡す場合だけ`--root`で共有ディレクトリを指定する。\n復元・後始末: 最終更新から7日を超えた領域は`atk`の実行時に自動で削除するため、回収は不要である。早く削除したい場合と、自動削除の対象外である`.git`を含む領域は`atk managed-temp cleanup --path <絶対パス>`で削除する。",

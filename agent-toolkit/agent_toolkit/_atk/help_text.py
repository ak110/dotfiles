...
HELP: dict[str, dict[str, str]] = {
    ...
    "atk agents wait": {
        "summary": "委譲先sessionの終端結果又は実行中通知を待って1行で出力する",
        "description": "目的: `agents_server`が出力する最初の終端結果又は実行中通知を待ち、その内容を標準出力へ1行で書く。\n利用場面: 委譲先の完了を前景又は背景ジョブで待ち、結果又は実行中通知を受領するとき。\n対象と出力: 自身の書込主体が所有する結果ファイルと通知ファイルを読み、単一のJSON文書を1行で書く。最初の待機で起動中sessionと未回収結果を登録簿へ固定し、再発行でも終端結果の回収まで対象ごとの待機所有権を保持する。通知だけを回収した場合は`status: running`を返す。\n前提: 待機するsessionを起動した`agents_server`と同じルートセッションで実行する。`status: running`では同じターン内に同じコマンドを再発行し、終端statusまで待機を継続する。待機対象登録が破損している場合は、当該登録を解放して理由を標準エラーへ書き、非0で終了する。待機対象を1件も取得できない状態が続く場合は、委譲先の起動そのものが失敗した可能性を示すため、終端結果の待機上限とは別の短い上限で理由を標準エラーへ書き、非0で終了する。この終了では標準出力へ何も書かず、同じコマンドを再発行せずに起動側の応答を確認する。\n復元・後始末: 回収した結果ファイルと通知ファイルは削除される。結果を保持しない`stop`とsession登録簿での喪失確定も待機対象登録を解放する。終端statusでは追加の結果受領操作は不要である。",
        "epilog": "実行例:\n\n  atk agents wait",
    },
    "atk agents notify": {

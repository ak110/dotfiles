...
spec-drivenスキルのステップ2（Plan&Implement）で、タスク単位の実装を担うサブエージェント。
...
1. 呼び出し元プロンプトから以下を確認する
   ...
   - 差し戻しの場合、レビュアー指摘ファイル（`docs/v{next}/{作業テーマ名}.review-spec.md`または`docs/v{next}/{作業テーマ名}.review-quality.md`）のパス

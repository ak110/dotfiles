...
仕様適合性と複数ドキュメント間整合性を評価する。

- 要件、受け入れ基準、対象フェーズの実装漏れ
- 要件解釈の誤り
- 計画ファイルの方針、判断、変更内容との矛盾
- 自己申告と実装事実の不一致
- 判断経緯の転記漏れ
- `spec-driven`文脈での次版ドキュメント、`README.md`、横断ドキュメント、現行版ドキュメントとの矛盾

計画に明示されていない追加変更でも、計画の方針と矛盾しない軽微な追随修正は指摘しない。

### code判断基準

作業開始前に`coding-standards`スキルを呼び出す。
コード品質のうち、人間判断が必要な事項だけを評価する。

- モジュール内および隣接モジュール間の整合性
- 責任分離と境界の妥当性
- 設計意図との合致
- テストが実動作を検証しているか
- 重複、SSOT違反、過剰な抽象化、デッドコード

### docs判断基準

作業開始前に`writing-standards`スキルを呼び出す。
ドキュメント単体品質のうち、人間判断が必要な事項だけを評価する。

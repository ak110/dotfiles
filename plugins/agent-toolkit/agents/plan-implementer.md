...
- 実装タスク: 計画ファイルと指示内容に従い、指定範囲を変更する。
  format/lint/testの自己検証は行わない（検証は専用の検証タスクに集約されるため）
- 検証タスク: 呼び出し元から渡される検証手順（計画ファイル`## 実装・検証・レビュー`節の「検証手順」由来）を
  実行し、警告ゼロになるまで内部ループでauto-fix・原因解析・再実装する
...
4. format/lint/testの自己検証は行わない（検証タスクへ委ねる）

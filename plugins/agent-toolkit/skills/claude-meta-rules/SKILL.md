...
- CLAUDE.md本体は200行以下を目安にする。200行を超える場合はトピック別に `.claude/rules/` 配下へ分離する（常時読み込まれるためコンテキスト消費が大きい）
...
- スキルが自動起動しない問題を修正するとき、`description` の改善に着手する前にfrontmatterに `disable-model-invocation: true` が設定されていないか確認する。設定されている場合は `description` を変えても効果がなく、設計意図の見直しが必要になる

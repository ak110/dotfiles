...
description: >
  計画ファイル合意後の実装フェーズ・レビューフェーズの運用手順。
  `ExitPlanMode`直後、および計画ファイルに沿って実装へ着手するときに最初に必ず呼び出すこと。
  計画ファイルの`## 実装・レビュー工程`節から参照される前提で、
  BASE_SHA記録・タスク分解・`plan-implementer`順次委譲・format/lint/test合格・
  3レビュアー（`spec-reviewer`→`code-quality-reviewer`→`document-quality-reviewer`）直列起動の手順、
  レビュアー出力ファイルパス規約、コミット禁止指示をまとめている。
  単発の単純な実装（数行の修正・既存パターン踏襲など）では呼び出し不要。
---

# 計画後の実装・レビュー工程

`ExitPlanMode`で合意を得たあとに進める実装フェーズとレビューフェーズの詳細手順、
サブエージェント呼び出しテンプレート、レビュアー出力ファイルパス規約をまとめる。
計画ファイルの`## 実装・レビュー工程`節からは本スキルを参照する前提で、
SSOTは本スキル側に置く。

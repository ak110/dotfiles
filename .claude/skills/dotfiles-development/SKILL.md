...
- ローカルで全体の自動チェックが必要な場合の実行方法: `make test`
  ...
  - 検証は変更ファイルに対応する近接検証を先に実行する。公開前の全体検証はCIへ委ね、ローカルでは次の3件を実行する。CIの成功を確認して全体検証の結論を確定する
    ...
    - 変更ファイルの外に残ったPythonの静的参照の検出: `uv run --frozen pyfltr run --commands=ty`。対象ファイルを渡さず、`agent-toolkit/`を含むリポジトリ全体を対象にする。名前を削除・改名したモジュールに追随していない未変更のテストや呼び出し元は、変更ファイルだけを対象とする近接検証では検出できないため、Pythonファイルを変更するレーンでは計画の`近接検証`行へ含める。あわせてaridと同じく、各レーンの統合でfast-forwardの前に専用worktreeで1回、および公開工程のpush前に1回実行する

...
- `session_review_invoked` — PostToolUse(Skill)が振り返りスキル呼び出しを観測してキーごとに記録する辞書
  - `agent-toolkit:session-review`の観測は配布物PostToolUse（`agent-toolkit/scripts/posttooluse.py`）が記録する
  - `session-review-dotfiles`の観測はdotfiles個人フックPostToolUse（`scripts/claude_hook_posttooluse.py`）が記録する
  - 配布物Stop hook（`stop_advisor.py`）と個人Stop hook（`claude_hook_stop.py`）が各々の対象キーを参照し、
    振り返り誘導の重複抑止に使う
  - PostToolUse(EnterPlanMode)で新規作業区切りとみなして辞書全体を空へリセットする
   （配布物PostToolUseが担当。両系統が共有する辞書のため、配布物側のリセットで個人フック側の観測キーも一括クリアされる）

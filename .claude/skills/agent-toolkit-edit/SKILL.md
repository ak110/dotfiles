...
- 配布物文面を扱う計画（素材原文反映・規範追記・例示拡張・規定改訂案など）では、`## 変更内容`のdiff改訂後文面の確定前に+側文字列の固有名照合を実施し計画段階で一般化表現へ置き換える
...
計画ファイル本文の`## 実行方法`には、検証ステップの手前へ`scripts/agent_toolkit_bump.py {patch|minor|major}`の実行ステップを必ず含める。
...
- `session_review_invoked`: PostToolUse(Skill)が振り返りスキル呼び出しを観測してキーごとに記録する辞書
  - `agent-toolkit:session-review`は配布物PostToolUseが、`session-review-dotfiles`は個人フックPostToolUseが記録する
  - 各Stop hookが対象キーを参照して振り返り誘導の重複抑止に使う
  - PostToolUse(EnterPlanMode)で新規作業区切りとみなして辞書全体を空へリセットする（配布物PostToolUseが担当）
...
- `session_review_extension_pending`: dotfiles個人フックPostToolUse(Skill)が`agent-toolkit:*`または`session-review-dotfiles`スキル使用を観測して記録
  - 配布物Stop hook（`stop_advisor.py`）が参照し、真の場合は振り返り誘導の送出を抑制する。個人フック`claude_hook_stop.py`が併用前提の誘導を送出するため、配布物側の誘導重複を避ける目的で使う
  - 書き込み元はdotfiles個人フック、読み取り元は配布物。配布物は個人フック不在時の未設定状態でも従来通り動作する
...
個人フックに配置した場合は`share/claude_settings_json_managed.posix.json`および同`win32.json`の`matcher`に新しいツール名を追加する必要があるか確認する。
...
プレフィックス・サフィックスの規約は`agent-toolkit/skills/agent-standards/references/claude-hooks.md`の「コーディングエージェント宛てメッセージの標識」節に従う。

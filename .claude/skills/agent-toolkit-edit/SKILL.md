...
レビュー判定も同様に未プッシュコミット範囲（計画着手前から`HEAD`まで）の累積bumpで対応済みを判定する。
差分単体では判定しない。
...
上記「判定基準」節に基づく種別選定根拠のみ`### エージェント判断`欄へ1行で記述する。
具体的なversion数値（現状値・目標値の`x.y.z`形式）は書かず、
`scripts/agent_toolkit_bump.py`の実行結果に従う。
...
配布物プラグインが`bin/`配下でCLIを提供する場合の実配置先は
`~/.claude/plugins/cache/<marketplace>/<plugin-name>/<version>/bin/<cli>`となる。
`<version>`は更新ごとに変わる。
...
- `docs/guide/claude-code-guide.md`「agent-toolkit」セクションのチェック内容要約は、要約が変わる変更時に併せて更新する。
  対象は次を含む。新しいcheck追加・既存check削除・検出範囲の大きな変更・依存ツールの変更・新規プラグイン追加
- `install-claude.sh`の`FILES`・`install-claude.ps1`の`$files`・`agent-toolkit/rules/`配下のmdファイル一覧は完全一致を保つ。
  追加・削除時は両スクリプトを手動同期する（整合性は`install_script_ssot_test.py`検査、自動同期手段は持たない）

...
- 自指的な表現（「本リポジトリは〜」）・特定設定値の前提（「`shouldUsePoint: false`のため〜」）・
...
- `session_review_extension_pending`: dotfiles個人フックPostToolUse(Skill)が
  `agent-toolkit:*`または`session-review-dotfiles`スキル使用を観測して記録
  - 配布物Stop hook（`stop_advisor.py`）が参照し、真の場合は振り返り誘導の送出を抑制する。
    個人フック`claude_hook_stop.py`が併用前提の誘導を送出するため、配布物側の誘導重複を避ける目的で使う

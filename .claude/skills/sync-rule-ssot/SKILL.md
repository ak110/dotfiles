---
name: sync-rule-ssot
description: dotfilesリポジトリでagent-toolkitルールファイルを追加・削除・リネームする際に必ず使う。配布元ディレクトリがSSOTだが、`install-claude.sh`/`install-claude.ps1`のファイルリスト配列のみ追従が必要。「ルールを追加」「ルールを削除」「`rules/agent-toolkit`を変更」などのキーワードで自動トリガーしてよい
...
chezmoi経由の配布と `claudize` コマンドはディレクトリを丸ごと同期するため、ファイルの追加・削除・リネームでコード修正は不要。

ただし `install-claude.sh` / `install-claude.ps1` はワンライナー依存を増やさないため、配布対象のファイルリストをスクリプト内の配列として保持している。
この1箇所のみ、配布対象ファイルの増減に合わせて更新する必要がある。

...
- ログと標準出力へ渡すメッセージは、実行環境が決めるエンコーディングで符号化される。Windowsでは当該エンコーディングがCP932になる場合があり、符号化できない文字は代替表現へ置き換わり、内容を解釈できなくなる。当該メッセージへ書く文字は`str.encode("cp932")`が成功する範囲に収める。可否は文字種ではなく当該符号化の成否で判定する。漢字にもU+20BB7のように符号化できないものがあり、em dash・en dash・`✓`・`•`も符号化できない

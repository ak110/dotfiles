...
配布対象なら `.chezmoi-source/` 側を編集すること
（直接編集は次回 `chezmoi apply` で上書きされる）。
...
そのため、例えば`.chezmoi-source`配下のファイルの文言修正などは、
Conventional Commits的には厳密にはfixになるが、
記述の方向性があまり変わらないような軽微な修正は`chore`などにしてよいこととする。

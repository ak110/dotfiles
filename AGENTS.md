...
- 通常開発は`develop`で行い、リリースは`master`向けのPRで行う。`master`は必須CIを通過したマージコミットだけで更新する
  - `agent-toolkit:process-wi`では、次の2条件がいずれも成立する場合に`develop`から`master`へのリリースPRを作成し、マージまで実施する。判定と実施はメインが担う。導入の経緯と根拠は[日次リリースの自動実施](docs/development/operations.md#日次リリースの自動実施)にある
    - 第1条件: 選定工程の完了時点の`atk wi list --skip-pull`の標準出力を判定の入力とする。当該セッションが処理対象へ固定した集合の項目を除いた残りが、`state`が`hold`の項目と、`state`が`inbox`かつ`ready`が偽の項目だけである。選定工程の完了でAWIの状態を照合するために実行する当該コマンドの出力をそのまま判定へ用い、判定専用の実行を追加しない
...
実行時に適用する規範の正本は`agent-toolkit/rules/`配下と各スキルにあり、下記の文書は当該正本を代替しない。
記載された事項を実行時の判断根拠として用いる場合は、当該事項の正本が実行時に読まれる文書側に存在することを確認する。

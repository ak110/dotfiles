...
- `agent-toolkit/`配下・`agent-toolkit/rules/`配下・`.claude-plugin/marketplace.json`の編集時は
  Skillツールで`agent-toolkit-edit`を呼び出す
...
対話シェル起動時に`dotfiles-fb list --type=tbd --status=unanswered --skip-pull`を自動実行し、
未回答TBDを1件1行で画面へ通知する。
`--skip-pull`によりログイン時のリポジトリアクセスを避ける。
0件時は出力なしで終了する。

通知は次の全条件を満たした場合のみ実行される。

- 対話シェル
- `dotfiles-fb`コマンド存在（`command -v dotfiles-fb`）
- feedback-inbox有効（`dotfiles-fb status`が終了コード0）

`dotfiles-fb disable`でfeedback-inboxを無効化すると、
`dotfiles-fb status`が非ゼロ終了するため通知は自動的にスキップされる。

...
- `agent-toolkit/rules/`・`agent-toolkit/skills/`配下のMarkdownの編集時は`agent-toolkit-edit`スキル方針に従う
  - `agent-toolkit:agent-standards`・`agent-toolkit:writing-standards`を併用して呼び出す
...
対話シェル起動時に`atk fb list --type=tbd --status=unanswered --skip-pull`を自動実行し、
未回答TBDを1件1行で画面へ通知する。
...
`atk fb disable`でfeedback-inboxを無効化すると、
`atk fb status`が非ゼロ終了するため通知は自動的にスキップされる。

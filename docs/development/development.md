...
変更時はdocs側で同じ情報を再掲している箇所があれば同じコミット内で合わせて更新する。
...
`pytools`の`project.scripts`コマンド向けbash補完を`completions/_pytools.bash`に集約する。
ログイン時の`register-python-argcomplete`実行コストを避けるため、補完スクリプトは事前生成してリポジトリにチェックインする。

- 配置: `completions/*.bash`を`.bashrc`が機械的にすべて`source`する。コマンド追加時に`.bashrc`を編集する必要は無い
- 自動生成: `scripts/gen-completions.py`が`pyproject.toml`の`[project.scripts]`を読み込む。
  対応するモジュールに`# PYTHON_ARGCOMPLETE_OK`マーカーがあるエントリのみを補完登録対象とする
- argcomplete対応: 各CLIモジュール先頭に`# PYTHON_ARGCOMPLETE_OK`を置き、
  `pytools._internal.cli.enable_completion(parser)`を`parser.parse_args()`の直前で呼び出す
- 再生成: `uv run python scripts/gen-completions.py`
- 検証: `uv run python scripts/gen-completions.py --check`（pre-commitフックで自動実行）
- 手書き補完の追加: `bin/`配下コマンドなどに個別補完を入れたい場合は`completions/<name>.bash`を新規追加する。
...
- プラグインに入れた場合: `.claude/rules/agent-toolkit.md` のチェックリストに従い `plugin.json` の `version` bumpと
...
```bash
uv pip install --requirements ~/dotfiles/requirements.txt
ipython --profile=ipy
```

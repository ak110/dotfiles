...
- `run_after_post-apply.sh.tmpl` ↔ `run_after_post-apply-windows.ps1.tmpl`
...
- CIは`.github/workflows/ci.yaml`の該当ステップ・ジョブの`env.UV_FROZEN`で自動適用される
...
本プロジェクトはdotfilesをホームディレクトリに反映することが主な機能である。
そのため、例えば`.chezmoi-source`配下のファイルの文言修正などは、Conventional Commits的には厳密にはfixになるが、記述の方向性があまり変わらないような軽微な修正は`chore`などにしてよいこととする。

...
GitHub Actionsのアクションはコミットハッシュにピン留めして実行する（[pinact](https://github.com/suzuki-shunsuke/pinact)で管理）。
ローカル更新は`make update-actions`（mise経由で`pinact run --update --min-age=1`を実行）、

...
背景の実装担当では、実装担当が所有する全プロセスの終了確認、残る`TaskStop`対象の停止、
`ListAgents`による実装担当のエージェントの稼働なし確認の順で書込所有権を移す。
...
完了報告は委譲元が待機を解いた後に検収する成果物であり、事象の通知は委譲元が待機中に受け取って
並行する工程の判断へ使うためである。この効果は、委譲元が並行する工程を持つ実行形態で生じる。
委譲元が完了までブロックされる実行形態では並行する工程が無いため当該効果は生じず、規定は送信だけを求める。
通知の対象をホスト別規定で定めるのは、委譲先が自身の起動形態を観測できず、
起動手段を選ぶのは委譲元とホストだからである。
Claude Codeでは`Agent`ツールで起動した委譲先を対象とする。
名前付き定義へ受信側契約を複製しないのは、非forkサブエージェントが`01-agent.md`を自動で読み込み、
同じ規定を二重に保持すると改訂時に不一致を生むためである。
ホスト別の手段と宛先を共有規範と共有agent定義へ書かないのは、Codexの親宛先を実測できておらず、
未実測の経路を全ホストへ配布する実施契約へ含めないためである。
Claude Codeの対象、`SendMessage`及び`to: "main"`は`agent-toolkit/rules/99-claude-code.md`だけが保持し、
当該規定を持たないホストは完了報告だけで返す。

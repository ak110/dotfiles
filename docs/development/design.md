...
②ではメインが通常レーンの計画起草、計画レビュー、実装単位の実装、実装レビューの各担当を`agents_server`で直接起動し、工程遷移と同一threadの継続を自ら制御する。変更の作成は実装担当が担う。
実装工程へ進む各レーンは担当工程に必要な正本だけを受け取り、実装レビュー収束後の計画最終化、通常型AWIの計画型変換、最新ベースへのrebase、競合解消後の単一実装レビュー、メインから直列に許可されたffマージ、AWI行ごとの終端、所有資源の回収までを完了する。上流要求と対象リポジトリ側の要求が混在する項目は`adopt`の対象から除く。
...
`判定`の値と渡す`model_type`の対応は`agent-toolkit/share/implementation.parent.md`を正本とし、見出しを持たない既存計画を`execute_fast`へ写し、受信側が起動できない入力を残さない。
...
計画時は計画構造検査と計画レビューを分け、実装後は実装レビュー担当が同じ最終差分へ実装契約照合と成果物評価を順番に実施する。原因commitに対応する計画契約があれば当該計画を実装契約とし、なければ原因分析結果、修正認可根拠、対象の検証結果、変更目的、対象、公開契約及び入力計画を起動文へ直接記載したCI記録を実装契約とする。CI記録はファイルとして作成せず、計画なしCI修正commitに起因する次のCI失敗では直前の記録を継承して7項目を更新する。計画契約がない処理では、最初の原因commit完全OID由来の独立CI実装レビュー表を作業rootへ置き、再帰中に原因commitが変わっても同じ表と起点OIDを継続する。commitの生成主体と固定集合にある別計画の有無を照合基準又は表の選択に用いない。正式な取得・保存操作で表をprivate-notesへcommit・pushして保存結果を再取得し、作業側の消失及びprivate-notesのcleanまでを異種レーン統合後と外部操作だけの固定集合から生じた計画なしcommitの検証境界に含める。
...
メインは全実装単位と最終検証の完了後、初回実装レビューの起動直前に、計画の既存節、開始時点の完全OIDで特定した実体及び計画外の明示入力から`review_contract`を生成する。
各条項は独立して成否を判定できる粒度とし、契約本文又は一意な参照先と出典を起動プロンプトへ直接含める。専用成果物と計画ファイルの固定H2は設けない。
実装レビュー担当は計画と差分、生成済みの`review_contract`を受け取り、計画照合を終えた後に同じコンテキストで`review_contract`へ照合する。
...
- 中間の調整担当の維持: 計画レビューと実装レビューの調整を専用のサブエージェント定義へ置き、メインが当該定義を起動する案は採らない。
  メイン、調整担当、レビュー担当及び作業担当の3段構造となり、工程遷移と同一threadの継続の制御が段ごとに分かれる。
  制御の所在が分かれた結果、実装レビュー収束後の最終化の主体が一意に定まらずレーン間で挙動が分かれる事象と、
...
フックで強制せず、`runtime-routing.md`「工程別モデル設定」と実装レビューを起動するメインの手順で統制する。
...
新規計画は、全ての要求を充足済みと確定して実装工程を省略する場合は計画レビュー完了まで、それ以外は実装レビュー完了まで`~/.claude/plans/dd-{名称}-{小文字16進数4桁}.md`と同じstemの`.detail.md`を計画作業root直下で更新する。
...
全ての要求を充足済みと確定した計画と、同一セッションで実装しない計画は計画レビューが収束した時点で、それ以外の計画は実装レビューが収束した時点で、所有主体が計画stemのバンドルを`atk plans commit`へ渡す。同コマンドはprivate-notesへ移動して対象限定commit・pushを完了し、成功後に作業側を回収する。外側の運用がpushを禁止した場合は`--skip-push`で対象限定commitまでを完了し、作業側を回収する。同一セッションで実装しない計画を計画レビューの収束時点で保存するのは、当該計画を後続のセッション又は別の環境で処理する主体が、キューの`plan_file`から計画の実体へ到達できるようにするためである。
キューの`plan_file`へ値を記録する経路と`atk wi`の着手可否判定は、記録する可搬値をそのまま解決した保存先の実体だけを受理する。計画作業rootの同名実体へ解決するフォールバックは、既に記録された値を読む用途にだけ残す。検証と記録が別の解決経路を用いると、`atk plans commit`を実行していない時点でも登録が成立し、記録値が実体を持たないまま後続のセッションへ渡るためである。この制約により、`agent-toolkit:process-wi`の通常型レーンは計画型への変換を実装レビュー後の計画最終化の直後へ置く。変換の受理条件を計画作業rootへ広げる案は、記録値が当該セッション限りの領域を指すため採用しない。
...
`agent-toolkit/share/implementation.parent.md`は工程順と渡す入力だけを定め、判定条件は同節を参照する。

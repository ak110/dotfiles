...
advisorの推奨反映先は対象ファイル単位、代替案は概念比較、ライフサイクルコストは恒常費・副作用・保守費の概念評価に限定する。
ファイル内の節・関数・行、同期対象、実装根拠による代替案の不採用と契約同期の成立性はメインが実測して確定する。
この境界を保つため、advisorはtranscript証拠、受領済み文書、自動ロード済み規範だけを入力とし、リポジトリの実装・規範・テストを追加読解しない。
...
`session-review-advisor`は、既定の抽出結果に含まれる全`kind=user`イベントを出現順のまま`sequence`・`line`付きの独立inventoryへ保持し、各行の介入分類、観測事象、原因・介入前予防処置を意味評価する。advisorが`completed`を返した場合、メインはホスト別に解決したplugin rootの同じ抽出器へ受領済み`transcript_path`を一度だけ渡し、読み取り専用で再実行する。メインは抽出結果から作成した`(sequence, line)`列との全件被覆、出現順、介入対応行の双方向集合差、`activation.sequence < inventory_sequence`を構造検収する。advisorが`evidence_insufficient`を返した場合は既存の証拠不足報告経路を維持し、構造検収と「提案無し」の確定へ進まない。これにより、advisorの意味評価とメインの入力・参照構造検査を分け、候補統合によるinventoryの欠落、自由記述だけの発火指定、介入後の発火を「提案無し」の確定前に拒否する。
この契約は既存の抽出結果、advisor報告、既存の表示経路だけを用い、新しい永続状態・所有者・表示経路・証拠抽出機構を追加しない。候補統合は`proposals`の重複排除だけに限定し、`intervention_inventory`と介入対応行を減らさない。

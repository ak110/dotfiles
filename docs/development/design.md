...
未回答UWIを解除条件にする項目は、既存の有効依存を保ったままUWIを明示依存へ登録し、
回答まで着手可否を`blocked`にする。
回答済みUWIは回答が本文へ保存されたことを確認して先に終端する。
終端によって依存が解除された項目は、ファイル名でterminal状態のUWIを再取得し、保存回答を反映してから処理する。
各項目の`depends_on`は、当該項目より先に終端すべき実在するAWI又はUWIのファイル名を保持する。
依存先がUWIの場合は人間の回答待ちを、AWIの場合は先行成果の完了待ちを表す。
...
pickerはAWI本文、対象実装、計画の先行成果依存を調査し、レーン内の実装依存順を技術判断する。
実装担当は渡された要求と計画を知る一方、キュー全体の採否や他項目の開始時機を決定しない。

### 保留状態を経由する計画型変換

通常型AWIをファイル名で計画化する間は、対象を`hold`へ置く。
`processing`はprocess-loopが計画作成から実装までを所有する状態であり、`agent-toolkit:process-wi`は選定工程の選定結果を検収した直後に選定済み項目を一括で移す。
...
計画型へ変換する公開操作は、`atk wi edit`の`--plan-file`互換経路と`atk wi convert-to-plan`に限定する。
`atk wi convert-to-plan`による`hold`統合は正規の計画変換として受理する。

通常型AWIのファイル名を1件以上指定した場合はファイル名モードとし、全対象が同一対象リポジトリのinbox通常型awiであることを計画調査前に一括検証する。
検証後に`atk wi hold <filename>... --target-repo=<repo>`を1回実行し、ファイル名昇順で全対象を`hold`へ移す。
自然言語要件を受領した場合は、この状態を経由せず、新しい計画型AWIを追加する。

同一セッションで、既に扱った同じ計画又は計画型awiを後続の方針に基づいて改訂し、別の処理経路が明示されていない場合は、本スキルの再開として扱う。自然言語modeとファイル名modeのいずれでも対象リポジトリを実装せず、計画の更新を検収し、計画型awiの投入までを完了する。更新後に実装承認を求めず、投入したawiファイル名、計画ファイル及び実装へ着手していないことを固定完了報告として返して終了する。
...
CLIは全入力が同一対象リポジトリの`hold`通常型awiであり、計画の全awi素材と一致することを最初の書込み前に検証する。
activeなUWI素材は状態を変更せず、統合依存へ保持できる。
検証後は最古のawiへ計画型本文、`source: plan-and-add-awi`、`plan_file`、計画ベースの`target_commit`及び全統合元の外部依存を設定して`inbox`へ移し、残る統合元を同じcommitで除去する。
...
`atk wi reject`は、process-loop内で要求の全てを不採用と確定した項目だけに用いる。
採用済み内容を統合した元項目と、別リポジトリへ移管して投入先を検収した元項目は、統合先又は移管先をnoteへ記録して`rm`で除去する。
技術的な失敗、入力不足、外部条件待ちは不採用へ変換せず、UWI依存を追加してactive状態に保つ。
...
主題の継続又はAWI投入と直接実装の境界が不明な場合は、変更又は投入の前に確認する。
...
正のtimeoutは中断後の終端と結果を待つ。timeout超過時もsessionまたはbackend processを強制終了せず、後続の`wait`または終端後の`send_message`を許可する。終端結果の30分保持を過ぎた場合は結果本文だけを破棄する。保持期限の経過とsessionを所有する実行主体の終了は独立に起こるため、同じ`send_message`はいずれの場合も保持済みの実効条件から会話を暗黙に再開する。結果本文を保持したまま所有主体だけが終了した場合は、再開の応答へ直前結果を`previous_result`として含める。`previous_result`は、`wait`又は`kill`が当該結果本文を呼び出し元へ返していない場合だけ付ける。返し済みの結果本文を継続入力の応答で再送しないためである。`timeout=0`でも中断要求の配送と`turn_control_lock`の取得には270秒の上限を適用し、終端は待たない。上限に達した場合は、中断要求が未配送か配送の成否が確定しないかを区別した`TimeoutError`を返し、sessionとbackend processは破棄しない。
...
目的は、委譲先が`agents_server`の`wait`で長時間の作業を待った場合に、MCPクライアントのツール呼び出し上限による失敗を避けることである。
構造の理由は、上限の判定に呼び出し元の申告ではなく実行環境の印を使う点にある。
`wait`は待機上限と呼び出し元の区分のいずれも入力として受け取らないため、上限の導出は呼び出し元の申告に依存しない。当該区分を入力として受け取る形では、渡し漏れた場合に上限が委譲先の側で長いままになる。
委譲先のセッションが起動するMCPサーバープロセスは`AGENT_TOOLKIT_DELEGATED_SESSION`と`AGENT_TOOLKIT_OWNER_SESSION`を継承するため、サーバー自身が当該印から委譲先の内側であることを判定できる。
知識境界として、`_common/wait_schedule.py`が既定上限の導出を担い、MCPサーバーは導出済みの値を待機へ使うだけとする。
委譲先は`wait`を再発行して待機を継続する。
却下した代替案は、全ての呼び出し元へ一律に短い上限を適用する案と、委譲先へ待機上限の明示を求める案である。
前者は当該上限の制約が働かない最上位セッションの待機を4分ごとの再発行へ分割し、後者は上限値の判定を委譲先の側の知識に依存させる。
...
送信は`atk agents-notify`が担い、委譲元は`wait`の応答の`notices`で受け取る。`wait`は終端前でも未回収の通知が現れた時点で復帰し、`status`を`running`のまま返す。呼び出し元は`status`だけで再待機の要否を判定し、`running`の応答へ同じ`session_id`の`wait`を再発行する。終端と同時に未回収の通知が残る場合は、`completed`、`failed`、`interrupted`のいずれかを返す応答へ`notices`と終端結果をともに載せる。呼び出し元は当該応答で通知の受領と終端の受領をまとめて完了する。応答へ載せた通知は状態ディレクトリから削除するため、同じ通知が二重に渡らない。
知識境界として、委譲先は宛先の解決と本文の送信だけを担い、回収の状態を持たない。委譲元は`wait`の応答だけから通知を受け取り、通知の取得のために別のコマンド、ポーリング又は追加の待機ループを実行しない。
却下した代替案は、通知口をMCPツールとして追加する案と、フックの発火を主経路とする案である。前者はCodexの委譲先にagents_server系のMCPツールが公開されないためengine横断で成立しない。後者はClaudeの委譲先だけが持つ発火機構であり、同じ理由で成立しない。委譲元が`wait`の外にいる間の通知は次の観測まで滞留するが、この滞留を解消するポーリングの導入は、通知の取得のために追加の待機ループを実行しないという要件と両立しないため採用しない。
...
`processing`項目は既に選定済みであるため除外条件を適用せず、優先が必要な場合はレーン割当又はレーン内の実装依存順で表現する。候補を、実装不要、処理中に新たに必要と判定した保留、計画型及び通常型へ分類する。要求の不採用と既存の変更による充足はpickerが確定せず、計画担当が確定する。
...
レーン工程ではメインが通常レーンの計画起草、計画レビュー、実装単位の実装、実行レビューの各担当を`agents_server`で直接起動し、工程遷移と同一threadの継続を自ら制御する。変更の作成は実装担当が担う。
実装工程へ進む各レーンは担当工程に必要な正本だけを受け取り、実行レビュー収束後の計画最終化、通常型AWIの計画型変換、最新ベースへのrebase、競合解消後の単一実行レビュー、メインから直列に許可されたffマージ、AWI行ごとの終端までを完了する。上流要求と対象リポジトリ側の要求が混在する項目は`adopt`の対象から除く。
採用する実施範囲を持たないレーンも、メインが起動する統合担当が計画レビュー後の計画証拠の保存とAWI行ごとの終端までを完了する。メインは統合完了の固定値を公開契約の終了状態へ照合し、照合の成立後に当該レーンの所有資源を回収する。所有資源の回収をレーンの完了条件へ置かないのは、統合担当の作業ディレクトリが回収対象のworktreeの内側にあり、自身の作業ディレクトリの削除を完遂できないためである。
...
競合時は同じexecutorと実装担当が関係計画の目的、実施内容、設計判断、完了条件を照合して最小限に解消し、同じ単一実行レビューを再度収束させてからffマージを要求する。

レーン工程のレーン実行中、メインは全レーンの状態把握、レーン間の調停及びユーザー判断への応答を保持し、
自身のコンテキストを消費する調査と、稼働中のレーンから独立した割り込み作業を委譲先へ移す。
AWI本文とレビュー指摘管理表を読む契機は、メイン自身が判断を確定する場面へ限る。
要求単位の由来はpickerが返した値を計画担当へ渡し、正本との照合は計画レビューが担う。メインが同じ本文を再取得すると、pickerと計画レビューが実測した同じ判定をメインのコンテキストで重ねることになるためである。

pickerの出力は、対象リポジトリのプロジェクト規範が選定結果とともにメインへ渡すと定めた内容を`project_notes`として運ぶ。目的は、プロジェクト規範がメインへ課す義務のうち選定時点でしか得られない情報を要するものを、選定工程の既存の1往復で成立させることである。構造の理由は、pickerが必須入力としてプロジェクト規範の絶対パスとAWI本文の双方を保持しており、当該判定を担える唯一の主体であることによる。知識境界として、何を載せるかの判定はプロジェクト規範が正本を持ち、`agent-toolkit/share/`配下の契約は受け渡しの形式だけを定める。同じ内容をレーンの完了報告へ載せる案は、レーンが並行して進むため他レーンの起動時点で当該内容が確定しておらず、レーン工程の起動へ間に合わないため採用しない。
...
実装単位の初回実装と近接検証は`execute_fast_model`へ割り当てることを既定とし、計画ファイル（詳細）の`### 実装担当のモデル区分`が`不可`と記載する計画では初回実装から`execute_model`を用いる。
区分を計画へ置くのは、計画の具体化の程度が計画ごとに異なり、委譲先が要する判断量を計画時点でしか確定できないためである。
区分の判定前に、利用者が観測する契約、認可及び計画の適用範囲を確定する。`不可`へ写像できる不足は、
対象ファイル若しくは内部識別子の追加探索、実装内部の設計判断又は近接検証の合否を確定する追加調査に限る。
起動側は`不可`の根拠をこの範囲へ照合し、外部契約又は認可の不足を含む計画を実装担当へ渡さない。
受信側は`不可`の根拠に記載された内部判断と追加調査を初回実装の職務として扱い、計画の欠落として差し戻さない。
当該不足が対象ファイル若しくは内部識別子の追加探索である場合、計画の`根拠`は探索範囲を確定済みの目的と適用範囲に属する直接影響範囲へ限定し、探索結果を変更対象へ加える認可も担う。
受信側は当該探索で特定したファイルを計画との差異として記録し、目的、適用範囲、ユーザー合意、公開契約、認可と安全境界を変えない範囲で実装する。
この境界を越える追加ファイルはエスカレーションし、対象探索を受理した後に同じ不足で往復する経路だけを除く。
区分を持たず常に`execute_fast_model`から始める案は、実装内部の具体化が不足する計画でエスカレーションと引継ぎの往復を必ず1回生じさせるため採用しない。
判定入力を計画の具体性だけに限り、工程別モデル設定が解決するモデル名と候補列を入力にしないのは、同設定が1キーへ複数候補を許し可用性で選択が変わるため、モデルの能力を基準にすると同じ計画の判定が実行時の候補選択で変わるためである。
`判定`の値と渡す`model_type`の対応は`agent-toolkit/share/exec.parent.md`を正本とし、見出しを持たない既存計画を`execute_fast`へ写し、受信側が起動できない入力を残さない。
fast担当がエスカレーションした場合だけ、
メインが`execute_model`の実効設定を解決し、継続条件に従って同じthreadの継続か新規fix担当の起動のいずれかを確定する。
...
同じ計画の実装単位は、最初の単位の直前にモデル区分が指すキーを1回解決した1つのfast担当が、先行依存と統合順に従って順に実装する。途中の単位をfix担当へ移した場合は、当該単位の完了後にfast担当の実効設定へ戻して残りの単位を続行する。
実装単位は同一worktreeへ書込主体を1つだけ置くため逐次実行され、単位ごとに担当を分けても並列化の利益が無い一方、規範、計画、対象コードの読み込みが単位数だけ重複するためである。
commit境界は計画の実装単位表を正本として維持する。単位ごとに実効設定を解決し直す案は、同じ計画の全単位が同じキーを対象とし解決結果が変わらないため採用しない。
...
fast担当からfix担当への移行は、初回起動に用いたキーと`execute_model`の実効`engine`・`model`・`effort`がすべて一致する場合だけ同一threadを継続する。
...
fix担当が実装単位を完了した後に残りの単位がある場合は、最初に保持した実効3値と現在のthreadを比較する。3値が一致する場合は同じthreadをfast担当へ戻し、一致しない場合はfix担当を終端して検収済みの先行commitと残りの単位を新規fast担当へ渡す。
...
pickerは要求の採否と、既存の変更による充足を確定しない。いずれも対象実装を読解して初めて確定するため、判定を計画担当へ置き、計画レビューで裏付けを検証する。誤って充足済みと確定した要求は`adopt`で終端して検知手段を失う一方、充足済みを未充足と判定した損失は計画側で実装単位を計上しない工程だけであり、損失が非対称であるためである。全ての要求を充足済みと確定した計画へ実装単位0件を許す案は、計画構造検査が実装単位表へ1行以上を要求するため採用せず、充足の確認だけを行う1行を置く形とする。pickerの挙動を検体で検証する契約テストを設ける案は、当該契約がタスク文書の文面として成立し機械検査の入力を持たないため採用せず、文書間の整合を検索で照合する。
計画担当は実施内容へ担当AWIを原則1ファイル1行で記録し、採否、採用範囲、実施しない範囲、理由を同じ行へ統合する。
...
独立した採否レビュー工程や専用の移管機構を追加すると、既存の項目別調査、計画担当の採否確定及び利用者確認経路が分断され、
...
振り返り用の参照文書の読込と所要時間目標の解決は、振り返り担当が受領したプロジェクト規範から自ら行う。メインが解決して担当へ渡す形は、担当の必須入力へ目標値と参照文書を加えることになり、担当が成果へ書く比較の条件を送信側の入力へ依存させるため採用しない。メインは目標の有無と目標値を成果ファイルの`## 対象セッション`から読み、`agent-toolkit:completion-report`へ渡す入力の保持要否を判定する。
...
抽出器の判定は、本文の表層一致ではなく記録側が持つ種別情報に基づける。`--warn`はフックの通知標識を持つ本文をフック実行の記録から得たものだけへ限定し、コマンド出力とファイル本文に現れた同形の文字列を警告として扱わない。あわせて、問題の不在を述べる本文を警告から除く。`final-result`の分類はCodexの記録が持つ`phase`を参照し、`commentary`のメッセージを最終結果として扱わない。表層一致だけで判定する形は、規範文書の記述例と委譲先の中間報告という実行時の問題でない本文を候補として提示し、原因を確定する工程を増やすため採用しない。
...
抽出器は、通常表示、警告、集計及びhook通知の走査を1回の記録読み込みでまとめて行う集約実行を持つ。集約実行は走査ごとの全量を呼び出し側が指定したディレクトリ配下のファイルへ書き、標準出力へは走査ごとの要約だけを返す。要約の範囲は、走査ごとの出力先と件数、通常表示のイベント種別ごとの件数、問題候補の特定に用いるイベントの位置と本文の冒頭、警告の分類ごとの件数、集計済みの走査の全量とする。知識境界として、どの走査を要約へ含めるかは抽出器が持ち、振り返り担当は要約だけで候補を確定できない場合に全量ファイルを読む。走査ごとに実行を分ける従来の形は、振り返り担当の実行ターン数が走査の数だけ増え、通常表示の全量が標準出力を経由して振り返り担当のコンテキストへ入るため採用しない。振り返り担当が全量から必要な集計を都度作成する形も、同じ集計を毎回作成し直す工程が加わるため採用しない。
...
`agent-toolkit:process-wi`は、当該セッション自身ではなく前のセッションを振り返りの対象とする。対象セッションが終了済みであるため計測範囲の終端が確定し、解析中に増えるユーザー発話の再照合と`--elapsed-until`の受け渡しが不要になる。公開工程の直列区間から振り返りと即時対応が外れ、次のセッションでは選定工程及びレーン工程と並行して実行するため、移送した工程が新たな直列区間を生じさせない。振り返りを当該セッションの公開工程に残す形は、終端担当の起動までの待ち時間を伸ばすため採用しない。
...
振り返りの対象は、起動された`agent-toolkit:process-wi`のセッションが保存済みのセッション記録から特定する。`atk wi process-loop`は子セッションのセッションID、transcriptパス及びthread IDのいずれも取得しないため、常駐ループ側で引き継ぐ経路は成立しない。`atk session-review-target`は、Claude CodeとCodexの記録から対象リポジトリで動いた自身以外の本体セッションを選ぶ。選定条件には`agent-toolkit:process-wi`の起動標識を持つことを加える。自身のセッションの開始より前に最後の更新を終えていることも同じ条件へ加える。条件を満たすもののうち更新時刻が最新の1件を返す。更新時刻の新しさだけを条件とする形は、消費側の振り返り担当が対象の終端と起動経路を前提とするのに対し、稼働中のセッションと当該スキルを起動していないセッションを返すため採用しない。終了の判定へ経過時間のしきい値を置く形は、常駐ループが直前のセッションの終了直後に次のセッションを開始するため、直前のセッションを対象から外すことになり採用しない。本体の判定は、Claude Codeでは最初の`user`レコードの`entrypoint`が`cli`であること、Codexでは先頭の`session_meta`レコードの`payload.originator`が`codex-tui`であることとする。可用性判定の記録と委譲先の記録はこの判定で除かれる。実行系の別を選定条件にしないため、常駐ループが実行系を切り替えても同じ探索で対象を特定できる。対象リポジトリごとに引き継ぎ記録を保持する形は、取得と完了の状態遷移を保つ手順を要するうえ、起動された側の探索1回で同じ対象を特定できるため採用しない。
...
調査担当は担当件数に応じて、2件以上なら同一対象リポジトリのキューID群を1回の`show <filename>...`で取得し、1件なら単数形で取得する。計画担当と初回計画レビュー担当はAWI由来素材があるときだけ
...
- 段階（対象）: 計画 → 実装
- レビュー契約: 計画レビュー、実行レビュー。実行レビューは計画照合と成果物評価を順番に行う

| 段階 | 工程名 | `track` | 走査順序と対象 |
| --- | --- | --- | --- |
| 計画 | 計画レビュー | `plan-review` | 独立要件の被覆を確認する |
| 実装 | 実行レビュー | `exec-review` | 計画照合の後、成果物評価を行う |
...
| レビュー表の系統値`exec-review` | 実行レビュー（計画照合と成果物評価を順に行う単一レビュー） |
...
実行レビューを単独で記載する場合は、計画照合と成果物評価を同じ担当が順番に行うことを含める。
...
計画時は計画構造検査と計画レビューを分け、実装後は実行レビュー担当が同じ最終差分へ実装契約照合と成果物評価を順番に実施する。原因commitに対応する計画契約があれば当該計画を実装契約とし、なければ原因分析結果、修正認可根拠、対象の検証結果、変更目的、対象、公開契約及び入力計画を起動文へ直接記載したCI記録を実装契約とする。CI記録はファイルとして作成せず、計画なしCI修正commitに起因する次のCI失敗では直前の記録を継承して7項目を更新する。計画契約がない処理では、同一のCI失敗処理の3件目の修正commitで最初の原因commit完全OID由来の独立CI実行レビュー表を作業rootへ置き、再帰中に原因commitが変わっても同じ表と起点OIDを継続する。commitの生成主体と固定集合にある別計画の有無を照合基準又は表の選択に用いない。正式な保存操作で表をprivate-notesへcommit・pushすることを、異種レーン統合後と外部操作だけの固定集合から生じた計画なしcommitの検証境界に含める。
計画構造検査は他の検査と同じシェル呼び出しへ連結せず、直接返された終了コード0だけを計画自己監査と計画レビューの開始条件にする。
実装契約照合では計画かCI記録からの逸脱、実装漏れ、未認可の挙動変化、生成・配布経路の追随漏れを確認し、成果物評価では`review_contract`に基づき正確性、回帰、境界条件、安全性、テスト不足・規範適合を確認する。
...
メインは担当の起動、同一threadの継続及び工程遷移を担い、レビュー担当と作業担当の完了報告を再検収しない。
レビュー担当はレビュー指摘管理表へ指摘レベル付きの指摘を記録し、指摘件数と要件仕様件数を返す。基準文書の読取で切り詰めの通知を受領した場合だけ、当該事実を示す1行を同じ返却へ加える。作業担当は詳細を成果物、計画、表へ記録して工程完了だけを返す。
...
計画担当は受領した承認範囲を計画の`## 変更履歴（計画時）`へ記録し、他の節は当該記録を参照する。承認範囲を計画へ後から追記すると各節の解釈が別々に確定し、同じ承認範囲をめぐる指摘が複数のラウンドへ分かれて現れるためである。
起草前に判定できなかった外部可視の変更は計画担当が差し戻す。メインは承認を取得して同じthreadへ返すか、UWIを登録して当該変更を保留とした計画の確定を指示する。

却下した代替案も同じ節へ記録する。
構造検査の終了コードを保存する新しいwrapperは、既存の独立コマンドと直接返却値の検収で要件を満たし、終了状態の保持と別の失敗経路を増やすため採用しない。
未承認のまま計画担当を起動して差し戻させる案は、起草前に判定できた変更でも起動と差し戻しの往復を生み、承認の取得時機が一意に定まらないため採用しない。
...
AWIの調査と項目別採否は計画担当が所有し、メインは計画ファイル初稿の確定後にレビュー修正ループへ入る。

レビューの役割固有契約を各SSOTへ残したまま、表のライフサイクル、`track`による帰属、モデル解決及び収束判定だけを
`agent-toolkit/share/review-loop-coordination.md`へ集約する。
レビュー種別ごとに同じ手順を複製する案は、ラウンドの境界とモデル解決の改訂が不一致になるため採用しない。
レビュー担当・レビューイーの契約を調整文書へ移す案は、知識境界と採否責務を混同するため採用しない。

差し戻しと停止の事由別の扱いも同じファイルへ集約する。
事由ごとに、当該レーンを終端するか回答を同じ担当threadへ返して継続するかと、キュー項目の扱い及び再開位置の記録要否を1つの表で定める。
同じ`needs_escalation`に対する終端と継続の2つの扱いが別々の文書へ書かれていたため、受領した主体が扱いを一意に選べなかった。
表を各文書へ複製せず参照とするのは、事由の追加時に扱いの不一致が生じないようにするためである。
停止で終える経路へ再開位置の記録を義務付けるのは、`processing`のまま残った項目を次のセッションが最初から実行し直すことを防ぐためである。
...
利用者確認を要する指摘は、メインが既存の確認経路で利用者へ確認する。

レビューのラウンド番号はメインが保持する値を正本とし、レビュー担当の起動、再レビューの指示、レビューイーへの指摘対応の指示のいずれにも`round`行として明示配布する。番号は収束判定、打ち切り上限、レビュー表の`round`列及び計画の`由来`欄で共通に使われる一方、各主体が自身のthread内の計数から導出できるため、正本を定めないと主体ごとに別の値が成立する。受信側が自ら採番する案は、計数のずれを突合できる基準が無く、実際に第3ラウンドの指摘が第2ラウンドとして記録される事象を生じたため採らない。番号を返却statusへ足す案は、指示から受信側への配布経路を設けないため採らない。

却下した代替案は次の2つである。

- 中間の調整担当の維持: 計画レビューと実行レビューの調整を専用のサブエージェント定義へ置き、メインが当該定義を起動する案は採らない。
  メイン、調整担当、レビュー担当及び作業担当の3段構造となり、工程遷移と同一threadの継続の制御が段ごとに分かれる。
  制御の所在が分かれた結果、実行レビュー収束後の最終化の主体が一意に定まらずレーン間で挙動が分かれる事象と、
  継続不能と判定した後に工程を進める手順が欠落して工程が停止する事象が発生した。
  当初はメインが調整を兼ねる案を却下し、ラウンドごとに計画全文とレビュー表がメインのコンテキストへ蓄積する構造を理由とした。
  この理由は現行の返却契約では成立しない。レビュー担当は件数の2行と、切り詰めを受領した場合の1行だけを返し、
  メインが表を読むのは要件仕様件数が1件以上のラウンドと第2ラウンド以降だけであり、計画全文とレビュー表のいずれもメインのコンテキストへ蓄積しないためである。
  Codexでは中間の調整担当が最初から存在せずメインが調整を担っており、ホスト間で構造を分けない利益も併せて得られる
- 新しい工程別モデル設定キーの新設: レビュー修正ループが解決する工程は既存の`plan_model`
...
ユーザーが明示した対象、範囲又は外部可視結果との差は、エージェント提案詳細の提示ではなく対応する確認済み回答でだけ確定でき、回答がない計画をレビュー完了にしない。
...
計画レビュー担当は受領した自身のタスク文書の絶対パスからplugin rootを導出し、同じrootの計画ファイル基準と`check_plan_file.py`を用いる。
...
新規計画メインの固定H2の名称と順序は`agent-toolkit/skills/plan-mode/references/plan-file-standards.md`を正本とし、本書へ再掲しない。
旧形式の見出しは読み取り互換に限定し、`## エージェント提案詳細`は実施内容のエージェント提案行へ1対1で対応させる。
...
一致判定が不一致を示した場合だけ、`atk review-table show`の`--format=jsonl`が返すデコード済みの値で差異を特定する。
...
規範が全文読解を要求する文書について、当該読解が成立しないまま後続の工程が確定する経路を2点で塞ぐ。
第1に、PreToolUseは`AskUserQuestion`の発行時に、`judgment-details.md`の読取を同一セッションで観測しているかを判定する。
観測していない場合は当該呼び出しを遮断し、当該文書の絶対パスを通知本文へ示す。
当該文書の発行前の全文読解は`agent-toolkit/share/rules-main.md`「確認を要する事項」が要求する。
観測の記録はPostToolUseが担う。
判定は次の順で行う。
稼働中のplugin rootを解決する。
当該rootへ文書の相対要素を結合して絶対パスを組み立てる。
`Read`の対象パスを実行ホストのパス規則で正規化する。
組み立てた絶対パスと正規化した対象パスの同一性を比べる。
`offset`と`limit`のいずれかを指定した呼び出しは観測としない。
同名の文書は作業ツリーの複製にも実在するため、末尾要素の一致と部分文字列の包含を判定に用いず、絶対パスの同一性だけを解除条件とする。
正規化は実行ホストのパス規則に従い、Windowsのバックスラッシュ区切りの絶対パスと大文字小文字の差も同じ経路で扱う。
読取手段を当該1経路へ限るのは、解除条件を、全文読解の成立を機械的に判定できる観測だけで構成するためである。
シェル経由の読取と部分読取は、パスの言及と全文への到達を区別できないため観測としない。
この限定により、シェル経由で読解した主体は当該文書を`Read`で読み直すまで遮断される。
第2に、計画レビュー担当と実行レビュー担当は、基準文書の読取で切り詰めの通知を受領した場合に、当該事実と全文読解の成否を完了報告の1行として返す。
呼び出し元は当該行が未到達を示す場合に当該ラウンドを収束させない。

知識境界は次のとおりとする。
遮断の対象とする工程境界と読解を要求する文書の対応はフックが持ち、全文読解を要求する条文そのものは各規範文書が持つ。
切り詰めの受領はレビュー担当だけが観測できるため、当該事実の申告はレビュー担当が担い、ラウンドの遷移は`agent-toolkit/share/review-loop-coordination.md`が担う。

却下した代替案は3つある。
1つは、`agent-toolkit/rules/01-agent.md`「判断の妥当性を評価する観点」が要求する全文読解も同じ遮断の対象へ含める案である。
評価の確定には機械的に観測できる工程境界が無く、遮断の発火点を定められないため採らない。
2つ目は、遮断を設けず規範の条文だけで全文読解を求める案である。
当該案は現行の状態であり、実行主体の申告以外の手段で読解の成立を検査できないため採らない。
3つ目は、`Read`の対象パスとBashのコマンド文字列が当該文書のリポジトリ相対パスを含むことを観測とする案である。
パスを出力するだけの呼び出しと部分読取も観測となり、全文読解の成立を判定できないため採らない。
...
前のセッションを対象とする振り返り、即時対応と自動コードレビュー監査の処置は、選定工程の開始時からレーン工程と並行して進め、公開工程の開始より前に完了する。全レーンのffマージとレーンごとの`adopt`・資源回収後に、メインは1件の終端担当へ委譲し、版数・生成物同期、統合後検証、push、CI及び終端工程を1回だけ実行させる。

計画ファイル（メイン）`## 検証区分`の`統合後検証`が挙げる検証は、レーンの専用worktreeで成立しない検証を含むため、終端担当が主作業ツリーで実行する。
終端担当は当該検証を、pushの後のCIが同値の検査として担うものと担わないものへ分け、担わないものだけをpushの前に1回実行する。
同値の判定は、CI定義の起動形、有効な検査の集合、当該検査が読む設定及び実行環境を照合し、当該検証の検査の集合がCIの集合へ含まれることを実測できた場合にだけ成立させる。
同値と判定したものはpushの前に実行せず、結論をCIの結論で確定する。同じ検査の二重実行は、当該実行の所要時間の分だけCIの開始を遅くするためである。
メインはレーン工程のマージ許可より前に各レーンの当該行を記録し、計画最終化で計画ファイルが移った後も値を保持する。
pushの前に実行した検証が失敗した場合は統合後検証失敗記録でCI修正と同じ経路へ進み、pushとCIの往復を経ずに修正する。
各レーンで実行させる案は、レーンで成立しない検証を含み、統合後の変更集合を対象にできないため採用しない。
起動形の名目上の一致だけで同値と判定する案は、CIが個別の検査を無効化している場合に当該検査の失敗を検出できないため採用しない。
CIが担う検証もpushの前に実行する案は、同じ検査の二重実行とCIの開始遅延を残すため採用しない。

`agent-toolkit:process-wi`は選定工程の開始時に前のセッションを対象とする`agent-toolkit:session-review`を起動し、振り返りの全即時対応を1件の委譲先で完了する。自動コードレビューも選定工程の開始時に1回確認し、確認時点の未対応指摘を是正するか、AWIへ記録する。review本文は状態を問わず全てのPR・MRを対象に取得する。inline commentとthreadは未解決threadを持つ対象だけを取得する。監査自体が是正済みと対応不要のthreadを解決するため、未解決threadの有無で限定しても未処置の指摘を取りこぼさない。取得はいずれもpaginationの終端まで行う。review本文の取得は横断クエリーの数回で終わるのに対し、inline commentの取得はPR数に比例した照会を要するため、限定の対象を後者に置く。新しいレビューの到着を能動的に待機しない。前のセッションが公開したPull Requestのレビューは次のセッションの選定工程で対象へ入るため、session-reviewとの実行順による限定を設けない。
...
公開対象が1つも無い処理回では、終端担当を起動せずに公開工程を短絡する。処理対象、回答済みUWI、固有の終端工程、延期`adopt`及びレーンの成果がいずれも無い場合、終端担当が実行する版数更新、生成物同期、push、CI確認及び固有の終端工程はどれも対象を持たない。短絡の可否は、終端担当がpush後に観測するのと同じ4項目をメインが取得して判定する。判定項目を別に定義する案は、公開状態の検収基準を二重に持ち、片方だけが更新される状態を生むため採用しない。

手動で起動した`agent-toolkit:process-wi`では、pickerが固定したAWIの完成条件に操作、対象及び外部可視の結果が明示された終端工程を、当該セッションの承認スコープとして選定工程で保持する。計画担当と終端担当が同じ範囲を要求する場合は再承認を求めず、範囲が広がる差分だけを確認経路へ送る。手動起動の指示は当該固定集合の完遂を委任する指示であり、同じ範囲の承認を工程ごとに繰り返す経路はユーザーの操作を増やすだけで認可の範囲を変えないためである。自動常駐起動を同じ扱いにする案は、起動時点でユーザーが対象集合を確認していないため採用しない。
...
新規計画は、全ての要求を充足済みと確定して実装工程を省略する場合は計画レビュー完了まで、それ以外は実行レビュー完了まで`~/.claude/plans/dd-{名称}-{小文字16進数4桁}.md`と同じstemの`.detail.md`を計画作業root直下で更新する。
...
保存の契機に達した時点で、所有主体が計画stemのバンドルを`atk plans commit`へ渡す。契機は起動経路ごとに`agent-toolkit/skills/plan-mode/references/plan-file-standards.md`の`## 計画ファイルの保存と参照`が正本として定め、本節は当該契機を再掲しない。同コマンドはprivate-notesへ移動して対象限定commit・pushを完了し、成功後に作業側を回収する。外側の運用がpushを禁止した場合は`--skip-push`で対象限定commitまでを完了し、作業側を回収する。同一セッションで実装しない計画を計画レビューの収束時点で保存するのは、当該計画を後続のセッション又は別の環境で処理する主体が、キューの`plan_file`から計画の実体へ到達できるようにするためである。
キューの`plan_file`へ値を記録する経路と`atk wi`の着手可否判定は、記録する可搬値をそのまま解決した保存先の実体だけを受理する。計画作業rootの同名実体へ解決するフォールバックは、既に記録された値を読む用途にだけ残す。検証と記録が別の解決経路を用いると、`atk plans commit`を実行していない時点でも登録が成立し、記録値が実体を持たないまま後続のセッションへ渡るためである。この制約により、`agent-toolkit:process-wi`の通常型レーンは計画型への変換の時機を実行レビュー後の計画最終化の直後へ置く。変換の受理条件を計画作業rootへ広げる案は、記録値が当該セッション限りの領域を指すため採用しない。
...
保存済み計画を再び更新する場合は、所有主体が`atk plans checkout`で計画stemのバンドルを作業root直下へ取得する。
同コマンドは取得時点の保存側の内容を状態ディレクトリの取得記録へ複製し、`atk plans commit`はこの記録がある場合に取得元の相対パスを保存先として採用する。
保存側の現在の内容が取得記録と一致しない場合は、保存先と作業側のいずれも変更せず競合として停止する。
更新の確定は保存側の既存ファイルへ内容を書き込む形で行い、作成日時を並び順に使う計画一覧のために保存側のinodeを維持する。
取得記録は取得から保存の成功までだけ存在し、成功時に削除する。
取得時点の内容を保持せずGitの履歴だけで並行更新を判定する案は、未コミットの直接編集と履歴の書き換えを検出できないため採用しない。
原因commitに対応する計画契約がない場合は、新しい計画と計画レビューを作成せず、計画作業root直下の`ci-<起点OID>.exec-review.tsv`を用いる。
...
メインは検査済みの各単位を先行依存と統合順に従って実装担当へ渡し、各commitと実装単位名の対応を維持する。
...
却下した代替案は、呼び先用文書を各スキルの`references/`配下へ置く案である。計画レビュー担当の呼び先用文書（`agent-toolkit/share/plan-review.subagent.md`）のように複数のスキルから参照される文書の所在が一意に定まらず、同じ役割の起動手順が2箇所へ分かれるため採らない。

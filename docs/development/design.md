...
構造の理由は、同じ外部仕様を固定する箇所を1つの正本へ集約し、正本を経由しない固定を機械検査で拒否する点にある。口語表現検査の通知が検出語を示す部分は`agent-toolkit/agent_toolkit/_hooks/pretooluse/content_checks.py`の`colloquial_detected_terms_text`を正本とする。`_hooks/`配下の検体が同じ見出し語を直接書くことは`content_checks_test.py`の契約テストが拒否する。

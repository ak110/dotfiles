...
Codexの`running`は終端未観測の状態であり、補助観測値から停滞へ変換しない。待機報告の観測値は書込主体と対象範囲を伴い、作業ツリー全体値は並行更新を含む補助情報としてだけ扱う。ホスト観測手段は一覧掲載と実際の呼び出し成功を別に検証し、失敗時は自身が起動して識別子を保持した対象だけをfallbackで照会する。複数委譲先の能力比較は全対象への同一機械照会が成立した場合だけ確定し、成立しない場合は未検証を返す。
...
`ListAgents`を利用できない場合は、保持ID、完了通知、停止結果、成果物観測を照合する。
...
計画担当はcommit境界を設計し、構造検査は実装単位名の一意性、先行依存先の実在、統合順及び空欄を検査する。

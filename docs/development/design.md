...
異なる計画ファイルのレーン（1つの計画ファイル専用のworktreeと実装担当で構成する並列実装の単位）だけを別worktreeで並列化する。
...
計画担当はバッチ全項目の採否記録を受け取り、実施内容表へ全項目を行として記録し、採用系（`採用`・`部分採用`）の行だけを実装対象とする。
`source: session-review`だけをエージェント由来とし、その他のsource、sourceがない項目や不明なsourceの不採用は不採用確認用`user_decisions`として
...
各レコードは`id`、`raw`、`question`、`answer_or_tbd`、`unanswered`、`resolution`、`decision`を保持し、新しい回答・TBDを対応するIDへ追記して過去の確認サイクルのレコードを削除も上書きもしない。
...
保留項目を含む全項目の採否記録と採用範囲で計画起草を続行し、回答・保留結果を確認できない場合は元項目を終端しない。
...
起動文の命令は、受信者側の正本が定める手順の範囲を狭めない。委譲元が確定した判断の記録（固定7列TSVの`対応内容`欄など）を受信者への命令へ転写すると、受信者が所有する採否判定と類似箇所の一括修正が失われるためである。
...
調査担当は担当ファイルを1件ずつ取得し、計画担当と初回計画レビュー担当はフィードバック由来素材があるときだけ
...
実装盲検レビュー担当へ計画を渡さない知識境界を維持し、計画を保持する`plan-impl-executor`が対応要否と後半の対応欄の確定前に
モード別の作業前OIDと承認済み計画から修正認可の上限を検査する。
レビュー指摘の永続表は、ラウンド、重大度、指摘箇所、指摘内容、対応要否、対応内容、対応不要理由の固定7列TSVとする。
`指摘内容`へ根拠と適用条件を記録し、`対応要否`に応じて後半3列を更新する。
表の更新は`_file_lock.py`の既存排他制御を使用するため、指定された管理対象領域の検収ではレビュー表と`atk review-table`が付随して作成するファイルだけを許容し、TSVの7列構造・行数を個別に確認する。
準拠系と盲検系は、系統ごとに1つのTSVを全ラウンドで使い続ける。調整主体は初回strict検証の前に各表へ`atk review-table init <レビュー表>`を実行し、両系統の担当が終端するまで各表を分離する。
盲検系へ準拠系の表や出力を渡さず、両系統の終端後に調整主体が各表を個別に全文読取して系統・ラウンドの帰属を保ったまま統合する。ラウンド列を先頭4列の複合キーへ含めることで、未解消の同じ指摘を別ラウンドの行として同じ表へ再提示できる。
通常の実装モードでは、呼び出し元が実装レビュー用managed temp領域を作成し、その絶対パスを`plan-impl-executor`へ渡す。各レビュー担当の新規起動と同じレビュー担当への継続接続のいずれでも、今回の系統表の絶対パスを必須差分入力として渡す。
再レビューでは、レビュー担当が新規起動でも同一thread継続でも同じ系統表を検証・全文読取し、今回のラウンドとして同じ表へ指摘を追加する。レビューイーは修正対象となる全系統表を全文読取してから対応を確定する。
各実装担当以降のHEAD又は`review_contract`へ混入した未承認契約は認可根拠に含めず、

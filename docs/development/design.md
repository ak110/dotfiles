...
session登録簿は全sessionについて、終端結果を返せるかどうか、公開status、engine、作業ディレクトリ、model、effort、model_type、起動種別及びturn通番を原子的に記録する。再起動後の解決は、終端、実行中、不在、読取不能を区別する。終端が確定しない記録は前プロセスがturnを実行している可能性を排除できないため再開せず、`turn_unobserved`として返す。候補切替は起動応答の後にengine可用性のため終端した場合だけ行い、起動例外では作成済み資源を残す可能性があるため例外を返す。`stop`はbackend資源の解放を先に完了し、成功後に保持集合と状態ファイルを同期する。上り通知は送信時刻と乱数を含む一意な最終名へ原子的に公開し、破損した通知と終端結果は待機側が可視の診断として扱う。
...
目的は、委譲先が`agents_server`の`wait`へ`timeout`を渡さずに長時間の作業を待った場合に、MCPクライアントのツール呼び出し上限による失敗を避けることである。
構造の理由は、上限の判定に呼び出し元の申告ではなく実行環境の印を使う点にある。
`wait`の`request_bucket`は呼び出し元が渡す値であり、渡し漏れた場合は上限が委譲先の側で長いままになる。
...
委譲先は`timeout`を省略したまま`wait`を再発行して待機を継続する。
却下した代替案は、全ての呼び出し元へ一律に短い上限を適用する案と、委譲先へ明示`timeout`の指定を求める案である。

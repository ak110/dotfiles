...
Claude backendは、委譲先が背景作業を残してturnを終えた場合、その完了通知による同じsessionの再開を一度だけ待ち、再開したturnの結果を`wait`へ返す。待機中は最初の結果を内部へ保留し、`status`と`turn_completed`を終端へ進めない。これにより、`terminal`と`result_available`を使う既存の全経路が、個別の例外条件を持たずに同じsessionを実行中として扱う。背景作業の完了通知が届かない場合は、結果保持期間と同じ上限で保留した最初の結果を確定する。
委譲先が自身の子の完了を待ってターンを終える場合も、同じ自動再開の対象とする。子の起動は`task_started`として観測でき、`DEFERRING_TASK_TYPES`の`local_agent`と`local_workflow`が委譲先の子に当たる。背景シェルは終端statusへ到達しない場合があるため当該集合から除かれており、自動再開の対象にしない。背景ジョブの終了状態は同じturnの中で確定させる。
この自動再開はClaude backendだけが持つ。目的は、委譲先が子の完了を待つ場合に、呼び出し元が追加の継続要求なしにタスク文書所定の返却値を受け取れるかどうかを、実行経路ごとの外部契約として確定することである。構造の理由は、待機表明で終端してよい条件を実行主体が観測できる入力へ結び付ける点にある。判定入力は`_agents_server_state.py`の`AUTO_RESUME_NOTICE`とし、`_agents_server_claude.py`の`_build_options`だけが起動時の指示へ加える。当該指示は自身が起動した委譲先の完了待ちだけを例外とし、背景ジョブを対象から除く。Codex backendは`LAUNCH_SYSTEM_PROMPTS`を`developerInstructions`へ渡すだけで当該指示を持たず、Claude CodeのAgentツールも持たないため、いずれも待機表明を許さない既定側になる。2026年9月4日、Claude Code 2.1.260で、子を背景起動した直後に待機表明でターンを終える同じ指示を3経路へ渡した。Claude backendだけが子の完了後の返却値を`wait`へ返し、Codex backendとAgentツールは待機表明の行を最終の返却値として返すことを実測した。再検証は同じ3経路へ同じ指示を渡す。
...
同じディレクトリ配下の`results/<session_id>.json`へ、終端したsessionの結果本文を原子的に書く。内容へsessionごとに1から始まり新しいturnの開始ごとに1増える`turn_seq`と、終端時刻の`finalized_at`を含める。`wait`又は`kill`が結果本文を返した後も当該ファイルを保持し、終端結果の保持期限が到来した時点とMCPサーバーの終了時に削除する。`atk agents-wait`は待機対象のturnの番号を引数で受け取り、`turn_seq`が当該番号以上の結果が現れるまで待つ。ファイルの存在だけを成功条件にしないのは、保持期限内に同じsessionへ次のturnを開始した場合、前のturnの結果が残っているためである。配送を削除の契機にする案は、呼び出し元の`wait`が終端と同時に本文を返すため、背景で待機する`atk agents-wait`が読取前のファイルを失うので採用しない。Claude Codeの`/goal`は、ターンを終えた時点で当該セッションが起動した未完了のBash背景ジョブが1件も無い場合に目標評価を発動するため、MCPツールの背景移行だけで待つと待機のたびに評価が動いてトークンを消費する。`atk agents-wait`を背景ジョブとして起動すると当該評価が延期され、委譲先の終端でセッションが再開する。結果本文の配送記録は`wait`が持つため、`atk agents-wait`は配送済みの標識を更新せず、呼び出し元が完了通知の後に`timeout=0`の`wait`を1回発行して配送を確定させる。MCPサーバーへ待機専用のツールを足す案は、当該ツールの呼び出しも背景移行の対象となり目標評価の延期条件を満たさないため採用しない。
...
レビュー表の公開操作は`atk review-table --help`、8列TSVの構造は`_review_table.py`と`_review_table_test.py`を正本とする。
...
全レーンのffマージとレーンごとの`adopt`・資源回収後に、メインは振り返り、即時対応、自動コードレビュー監査の処置を完了する。その後に1件の終端担当へ委譲し、版数・生成物同期、push、CI及び終端工程を1回だけ実行させる。

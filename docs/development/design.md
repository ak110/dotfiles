...
全ての要求を充足済みと確定した計画は計画レビューが収束した時点で、それ以外の計画は実装レビューが収束した時点で、所有主体が計画stemのバンドルを`atk plans commit`へ渡す。同コマンドはprivate-notesへ移動して対象限定commit・pushを完了し、成功後に作業側を回収する。外側の運用がpushを禁止した場合は`--skip-push`で対象限定commitまでを完了し、作業側を回収する。

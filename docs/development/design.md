...
`agent-toolkit:session-review`のセッション内工程は、メインによる改善点の列挙と、準備スクリプト`agent-toolkit/skills/session-review/scripts/session_review_prepare.py`の1回の実行だけで構成する。準備スクリプトは証拠bundleの抽出、統計と既存キュー項目の取得、素材AWI本文の生成、`atk wi add --source session-review`による投入までを行う。候補の一次選別、原因分析、対策の実装と再発防止策は、素材AWIを取得した`agent-toolkit:process-wi`のレーンが`agent-toolkit/skills/session-review/references/lane-processing.md`に従って同じ項目の処理で完了する。2026年9月23日から25日のdotfiles向け親セッション21件では、振り返り担当が規範の読込、候補の読解、原因調査、AWI起草と投入、報告の組立をLLMの応答で直列に行い、1応答あたり約11秒から17秒、58回から237回の応答で580秒から3375秒を要した。応答回数が律速であり、記録長31秒の対象でも600秒前後を要したため、担当の構成を保ったまま機械化する案は300秒の目標に届かない。担当の起動そのものをセッション内工程から外し、分析をキュー経由でレーンへ移す構成だけが目標へ到達する。分析の総量は減らず、レーンの計画、実装、実行レビューの工程で深さを保つ。

素材AWIは、処理するレーンが元のセッション記録を開かずに判定と原因分析を完了できる本文とする。抽出器の個別証拠は、ユーザー介入、失敗したコマンドの診断、委譲返却、エスカレーションの本文を切り詰めず、hook通知など定型本文だけに上限を残す。記録位置（locator）、集約キー、証拠ファイルのパスは素材へ載せない。ユーザーコメントが、裏付けのためにログを探し直す契機になる細かい情報を書き過ぎないよう求めたためであり、準備時の一時領域は処理時に残らない。候補の参照には判定入力と対応付ける短い候補IDだけを使う。素材AWIは1セッションにつき1件で観測事象がセッション固有であるため、`agent-toolkit:wi-standards`の条件付き重複判定、起草完了ゲート及び投入前Challengerレビューの対象外とし、既存項目との照合は処理側が素材に載った一覧から行う。旧構成で振り返り担当の投入AWIを親が別の委譲先でレビューした工程は、担当の撤去に伴い消滅する。

素材AWIを処理したレーンは、候補ごとの問題、原因、対策、再発防止策及び見送りの理由をまとめた事後承認型UWIを項目ごとに1件投入してから素材AWIを終端する。素材AWIは投入時点で対策方針が確定していないため、2026年9月25日のユーザー要求により、ユーザーが項目単位で対策の可否を判断できる記録を残す。本文は`session_review_report.py`が素材AWIと判定JSON、分析JSONから決定的に生成し、候補集合の過不足、未判定の残存、除外の根拠、再発防止策の実体（実装済み成果物、未完了のAWI、確認中のUWI）の欠落を全件示して非0で終了する。ユーザー介入の候補は、単発の照会、判断を変えない追加要求、理由の無い拒否・中断の3区分だけを根拠付きで除外でき、分析した場合は欠陥として再発防止策の実体を要する。規範の条文だけでは担当の判断で介入が「ユーザーの選好」などの理由で外れた観測（2026年9月24日の振り返り成果2件）があり、同じ経路を生成器の検査で閉じる。候補ごとの直接的原因、根本原因と既存規範が適用されなかった理由はレーンの計画ファイル（バグ）が持ち、生成器の入力へ重複させない。
...
`agent-toolkit:session-review`は起動元である当該セッション自身を対象とし、`agent-toolkit:completion-report`が全てのセッションの終了時に起動する。抽出はセッション内で行い、分析と対策だけを素材AWI経由で後続のレーンへ移す。抽出まで次のセッションへ移す形は、対象の探索と経路差を恒常的に保つため採用しない。

メインの作業完了報告は`agent-toolkit:completion-report`だけが所有する。同スキルは振り返り前の作業結果を`work-complete`段階、振り返り結果を`review-result`段階として別々に検査する。作業完了報告は検査直後に出力して同じ応答で振り返りを起動し、振り返り結果報告は準備スクリプトの終了後に、素材AWIのファイル名、候補件数の種別別内訳、所要時間及びメイン由来の改善点の件数を載せて出力する。2026年9月25日のユーザー指示により、元作業の完了通知を振り返りの待機より先に置く順序を採用した。`review-result`段階は`success`、`not-run`又は`failed`の状態を必須入力とし、成功時は要約行と対策として投入したWI、未実施時と失敗時は理由を固定見出しで検査する。同スキルは常に`agent-toolkit:session-review`を起動する。工程番号は各工程の入力が揃う順序を示し、振り返りの成果へ依存しない必須是正レーンは振り返りの稼働中も開始する。個別スキルは共通形式を持たない。

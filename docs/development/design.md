...
①ではメインがpickerを設定モデルへ直接委譲し、実装不要、既存実装で充足済み、reject、hold、計画型、通常型を分類する。
...
通常型のバッチでは、メインがpickerを設定モデルへ直接委譲する。
pickerは項目別の原文、実測、履歴及び投入元識別子を照合し、実装不要、既存実装で充足済み、reject、hold、計画型及び通常レーンを確定する。

...
同じディレクトリ配下の`results/<session_id>.json`へ、終端したsessionの結果本文を原子的に書く。内容へsessionごとに1から始まり新しいturnの開始ごとに1増える`turn_seq`と、終端時刻の`finalized_at`を含める。`wait`又は`kill`が結果本文を返した後も当該ファイルを保持し、終端結果の保持期限が到来した時点とMCPサーバーの終了時に削除する。同じsessionの新しいturnを開始する時点で、前のturnの結果ファイルを同期的に削除する。削除は`send_message`と再開の応答を返す前に完了させるため、応答を受け取った呼び出し側は`atk agents-wait <session_id>`だけで当該turnの終端を待てる。待機対象のturnの番号を引数で受け取る案は、呼び出し側が`turn_seq`を転記する必要を残し、転記の誤りが別のturnの結果を当該turnの終端として受領させるため採用しない。`atk agents-wait`は結果ファイルの検査と同じ周回で未回収の通知を回収し、MCPの`wait`と同じ条件で当該ファイルを削除する。回収の条件と削除を共通の実装へ置き、両経路が同じ通知を二重に配送しない。終端結果が未到達で通知だけを回収した場合は`status`を`running`として1行で返し、呼び出し側は再待機の要否を`status`で判定する。配送を削除の契機にする案は、呼び出し元の`wait`が終端と同時に本文を返すため、背景で待機する`atk agents-wait`が読取前のファイルを失うので採用しない。Claude Codeの`/goal`は、ターンを終えた時点で当該セッションが起動した未完了のBash背景ジョブが1件も無い場合に目標評価を発動するため、MCPツールの背景移行だけで待つと待機のたびに評価が動いてトークンを消費する。`atk agents-wait`を背景ジョブとして起動すると当該評価が延期され、委譲先の終端でセッションが再開する。結果本文の配送記録は`wait`が持つため、`atk agents-wait`は配送済みの標識を更新せず、呼び出し元が完了通知の後に`timeout=0`の`wait`を1回発行して配送を確定させる。MCPサーバーへ待機専用のツールを足す案は、当該ツールの呼び出しも背景移行の対象となり目標評価の延期条件を満たさないため採用しない。
...
実装工程へ進む各レーンは担当工程に必要な正本だけを受け取り、実行レビュー収束後の計画最終化、通常型AWIの計画型変換、最新ベースへのrebase、競合解消後の単一実行レビュー、メインから直列に許可されたffマージ、AWI行ごとの終端、所有資源の回収までを完了する。上流要求と対象リポジトリ側の要求が混在する項目は`adopt`の対象から除く。
採用する実施範囲を持たないレーンも、メインが起動する統合担当が計画レビュー後の計画証拠の保存、AWI行ごとの終端、非マージ用の所有資源回収までを完了する。メインは統合完了の固定値を公開契約の終了状態へ照合する。
...
振り返りの抽出担当へは、同じ対象リポジトリの未処理のキュー項目のファイル名と要約の一覧を渡す。抽出担当は候補が当該一覧と同じ事象を指す場合に該当ファイル名を添え、フックの警告通知だけを根拠とする候補を通知の種別ごとに1件へまとめる。既登録かどうかの照合をメインから抽出側へ移すことで、メインの判定を新しい事象へ限定する。抽出担当が原因、解決案、反映先及び採否を作成しない契約は維持し、候補の提示形式だけを変える。一覧を渡さずメインが候補ごとに照合する従来の形は、候補数に比例してメインのコンテキストを消費するため採用しない。
...
準備は`agent-toolkit/skills/session-review/scripts/session_review_prepare.py`が1回の実行で行い、抽出器の実在確認、記録の識別子、管理対象一時領域、観測境界、対象リポジトリ及び未処理項目の一覧を1つのJSONで返す。一覧の取得には`atk wi list --summary-only`が返すファイル名と要約だけのJSON Linesを用いる。準備が成立しない場合は非0の終了コードと不足した項目名を返し、メインが分析失敗として扱う。メインが`atk wi list --json`の出力を都度の整形処理へ繋ぐ形は、整形の実装がセッションごとに異なり失敗と書き直しの往復を生むため採用しない。
...
振り返りの対象を次のセッションへ渡す経路は、`atk session-review-queue`が対象リポジトリごとに保持する。`atk wi process-loop`は子セッションのセッションID、transcriptパス及びthread IDのいずれも取得しないため、常駐ループ側で引き継ぐ経路は成立しない。`claim`は未処理のセッションを返してから現在のセッションを同じ記録へ登録し、`done`が検収を終えたセッションを記録から除く。登録を先に行う形は、当該セッションが自身を振り返る対象になるため採用しない。取得と同時に記録から除く形は、振り返りを完了しないまま終了したセッションが失われるため採用しない。

...
各項目の`depends_on`は、当該項目より先に終端すべき実在するフィードバック又はTBDのファイル名を保持する。
依存先がTBDの場合は人間の回答待ちを、フィードバックの場合は先行成果の完了待ちを表す。
値は保存済みのメタデータを正本とし、本文の記述から依存を再構築しない。
無関係なactive TBDを状態遷移の一律条件にしない。
...
CLI（`atk mq add --batch`）とWeb UIの双方が同じ解析・書込み経路（`_atk_wi_batch.py`）を用いる。
...
SSEは`/api/events`（フィードバック）、`/api/plans/events`（計画ファイル）、`/api/sessions/events`（セッション）へ画面ごとに分けた。単一経路へまとめると、ある画面の更新が他の画面のクライアントにも再取得を促し、表示中でない画面の更新で不要な再読込が起きる。経路を分けることで、購読者集合と配信契機を画面ごとに閉じられる。却下した代替案は、単一のSSE経路で種別付きイベントを配信し、クライアント側で振り分ける案である。サーバー側の購読者集合が画面をまたいで1つになり、片方の画面の配信不良が他方へ波及するため採らない。

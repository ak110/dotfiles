...
別セッションを対象とする経路では、振り返りの全工程を1件のサブエージェントが担い、当該サブエージェントは他の委譲先を起動せず抽出器を直接実行する。1件が対象セッション全体を1回だけ解析するため、個別起動による分析対象の重複と欠落を防ぐ目的は維持される。準備、抽出担当の起動、返却の検収及び再照合をメインへ残す形は、公開工程から外した工程をそのまま選定工程のメインへ移すだけとなり、メインのコンテキストを圧迫するため採用しない。返却は候補数に比例するため、成果はメインが所有する管理対象一時領域のファイルへ書き、ツール戻り値には`status`、出力先の絶対パスと行数だけを返す。

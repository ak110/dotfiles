...
ファイル間の参照はメイン側からdetail側への片方向だけとし、detail側は逆参照フィールドを持たない。
...
各行は単位の目的、メイン側`## 実施内容`表の対象行、先行依存、統合順及び近接検証を対応付ける。
単一単位の場合も行を明示し、複数単位では各commitが独立して検証・レビュー・取り消しできる境界を定める。
構造検査は単位IDと統合順の連続性を確認する。
併せて先行依存の実在と、メイン側で`採用`又は`部分採用`とした実施内容行の過不足ない被覆を確認する。
読み取り互換の旧3列表では全実施内容行を被覆する。

計画担当は実装単位を設計し、構造検査は表の構文と被覆を判定する。

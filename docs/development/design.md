...
連続する実装単位のCodex担当は、単位ごとに再解決した`engine`、`model`、`effort`がすべて一致する場合だけ同じthreadを継続する。
...
起草担当とレビュー担当は同一対象リポジトリの複数ファイル名を1回の`show <filename>...`で取得し、
複数件表示の区切りや末尾のTBD一覧を各本文境界へ含めない。1ファイル担当の調査と警告・エラー後の再取得は単数形を使う。

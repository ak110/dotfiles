...
fork先で引数の展開が成立しない場合は、`agent-toolkit:shell-exec`が固定の`引数未受領`結果を返し、コマンドを実行しない。呼び出し元は通常の完了報告と実行結果を区別でき、推測したコマンドによる副作用を生じさせない。

...
advisorは各`completed`返却候補について、既定抽出の全利用者入力イベントID列と確認済みイベントID列の値・件数・順序を自己照合する。初回不一致ならadvisor内部で累積出力を1回だけ訂正して再照合し、一致なら`completed`、再不一致なら`evidence_insufficient`を返す。既定抽出失敗又はtranscript取得不能だけに限定した現行の証拠不足条件は、この内部再照合失敗を含む条件へ拡張する。

メインは初回`completed`の不一致時に不足・余剰イベントID列だけを同一advisor sessionへ返し、advisor内部訂正とは別の外部訂正を1回だけ求める。advisorは初回報告を含む累積出力全体を訂正し、完了前自己照合を適用して返却する。メインは訂正済み`completed`の問題一覧と確認済みイベントID列を再検収し、再不一致なら`evidence_insufficient`として既存経路へ終端する。advisorは完了前自己照合、各返却候補の内部1回訂正及び内部再照合の返却statusを所有し、メインは外部訂正要求の1回制限と受領後再検収の失敗終端を所有する。新しい永続状態又は別advisor sessionは追加しない。

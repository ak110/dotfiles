...
根本原因を成立させる各単位を検証済みの実装済み処置へ対応づける。同一対象リポジトリで`process-loop`が処理できる有効なactiveフィードバックも対応先にできる。

部分被覆では未被覆単位だけを候補とし、全単位の被覆を確認した場合だけ「提案無し」を許可する。

activeフィードバックを被覆に使う場合は、ファイル名、状態、対象リポジトリ、被覆単位、処理可能性及び非重複理由を報告へ残す。既存規範が機能しなかった事象では、規範の存在を有効性の根拠にしない。

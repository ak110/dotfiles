...
計画作成中の項目と外部編集中の項目を分けていた`planning`と`editing`は、自動処理から除外する点で`hold`と同じ振る舞いであり、状態集合の定義を型と操作ごとに分岐させていたため廃止した。役割は`hold`へ統合し、状態集合の定義を`_atk_wi_common.py`へ集約する。
...
Claude Agent SDKで開始する`agents_server`のセッションは、この設定だけでは意図した保持期間にならないため、`agent-toolkit/scripts/_agents_server_claude.py`の`_build_options`が環境変数で経路別に明示する。
...
CLI（`atk wi add --batch`）とWeb UIの双方が同じ解析・書込み経路（`_atk_wi_batch.py`）を用いる。
...
①はpickerが処理対象を確定した直後に、選定時点で`inbox`だった項目へ既存の一括`start-processing`を1回実行する。メインは出力の検収時に全件の`processing`配置を確認して①を完了する。遷移をpickerへ置くのは、対象集合を確定した主体と遷移させる主体を分けると、確定から遷移までの間に別主体の検収工程が入り、遷移前に②へ進む経路が残るためである。
...
対象リポジトリの変更では要求を満たせず上流リポジトリの改訂を要する要求を含む項目は、pickerが上流投入を要する項目として示し、投入先と上流要求を構造化してメインへ渡す。メインはレーンの起動前に、投入先と要求を固定入力として1件の委譲先へ渡し、投入させる。上流の改訂だけで成立する項目はレーンを起こさず、投入先のファイル名を元項目の`depends_on`へ加えて`inbox`へ戻す。対象リポジトリで実施できる要求を併せ持つ項目は分類を分けず通常のレーンへ渡す。レーンはffマージ後に所有資源を回収して固定の完了報告を返し、メインが元項目を`adopt`せず`inbox`へ戻す。上流投入をレーンの起動前へ置くのは、レーンの完了報告が固定文字列であり、上流の識別子と要求本文を載せる復路を持たないためである。1つの項目を要求ごとに異なる保存状態へ分けないのは、キュー項目の保存状態がファイル単位で1つに定まるためである。投入先と要求をメインが固定して渡すのは、投入先を自ら決める不可逆操作を委譲先が実行しないためである。元項目の`depends_on`更新と`inbox`への復帰はメインが保持する。
...
`_stop_gate.py`はCron作成後も継続中とするStop判定を正本として保持する。
...
Stopの登録は共通入口1件とする。
共通入口から、`agent-toolkit/scripts/_stop_gate.py`の`is_pending_async_work`による入力待ち判定と、`autonomous_exit.py`による常駐ループの`agent-toolkit:exit-session`呼び忘れを順に実行する。
...
これを実装の時点で検出するため、イベントごとの出力契約を`agent-toolkit/scripts/_hook_output_contract.py`へJSON Schemaで定義する。
`agent-toolkit/scripts/hook_output_contract_test.py`が`agent-toolkit/hooks/hooks.json`の全登録エントリの出力を当該契約へ照合する。
...
全レーンのffマージとレーンごとの`adopt`・資源回収後に、メインが1件の終端担当へ委譲し、版数・生成物同期、push、CI及び終端工程を1回だけ実行させる。

`agent-toolkit:process-wi`は全レーン、版数・生成物同期、push、CI及び固有終端工程の完了後に`agent-toolkit:completion-report`を起動する。`agent-toolkit:completion-report`は必須の`agent-toolkit:session-review`を実施し、必要な再公開の完了後、固定報告の前に`agent-toolkit:process-wi`が自動コードレビューを1回確認する。review本文は状態を問わず全てのPR・MRを対象に取得する。inline commentとthreadは未解決threadを持つ対象だけを取得する。監査自体が是正済みと対応不要のthreadを解決するため、未解決threadの有無で限定しても未処置の指摘を取りこぼさない。取得はいずれもpaginationの終端まで行い、確認時点の未対応指摘を是正するか、AWIへ記録する。review本文の取得は横断クエリーの数回で終わるのに対し、inline commentの取得はPR数に比例した照会を要するため、限定の対象を後者に置く。新しいレビューの到着を能動的に待機せず、session-reviewと並列実行しない。同一セッション是正で成果物を変更した場合は通常の公開工程を完遂し、その公開後にsession-review又は自動レビュー監査を繰り返さない。
...
振り返りの即時対応と自動コードレビュー監査の是正による再公開では、pushまでを当該セッションで確定し、CIの終端を待たない。委ねたCIは、次セッションの終端工程が初回の終端担当にベースbranchのHEADについて確認させることで回収する。次セッションのpushが無い場合も、当該HEADに対する確認は同じ経路で成立する。commitだけを行って公開を次セッションへ回す案は、ベースbranchへ未公開のcommitを残し、常駐ループの開始前更新が`git pull --rebase`で停止する経路を再び開くため採用しない。

終端担当はpushの完了後にベースbranchの公開状態を観測し、`base_branch_state`として返す。メインは固定報告の前に同じ項目を1回観測する。終端担当が返却した時点のOID一致だけを判定に用いると、作業ツリーの未コミット差分と中断状態が観測されず、以降の工程を経た後の状態も対象に入らないためである。解消できない場合はUWIへ引き継ぎ、観測を経ないまま完了報告へ到達させない。
...
原因commitに対応する計画契約がない場合は、新しい計画と計画レビューを作成せず、計画作業root直下の`ci-<起点OID>.exec-review.tsv`を用いる。最初の原因commit完全OIDを起点OIDとし、同じ再帰的CI失敗処理では原因commitが変わっても同じ表を継続する。`atk plans commit`は当該表だけを`private-notes/plans/ci/`へcommit・pushし、`atk plans checkout ci/ci-<起点OID>.exec-review.tsv`は保存実体を作業rootへ再取得する。再取得した表の検証後は同じ保存操作で作業側を回収し、private-notesのcleanを確認する。
...
UWI終端の判断基準は`agent-toolkit:bugfix`が正本とする。書式と機械検査は`agent-toolkit/scripts/_plan_format.py`、
...
構造の理由は、拒否の判定を`agent-toolkit/scripts/_atk_help.py`の`add_subcommands`が生成する全サブパーサーの既定クラスへ置き、サブコマンドごとに適用の有無が分かれる状態を表現できなくする点にある。argparseはサブパーサーが解釈できない引数をトップレベルの`parse_args`で報告するため、既定の動作ではトップレベルのusageだけが出て、当該サブコマンドが受理するオプションが出力へ入らない。
...
移設に伴い、配布物独立性（`agent-toolkit/`配下から`pytools/`を参照しない）を保つため、ファイルロックとwatchdogイベント判定はagent-toolkit側の実装へ置き換えた。private-notesの解決は`_atk_wi_common`の既存関数を用い、外部プロセスの起動を伴わない。作成日時インデックスのキャッシュディレクトリ名は統合前の名前を維持する。名前を変えると初回観測時刻が失われ、一覧の並び順が変わるためである。
...
リモートホスト側ヘルパーの起動コードは`_atk_serve_remote.remote_bootstrap`へ集約する。

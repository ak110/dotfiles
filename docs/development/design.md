...
レーンは1つ以上の計画ファイルと専用worktreeと実装担当で構成する並列実装の単位とし、異なるレーンだけを別worktreeで並列化する。
書込所有権の排他境界はworktree単位とするため、異なるレーンは変更ファイルが重複しても相互に待機せず、レーンごとの別worktreeで並列実装する。対象ファイルの重なりとベースコミットの一致はレーン割当の判定にだけ用いる。
...
計画担当は実施内容表へ担当項目集合の各項目を1行ずつ記録し、全計画の担当項目集合が合わせて計画対象集合を過不足なく被覆する。
...
全項目の可視性と個別判断を保ちながら、実施内容表では採用系の行だけを実装対象として扱える。
...
計画ファイルは、`<計画名>.md`（人間・メイン向け、要求と合意を確認する固定領域）と`<計画名>.detail.md`
（実装者向け、変更を再現する領域）の2ファイル構成とする。旧形式（単一ファイル）はこの2領域を同一ファイル内で
分け、読み取り互換として維持するが、新規作成では生成しない。
メイン側だけを読む読者（メイン、進捗確認）と、detail側だけを読む読者（実装担当・実装担当）を分け、
終端工程・進捗ログ・検証区分などレーンへ渡す必要のない情報をメイン側へ閉じ込める。
...
レビュー指摘は系統・ラウンド単位で変更履歴へ1行に集約し、指摘原文・個別採否・対応内容はレビュー指摘管理表を正本とする。
ファイル間の参照はメイン側からdetail側への片方向だけとする。
対応はファイル名のstem一致（`<stem>.md`と`<stem>.detail.md`）で確定し、他文書・frontmatter・完了報告・進捗参照が
示す「計画ファイル」のパスは常にメイン側パスを指す。
...
各行は単位の目的、先行依存、統合順及び近接検証を対応付ける。実施内容との対応は目的とファイル群別の変更説明で示す。
単一単位の場合も行を明示し、複数単位では各commitが独立して検証・レビュー・取り消しできる境界を定める。
構造検査は単位IDと統合順の連続性、先行依存の実在を確認する。旧6列表は読み取り互換として受理し、撤去した列の値を検査しない。

計画担当は実装単位を設計し、構造検査は表の構文を判定する。
`plan-impl-executor`は検査済みの単位を順に渡し、実装担当は割り当てられた行の目的と対象だけを実装する。
単位が未定義又は不正な場合は`needs_escalation`で呼び出し元へ返し、実装側で単位を推測しない。
人間・メイン向けの実施内容表へ単位IDを複製しないため、要求の正本と実装順序の知識境界を維持できる。

メイン側の実施内容表へ単位ID列を追加する案は、人間向けの要求と実装内部の順序を同じ表へ混在させるため採用しない。
単位が無い計画全体を1つのcommitとして扱う案は、設計漏れを実装時の暗黙判断で補い、独立した検証境界を失うため採用しない。

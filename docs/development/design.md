...
| 統合差分 | 統合差分レビュー | `implementation-review` | 統合後に生じた差分と直接影響範囲を計画照合後に評価する |
...
| レビュー表の系統値`implementation-review` | 実装レビュー及び統合差分レビューの単一レビュー |
...
`plan-impl-executor`は実装レビュー担当の二段階の候補を実測して統合し、レビュー担当は成果物を変更しない。
計画を保持する`plan-impl-executor`は、対応要否と応答欄の確定前にモード別の作業前OIDと承認済み計画から修正認可の上限を検査する。
レビュー指摘の永続表の構造と操作は`agent-toolkit/scripts/_review_table.py`を正本とする。
共通の表ライフサイクル、モデル解決と収束判定は
`agent-toolkit/skills/plan-mode/references/review-loop-coordination.md`へ集約する。
計画レビューは計画ごとの表へ`plan-review`を付け、実装レビューと統合差分レビューは1つの`review.tsv`へ`implementation-review`を付ける。
...
統合後にレーン内でレビュー済みの累積差分全体を再度レビューする案は、未変更部分へ同等のレビューを二重に課すため採用しない。
統合後に新しく作成したcommit（版数・manifest同期、CI修正、rebase競合の解消等）が振る舞い又は公開契約を変更する場合、
メインは`implementation-review`を起動し、初回は当該commitの変更ファイルと実際の消費先を対象とする。再レビューは前回レビュー対象OIDからの修正差分、
採用・未解決指摘及び修正担当が列挙した直接影響範囲に限定する。共有parser、共通状態、公開契約、生成元又は配布元を変更した場合は、実際の全消費先まで範囲を拡大する。
振る舞いも公開契約も変えない場合は、実装レビューを起動しない。判断に確信が持てない場合は起動する。

...
③の工程をメインが自ら実行する案は、CI修正レーンの起動と受領がメインのコンテキストを消費し、ユーザー判断への応答余地を狭めるため採用しない。

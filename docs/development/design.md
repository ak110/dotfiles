...
成果ファイルの6節と順序は受信側タスク文書の`## 出力`を意味契約の正本とする。`session_review_report.py`は同じ6節の骨格と候補判定・所要時間の表を生成し、律速区間、削減量、削減不能の根拠、未計測区間、抽出器のイベント種別と値を持つ構造化入力から、短縮後所要時間と180秒目標との比較を決定的に生成する。最終検査ではH2の集合と順序及び機械生成部分を同じ構造化入力から照合する。意味判断を担当が各節へ追記するため、ファイル全体の完全一致は要求しない。親文書へ節名を再列挙する案は契約の保持箇所を増やし、生成scriptを2節だけの正本とする案は担当が必要な節を正規経路で作成できないため採用しない。

...
通常レーンは1つの計画ファイルと1つの専用worktreeを持つ。`depends_on`だけを分割不能な成分とし、変更対象ファイル又は公開定義の重なりは直列統合費用として扱う。pickerは同一レーン案と分割案について、並列実装の最大時間と待機、rebase、競合解消及び再検査の直列時間を比較し、レーンごとの実装時間と統合時間を固定出力へ残す。

...
`last_action`は、assistantのテキスト出力ではその抜粋、ツール呼び出しではツール名、Codex backendでは進行中itemの種別を持つ。
...
却下した代替案は、`progress`だけを表示元として維持する案と、ツール呼び出しの引数とコマンド文字列を表示へ含める案である。
前者はテキスト出力の無い区間で説明欄が`label`のまま止まり、稼働しているかを判別できない。
後者は秘匿値を画面へ表示し得る一方、稼働の可視化という目的はツール名と経過時間で満たされる。

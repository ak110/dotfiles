...
素材AWIは、処理するレーンが元のセッション記録を開かずに判定と原因分析を完了できる本文とする。抽出器の個別証拠は、ユーザー介入、失敗したコマンドの診断、委譲返却、エスカレーションの本文を切り詰めず、hook通知など定型本文だけに上限を残す。記録位置（locator）、集約キー、証拠ファイルのパスは素材へ載せない。ユーザーコメントが、裏付けのためにログを探し直す契機になる細かい情報を書き過ぎないよう求めたためであり、準備時の一時領域は処理時に残らない。候補の参照には判定入力と対応付ける短い候補IDだけを使う。対象セッションの記録の所在（Claude Codeのtranscriptの絶対パス又はCodexのthread ID）は参考情報に載せ、素材で直接原因を確定できない候補に限って処理側が抽出器で照会し、記録に理由の本文が無い事象は再現で確かめる。会話圧縮は記録から区間を失わせないため、圧縮を原因未確定の根拠にしない（2026年9月26日、ユーザー回答）。素材AWIは1セッションにつき1件で観測事象がセッション固有であるため、`agent-toolkit:wi-standards`の条件付き重複判定、起草完了の条件及び投入前Challengerレビューの対象外とし、既存項目と重なるかは処理側が素材に載った一覧から確かめる。旧構成で振り返り担当の投入AWIを親が別の委譲先でレビューした工程は、担当の撤去に伴い消滅する。

素材AWIを処理したレーンは、候補ごとの問題、原因、対策、再発防止策及び見送りの理由をまとめた事後承認型UWIを項目ごとに1件投入してから素材AWIを終端する。素材AWIは投入時点で対策方針が確定していないため、2026年9月25日のユーザー要求により、ユーザーが項目単位で対策の可否を判断できる記録を残す。本文は`session_review_report.py`が素材AWIと判定JSON、分析JSONから決定的に生成し、候補集合の過不足、未判定の残存、除外の根拠、再発防止策の実体（実装済み成果物、未完了のAWI、確認中のUWI）の欠落を全件示して非0で終了する。ユーザー介入の候補は、単発の照会、判断を変えない追加要求、理由の無い拒否・中断の3区分だけを根拠付きで除外でき、分析した場合は欠陥として再発防止策の実体を要する。規範の条文だけでは担当の判断で介入が「ユーザーの選好」などの理由で外れた観測（2026年9月24日の振り返り成果2件）があり、同じ除外を生成器のチェックで防ぐ。候補ごとの直接的原因、根本原因と既存規範が適用されなかった理由はレーンの計画ファイル（バグ）が持ち、生成器の入力へ重複させない。
...
振り返りの候補は、候補が保持する位置の集合を保ったまま除外と集約だけで減らす。除外の対象は恒久対策の要否が記録の構造から定まる事象に限り、想定外事象を持たない成功の定型形式だけの委譲返却と、単一の検索が出力なしで一致0件を返した結果がこれに当たる。hookが遮断したツール呼び出しの失敗と、hook通知と同じ本文の警告は、hookの発生源標識又は通知本文との一致から出所を確定できる。このため両者をhook通知の候補として扱い、発生源と区分ごとの上限の対象にする。上限では件数の多い種類に加えてツールごとの最多の種類を残し、件数の少ないツールの遮断も候補に残す。反復注記とUUIDは原因を区別しないため、種類の本文から除く。自由記述の委譲返却や本文から原因が定まらない失敗など、本文の意味の判断を要する事象は候補に残す。2026年9月26日の処理回の記録には候補が202件あり、見送った候補の多くは成功の定型返却と、hookの遮断による失敗とhook警告の重複だった。ユーザーが候補の機械的な削減を求めたため、この規則を導入し、同じ記録の候補は100件になった。件数の上限だけで候補を減らす案は採らない。判定表が候補の位置の集合と一致することを求める検証と両立せず、対策へ結び付く候補も件数の順で除外するためである。
...
`agent-toolkit:session-review`は起動元である当該セッション自身を対象とし、`agent-toolkit:completion-report`が全てのセッションの終了時に起動する。抽出はセッション内で行い、分析と対策だけを素材AWI経由で後続のレーンへ移す。抽出まで次のセッションへ移す形は、対象の探索と、実行系ごとに異なる記録の取得手段を恒常的に抱えるため採用しない。

素材AWIは`atk wi process-loop`が次のセッションを起動する着手可能な項目になる。素材AWIだけを処理するセッションも終端で新しい素材AWIを投入するため、常に次のセッションへ渡すと常駐処理が待機へ入らない（2026年9月26日、ユーザー報告）。`agent-toolkit:process-wi`は公開工程の開始時に対象リポジトリの着手可能な他の項目を数え、0件なら振り返りを公開の前へ移し、投入された素材AWIを同じセッションの是正レーンで処理してから公開する。完了報告の振り返りは再起動しない。1件以上なら次のセッションが他の項目と一緒に素材AWIを処理するため、素材AWIのためだけのセッションは起動されない。公開の後に振り返りを実施したまま同じセッションで対策を実装する案は、終端担当を1回とする公開工程の外にcommitが残り、待機中は公開されないため採用しない。process-loopが素材AWIだけではセッションを起動しない案は、対策の完了を次の通常の項目まで遅らせ、ユーザーが示した「対策完了が遅れすぎる」懸念に反するため採用しない。公開前へ移した振り返りは公開工程と是正レーン自身を対象に含まない。

メインの作業完了報告は`agent-toolkit:completion-report`だけが所有する。同スキルは振り返り前の作業結果を`work-complete`段階、振り返り結果を`review-result`段階として別々にチェックする。作業完了報告はチェック直後に出力して同じ応答で振り返りを起動し、振り返り結果報告は準備スクリプトの終了後に、素材AWIのファイル名、候補件数の種別別内訳、所要時間及びメイン由来の改善点の件数を載せて出力する。2026年9月25日のユーザー指示により、元作業の完了通知を振り返りの待機より先に置く順序を採用した。`review-result`段階は`success`、`not-run`又は`failed`の状態を必須入力とし、成功時は要約行と対策として投入したWI、未実施時と失敗時は理由を固定見出しで持つかをチェックする。同スキルは常に`agent-toolkit:session-review`を起動する。工程番号は各工程の入力が揃う順序を示し、振り返りの成果へ依存しない必須是正レーンは振り返りの稼働中も開始する。個別スキルは共通形式を持たない。

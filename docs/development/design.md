...
`atk agents wait`の待機所有権は書込主体につき1件とし、`wait-locks/<書込主体>.lock`のファイルロックで表す。同じ書込主体から2つ目の待機が発行された場合は、書込主体、対象session群及び「先行する待機の終了後に再実行する」という次の操作を診断へ載せて終了コード8で終える。異なる書込主体の待機は互いに遮断しない。却下した代替案は、対象sessionごとにロックを取る案と、回収処理をCLIとMCPで別々に持つ案である。第1の案は、待機中に対象sessionが動的に増えるため、一部のsessionのロックだけを取得した状態が生じ、同じ書込主体の2つの待機が対象集合を分割して保持し得る。第2の案は、所有者照合と消費済み判定の正本が2箇所へ分かれ、片方の修正が他方へ及ばない。
...
開始時対象の待機ロックが競合した場合は終了コード8で復帰する。

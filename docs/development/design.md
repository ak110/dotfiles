...
初回実装担当と今回routeの実効`engine`、`model`及び`effort`がすべて一致し、両方の`engine`がCodexの場合だけ元の実装担当threadを継続する。いずれかの実効値が異なる場合を含むそれ以外は旧担当の終端確認後に今回routeで新規起動し、検収済み状態を開始前に1回だけ渡す。開始後は同じ書込担当が再判定からamendまでを完結する。

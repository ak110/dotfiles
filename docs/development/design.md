...
計画準拠系と独立系は、系統ごとに1つのTSVを全ラウンドで使い続ける。調整主体は初回strict検証の前に各表へ`atk review-table init <レビュー表>`を実行し、両系統の担当が終端するまで各表を分離する。
独立系へ計画準拠系の表や出力を渡さず、両系統の終端後に調整主体が各表を個別に全文読取して系統・ラウンドの帰属を保ったまま統合する。ラウンド列を先頭4列の複合キーへ含めることで、未解消の同じ指摘を別ラウンドの行として同じ表へ再提示できる。
...
書込担当は各fixup作成前、autosquash直前及びamend直前の各phaseで`agent-toolkit/skills/commit/references/history-rewrite.md`が定める汎用のプッシュ済み判定を再実行する。
同判定は`git fetch --all --prune`と`git for-each-ref --contains=<対象sha> refs/remotes/`を使う。
未pushかつ単一の書込担当が所有するworktreeの履歴書換え保護は同判定へ一本化し、remote広告refの直積証跡・replace ref・graft・
...
autosquash成功後は書込担当が`git rev-parse HEAD`で取得した書換え後HEADの完全OIDへautosquash成功後の2回目のpush済み判定対象を置換する。
...
起草担当（`agent-toolkit:plan-mode`）が調査と計画ファイル初版の起草までを担い、
...
- メイン兼任の維持: `plan-and-add-feedback`経路でメインが起草担当と調整主体を兼ねる現行構成は、
...
メイン側だけを読む読者（メイン、進捗確認）と、detail側だけを読む読者（実装担当・書込担当）を分け、

...
AWI処理は、①選定とレーン分け、②レーン実行、③全レーン後の終了に分ける。
...
①はpickerが処理対象を確定した直後に、選定時点で`inbox`だった項目へ既存の一括`start-processing`を1回実行する。メインは出力の検収時に、`decisions[].awi`の全件が`processing`にあることを確認して①を完了する。`start-processing`の引数集合と`decisions`のうち選定時点`inbox`の集合の一致は、引数を決めるpickerが実行前に検査する。比較集合を対象リポジトリの`processing`全件へ広げる案は、ユーザーが処理対象を明示した処理回で当該回の対象外の`processing`項目が残り、正常な処理回を停止させるため採用しない。遷移をpickerへ置くのは、対象集合を確定した主体と遷移させる主体を分けると、確定から遷移までの間に別主体の検収工程が入り、遷移前に②へ進む経路が残るためである。
...
②ではメインが通常レーンの計画起草、計画レビュー、実装単位の実装、実行レビューの各担当を`agents_server`で直接起動し、工程遷移と同一threadの継続を自ら制御する。変更の作成は実装担当が担う。
...
①の手順は`${CLAUDE_PLUGIN_ROOT}/share/pick-wi.parent.md`へ集約し、本体には目的と不変条件だけを残す。
①でメインが読む資料を1件にするのは、pickerの起動までに読む資料の件数が選定の固定コストを直接押し上げるためである。
...
③ではメインが1件の終端担当へ版数・生成物同期、push、CI、必要なCI修正、AWI固有の終端工程及び延期`adopt`を委譲し、受領した固定形式の値を公開契約の終了状態へ照合する。`agent-toolkit:session-review`と自動コードレビュー監査は①で起動し、完了報告と終了とあわせてメインが保持する。全体レビューは追加しない。
...
③の工程をメインが自ら実行する案は、終端担当が自ら行うCI修正を含む工程全体がメインのコンテキストを消費し、ユーザー判断への応答余地を狭めるため採用しない。
...
`agent-toolkit:session-review`の初期分析は、起動時の`session_review_model`で選んだ通常の読み取り専用サブエージェントだけが担う。サブエージェントはセッション全体から問題候補と再取得可能な証拠位置を列挙し、原因、対策、反映先及び採否を決めない。メインは列挙証拠だけを再取得し、問題があれば`agent-toolkit:bugfix`で原因と恒久対策を確定する。全量分析をメインでも繰り返す案と専用advisor定義は、同じ母集団の二重処理になるため採用しない。振り返りが読む記録の量は常駐セッション本体が扱う量と異なるため、`orchestrate_model`を流用せず工程別モデル設定へ専用キーを置く。既定値は他の工程別キーと同じ`codex:gpt-5.6-sol/medium`とする。メインは問題候補ごとの判定記録を作成し、欠陥と判定した候補へ処置が対応することを振り返りの終端前に確認する。既存規範の存在だけを処置として扱う分類は、規範がある状態で発生した事象の再発経路を閉じないため採用しない。
...
抽出担当の起動は、振り返り用の参照文書の読込と所要時間目標の解決より前に置く。当該2工程は抽出担当へ渡さないメイン専用の入力であり、抽出担当の稼働と並行して進められる。

初回返却後のユーザー発話の再照合は、抽出器の`--user-events`のローカル実行が担う。当該照会は照合済み境界と再照合境界の区間にあるメイン記録のユーザー発話イベントを、記録名と行番号を伴って返す。抽出担当へ`send_message`と`wait`で往復する形は、決定的なコマンドの出力だけで確定する照会に委譲先の推論と待機を要するため採用しない。

抽出担当の返却は、既存キュー項目と重なる候補ごとに判定の根拠となる本文の該当箇所を逐語で含める。判定だけを返す形は、メインが採否を確定するために同じ本文を取得し直すため採用しない。
...
抽出器は、通常表示、警告、集計及びhook通知の走査を1回の記録読み込みでまとめて行う集約実行を持つ。集約実行は走査ごとの全量を呼び出し側が指定したディレクトリ配下のファイルへ書き、標準出力へは走査ごとの要約だけを返す。要約の範囲は、走査ごとの出力先と件数、通常表示のイベント種別ごとの件数、問題候補の特定に用いるイベントの位置と本文の冒頭、警告の分類ごとの件数、集計済みの走査の全量とする。知識境界として、どの走査を要約へ含めるかは抽出器が持ち、抽出担当は要約だけで候補を確定できない場合に全量ファイルを読む。走査ごとに実行を分ける従来の形は、抽出担当の実行ターン数が走査の数だけ増え、通常表示の全量が標準出力を経由して抽出担当のコンテキストへ入るため採用しない。抽出担当が全量から必要な集計を都度作成する形も、同じ集計を毎回作成し直す工程が加わるため採用しない。
...
`agent-toolkit:process-wi`は、当該セッション自身ではなく前のセッションを振り返りの対象とする。対象セッションが終了済みであるため計測範囲の終端が確定し、解析中に増えるユーザー発話の再照合と`--elapsed-until`の受け渡しが不要になる。③の直列区間から振り返りと即時対応が外れ、次のセッションでは①及び②と並行して実行するため、移送した工程が新たな直列区間を生じさせない。振り返りを当該セッションの③に残す形は、終端担当の起動までの待ち時間を伸ばすため採用しない。

別セッションを対象とする経路では、振り返りの全工程を1件のサブエージェントが担い、当該サブエージェントは他の委譲先を起動せず抽出器を直接実行する。1件が対象セッション全体を1回だけ解析するため、個別起動による分析対象の重複と欠落を防ぐ目的は維持される。準備、抽出担当の起動、返却の検収及び再照合をメインへ残す形は、③から外した工程をそのまま①のメインへ移すだけとなり、メインのコンテキストを圧迫するため採用しない。返却は候補数に比例するため、成果はメインが所有する管理対象一時領域のファイルへ書き、ツール戻り値には`status`、出力先の絶対パスと行数だけを返す。

振り返りの対象は、起動された`agent-toolkit:process-wi`のセッションが保存済みのセッション記録から特定する。`atk wi process-loop`は子セッションのセッションID、transcriptパス及びthread IDのいずれも取得しないため、常駐ループ側で引き継ぐ経路は成立しない。`atk session-review-target`は、Claude CodeとCodexの記録のうち対象リポジトリで動いた自身以外の本体セッションを選び、更新時刻が最新の1件を返す。本体の判定は、Claude Codeでは最初の`user`レコードの`entrypoint`が`cli`であること、Codexでは先頭の`session_meta`レコードの`payload.originator`が`codex-tui`であることとする。可用性判定の記録と委譲先の記録はこの判定で除かれる。実行系の別を選定条件にしないため、常駐ループが実行系を切り替えても同じ探索で対象を特定できる。対象リポジトリごとに引き継ぎ記録を保持する形は、取得と完了の状態遷移を保つ手順を要するうえ、起動された側の探索1回で同じ対象を特定できるため採用しない。
...
前のセッションを対象とする振り返り、即時対応と自動コードレビュー監査の処置は、①の開始時から②と並行して進め、③の開始より前に完了する。全レーンのffマージとレーンごとの`adopt`・資源回収後に、メインは1件の終端担当へ委譲し、版数・生成物同期、統合後検証、push、CI及び終端工程を1回だけ実行させる。

計画ファイル（メイン）`## 検証区分`の`統合後検証`が挙げる検証は、レーンの専用worktreeで成立しない検証を含むため、終端担当が主作業ツリーでpushの前に1回実行する。
メインは②のマージ許可より前に各レーンの当該行を記録し、計画最終化で計画ファイルが移った後も値を保持する。
失敗した場合は統合後検証失敗記録でCI修正と同じ経路へ進み、pushとCIの往復を経ずに修正する。
各レーンで実行させる案は、レーンで成立しない検証を含み、統合後の変更集合を対象にできないため採用しない。

`agent-toolkit:process-wi`は①の開始時に前のセッションを対象とする`agent-toolkit:session-review`を起動し、振り返りの全即時対応を1件の委譲先で完了する。自動コードレビューも①の開始時に1回確認し、確認時点の未対応指摘を是正するか、AWIへ記録する。review本文は状態を問わず全てのPR・MRを対象に取得する。inline commentとthreadは未解決threadを持つ対象だけを取得する。監査自体が是正済みと対応不要のthreadを解決するため、未解決threadの有無で限定しても未処置の指摘を取りこぼさない。取得はいずれもpaginationの終端まで行う。review本文の取得は横断クエリーの数回で終わるのに対し、inline commentの取得はPR数に比例した照会を要するため、限定の対象を後者に置く。新しいレビューの到着を能動的に待機しない。前のセッションが公開したPull Requestのレビューは次のセッションの①で対象へ入るため、session-reviewとの実行順による限定を設けない。

状態を問わず全件を毎回取得する案は、Pull Request数に比例するinline commentの照会を削減できないため採用しない。

終端担当の完了後に`agent-toolkit:completion-report`を起動する。同スキルは①で完了した`agent-toolkit:session-review`を再実施せず、固有成果と振り返り結果を1回だけ固定報告し、その後に`agent-toolkit:exit-session`を起動する。各工程は同じ完了本文を再生成しない。

終端担当の起動後に生じた是正commitはローカルのベースbranchへ保持し、終端担当を再起動しない。次のセッションの③は初回の終端担当で当該commitを公開し、ベースbranchのHEADについてCIを確認する。
...
公開対象が1つも無い処理回では、終端担当を起動せずに③を短絡する。処理対象、回答済みUWI、固有の終端工程、延期`adopt`及びレーンの成果がいずれも無い場合、終端担当が実行する版数更新、生成物同期、push、CI確認及び固有の終端工程はどれも対象を持たない。短絡の可否は、終端担当がpush後に観測するのと同じ4項目をメインが取得して判定する。判定項目を別に定義する案は、公開状態の検収基準を二重に持ち、片方だけが更新される状態を生むため採用しない。

手動で起動した`agent-toolkit:process-wi`では、pickerが固定したAWIの完成条件に操作、対象及び外部可視の結果が明示された終端工程を、当該セッションの承認スコープとして①で保持する。計画担当と終端担当が同じ範囲を要求する場合は再承認を求めず、範囲が広がる差分だけを確認経路へ送る。手動起動の指示は当該固定集合の完遂を委任する指示であり、同じ範囲の承認を工程ごとに繰り返す経路はユーザーの操作を増やすだけで認可の範囲を変えないためである。自動常駐起動を同じ扱いにする案は、起動時点でユーザーが対象集合を確認していないため採用しない。
...
各画面の非同期処理は、共通シェルが発行するマウント世代を内包した継続判定を入口で受け取る。待機後にDOMへ到達し得る処理は、この継続判定の`wait`だけを経由して待機する。`wait`は開始元のマウントが失効した成功・失敗を呼び出し側へ返さないため、各呼び出し側が個別の判定を置かなくても`catch`と`finally`を含む後続処理を一括して遮断する。画面内で複数のプレビュー取得が競合する場合は、計画ファイル画面の`previewGeneration`で新しい取得だけを残す。画面間と画面内の世代を分けることで、画面遷移時の後始末と同一画面内の取得順序を独立して判定できる。
...
3画面のヘッダー、ナビゲーション、ボタン及びモバイル時の折り返しは`shell.css`を単一の正本とする。画面固有のCSSは、当該画面のコンテンツ整列に従属する水平方向の余白だけを上書きし、高さ、フォントサイズ、内側の余白とブレークポイントを上書きしない。3画面を遷移しても操作対象の大きさと位置が変わらない状態を保つためである。画面ごとに寸法を上書きする案は、共通側の変更が各画面の上書きで打ち消され、同じ部品の見え方が再び分かれるため採らない。
...
画面別CSSは`body[data-screen="<画面名>"]`で限定し、取得した全てのスタイルシートを常時有効に保つ。共通シェルは遷移時にスタイルシートの`disabled`を切り替えない。切替方式は、遷移のたびにスタイル計算の対象集合を入れ替え、利用者が画面の切り替わりを不連続な描画として観測するため採らない。あわせて共通シェルは`document.body`のクラス属性を一括代入せず、画面HTMLが持つクラス集合だけを差分で入れ替える。一括代入は画面スクリプトが実行時に付けたクラスを遷移のたびに失わせるためである。先読みが済んでいない画面へ遷移する区間は、共通シェルが`document.body`へ`data-navigating`を付けて進行表示の対象とし、取得の成否が確定した時点で外す。無表示のまま待つ案は、利用者が操作の受理を判別できないため採らない。

共通シェルの画面登録契約は`mount`と`unmount`に加えて任意の`prefetch`を受け取る。`prefetch`は画面DOMを表示しないまま当該画面の初期表示に必要なサーバー応答を取得し、画面スクリプトの内側へ保持する。共通シェルは初期表示の画面をマウントした後の先読みで、資産の取得に続けて当該画面の`prefetch`を1回だけ呼ぶ。以後の`mount`は保持済みの応答があればそれで即時に描画し、続けて通常の取得を1回行って内容を最新化する。目的は、セッション画面の初回表示で一覧の取得を待つ区間を、利用者が他の画面を操作している間へ移すことである。知識境界として、どの応答を先読みするかは各画面スクリプトが持ち、共通シェルは`prefetch`の有無と呼び出し回数だけを扱う。`prefetch`を必須にする案は、先読みする応答を持たない画面へ空の実装を置かせるため採らない。共通シェルが画面別のAPIを直接呼ぶ案も、画面の初期表示に必要な要求の集合を共通シェルが知る必要が生じるため採らない。

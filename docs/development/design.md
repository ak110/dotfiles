...
起動文が絶対パスで示す入力のうち、起動時点で作成済みのものは起動側が起動前に実在を確認する。委譲先が受領した値から自ら解決して読むplugin配下の資源も、同じ時点で起動側が確認する。委譲先のタスク文書は、当該実在を着手前に再確認して差し戻す手順を持たない。欠落の検知点を起動側へ一本化することで、委譲先の起動、規範と対象の再読、呼び出し元の再判断を重複して費やす往復が生じない。起動側の当該検査は起動側が自ら行うものであり、解決した絶対パスを委譲先へ渡して実行させない。委譲先が当該資源を自ら解決する手順は、plugin導入版の更新時に古いパスを使わないために維持する。起動側へ移すのは絶対パスの実在という機械的に検査できる判定に限る。担当種別ごとに異なる必須入力の完備と、起動側が組み立てる契約の各条項の充足は、起動側が個別の検査手順を持たない限り検知できないため、当該入力を消費する委譲先の受領時の判定として残す。Codex互換レイヤーが自ホストのplugin導入情報から起点を確定する処理の失敗も、起動側から観測できないため差し戻しを残す。

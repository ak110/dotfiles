...
解析と保存後の本文照合は`decode_entry_text`で復号とLF正規化を一括して行う。
...
知識境界として、復号と改行正規化は`_atk_wi_frontmatter.py`の`decode_entry_text`が持つ。
...
一致判定が不一致を示した場合だけ、`atk review-table show`の`--format=jsonl`が返す復号済みの値で差異を特定する。

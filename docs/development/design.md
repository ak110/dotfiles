...
フィードバックの型と保存状態は直交する。`normal`はレビュー済み計画が未関連、`plan`はレビュー済み計画が関連することを表し、保存状態は処理主体による所有段階を表す。自然言語要件からは新しい`inbox(plan)`を作成し、既存の`inbox(normal)`は競合防止の`planning(normal)`を経て同じ項目を`inbox(plan)`へ変換する。全ての実装要求を計画と計画レビューへ通す。
...
`ready`と`blocked`は保存状態ではなく、依存、TBD回答、cooldown、metadata及び計画の有効性から導出する着手可否である。TBD回答待ちは`hold`へ移さず、元項目を`inbox`のままTBDへ依存させて`blocked`とする。回答保存後はTBDを先に終端し、依存解除後に元項目を次の処理対象へ戻す。
...
ユーザーコメントへ外部操作、対象及び範囲が明記されていれば、その要求単位の範囲で利用者認可として扱う。source値又は予約節の存在だけでは認可にしない。
...
定期再確認を装着する主体は、当該セッションに適用される定期報告、cooldown解除、期限監視などの経過時間起動の義務を列挙し、経過を実測するコマンドと判定閾値を定期promptへ含める。該当する義務が無い場合は含めない。可変な作業状態は既存の正本から読み直し、promptへ写さない。
遠隔ホスト側の`nohup`／`setsid`による切り離しは完了通知経路を持たないため、実行主体自身が応答不能になる工程だけに用いる。それ以外は、`run_in_background=true`でSSH接続を保持するか、対象を限定した前景実行を用いる。
既存のCron周期を短縮する案は完了通知経路の欠落を解消せず全待機へ固定費を加えるため採用しない。正当な切り離しと誤った切り離しはコマンドの形だけでは区別できるとは限らないため、hook／parserによる自動コマンド検査も追加しない。既存の定時起動と文書間契約テストだけで接続を検証し、新しいschedulerや永続状態を設けない。
...
source欠落又は`human`だけを本文の既定で人間由来とし、`plan`を含むその他の値を既定でエージェント由来とする。計画、ユーザーコメント、TBD回答及び出所付き対話回答に記録した明示由来を要求単位で優先する。
人間由来要求の不採用候補は通常の確認経路へ送る。回答を得られない場合はTBDを保存して対象項目を`inbox`かつ`blocked`で保持する。
...
この境界では項目単位の永続的な由来スキーマを新設せず、既存のsourceと要求単位の明示由来を判定に用いる。
人間由来の本文又は回答へ外部操作、対象及び範囲が明記されている場合だけ、その要求単位を利用者認可として扱う。
...
各自動投入経路は呼出元が指定したsourceを`agent-toolkit:feedback-standards`へ渡し、同スキルの投入・保存照合契約を適用する。
ユーザーが手動起動した投入だけは`agent-toolkit:add-feedback`を入口とし、sourceを受領していない場合は値を推測しない。
...
`plan-impl-executor`はレビュー修正方針を確定する前に、最初の実装担当の起動前のレーンの完全OIDにある公開契約へ承認済み変更を合成し、修正認可の上限を確定する。
後続節で再採用済みの論点と計画目的へ帰属して必要な追加ファイルは許容し、
対応付け不能、計画間衝突、認可上限超過のいずれかは実装担当へ渡さず呼び出し元へ返す。
...
投入元識別子を本文全体の既定由来に用いる。計画、ユーザーコメント、TBD回答、出所付き対話回答の明示由来を要求単位で優先する。人間由来入力へ操作、対象、範囲が明記された場合だけ、その範囲を利用者認可に用いる。
...
`--index`は既定抽出と同じ境界・イベント順序を用い、各イベントについて`kind`、`sequence`、存在する場合の`line`だけをJSONLで返す。
advisorは本文を含む既定抽出を問題発見、全利用者入力の点検、完了前自己照合に使用する。
メインはadvisorが`completed`を返した場合だけ同じ抽出器の`--index`を1回実行し、全`kind=user`イベントから作成した`(sequence, line)`列で確認済みイベントID一覧の全件被覆と出現順を構造検収する。
問題一覧の証拠位置は`--detail`、`--stats`、`--warn`又は`--hook-notices`の完全なqueryと`event_index`だけを保持し、既定抽出を問題locatorに使用しない。
メインは問題一覧が参照するdistinctな完全queryを同じ引数列と順序で各1回だけ再実行する。
この分離により、advisorの発見に必要な本文を維持しながら、メインの構造検収へ本文を再送しない。

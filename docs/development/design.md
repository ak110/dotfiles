...
処理中に確定した必須是正を稼働レーンへ追加する場合は、現在のturnへの事前通知と、工程境界後の開始指示を分離する。`delivery: steered`は通知のキュー投入だけを示すため開始確認に使わない。メインは現在の工程境界とturnの終端を受領した後、同じthreadへ開始指示を送り、`delivery: reply_started`で追加作業の開始を確定する。開始前にレーンが終端した場合は公開応答から同じthreadの再開可否を判定し、再開不能な場合だけ新しいレーンを作成する。現在の工程境界を受領する前に新しいworktreeを作成しない。
...
画面の実装では、同じ対象HEADで独立した実描画レビューと実行レビューを並列に始める。実描画レビューは実際の描画と操作を確認し、実行レビューは差分と提出された検証記録を照合する。両者の観測は互いの完了を入力に必要としないため、修正後も同じHEADで並列に再判定し、統合前に両方の収束を確認する。使いやすさの判断基準は`writing-standards`のUI/UX資料と`review-standards`へ置き、画面レビューの親子文書は起動と返却の経路だけを持つ。

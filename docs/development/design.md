...
レビュー指摘の永続表は重大度、指摘箇所、指摘内容、対応要否、対応内容、対応不要理由の固定6列TSVとする。
`指摘内容`へ根拠と適用条件を記録し、`対応要否`に応じて後半3列を更新する。
表の更新は`_file_lock.py`の既存排他制御を使用するため、指定された管理対象領域の検収ではレビュー表と`atk review-table`が付随して作成するファイルだけを許容し、TSVの6列構造・行数を個別に確認する。
計画準拠系と独立系は、各ラウンドでも別々のTSVを使う。調整主体は初回strict検証の前に各表へ`atk review-table init <レビュー表>`を実行し、両系統の担当が終端するまで各表を分離する。
独立系へ計画準拠系の表や出力を渡さず、両系統の終端後に調整主体が各表を個別に全文読取して系統・ラウンドの帰属を保ったまま統合する。同じ先頭3列の複合キーを次ラウンドで再提示できるよう、ラウンド間で表を再利用しない。
通常の実装モードでは、呼び出し元が実装レビュー用managed temp領域を作成し、その絶対パスを`plan-impl-executor`へ渡す。各レビュー担当の新規起動と同じレビュー担当への継続接続のいずれでも、今回の系統・ラウンド表、同じ系統の過去全ラウンド表をラウンド順で渡す。

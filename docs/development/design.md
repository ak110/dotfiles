...
移設に伴い、配布物独立性（`agent-toolkit/`配下から`pytools/`を参照しない）を保つため、ファイルロックとwatchdogイベント判定はagent-toolkit側の実装へ置き換えた。private-notesの解決は`_atk_mq_common`の既存関数を用い、外部プロセスの起動を伴わない。作成日時インデックスのキャッシュディレクトリ名は統合前の名前を維持する。名前を変えると初回観測時刻が失われ、一覧の並び順が変わるためである。

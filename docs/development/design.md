...
準備は`agent-toolkit/skills/session-review/scripts/session_review_prepare.py`が1回の実行で行い、抽出器の実在確認、記録の識別子、管理対象一時領域、観測境界及び対象リポジトリを1つのJSONで返す。同スクリプトはキューと振り返り用の参照文書を解決しない。準備が成立しない場合は非0の終了コードと不足した項目名を返し、メインが分析失敗として扱う。

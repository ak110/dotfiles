...
成果ファイルの6節と順序は受信側タスク文書の`## 出力`を意味契約の正本とする。`session_review_report.py`は同じ6節の骨格と候補判定・所要時間の表を生成し、最終検査ではH2の集合と順序及び機械生成部分を照合する。意味判断を担当が各節へ追記するため、ファイル全体の完全一致は要求しない。親文書へ節名を再列挙する案は契約の保持箇所を増やし、生成scriptを2節だけの正本とする案は担当が必要な節を正規経路で作成できないため採用しない。

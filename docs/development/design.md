...
pickerの有効出力を受領したメインは、選定結果と処理開始を検証した時点でレーン起動へ進む。並行している前セッション振り返りと監査の終端を当該遷移の条件にせず、受領済みのpickerを再び待機せず、再起動もしない。振り返りと監査は公開工程の開始だけをゲートし、picker出力が不正な場合だけ同じpickerへ再取得を依頼する。
...
観測していない場合は当該呼び出しを通しつつ、当該文書の絶対パスを警告本文へ示す。
SessionStartは委譲されていないセッションに限り、質問前に目的、選択肢、推奨案、影響範囲を確認する短いチェックリストを注入する。
単純な確認質問まで全文読解の再実行で遮断せず、判断を伴う場面では規範が要求する全文読解を維持するためである。
...
読取手段を当該1経路へ限るのは、警告の抑止条件を、全文読解の成立を機械的に判定できる観測だけで構成するためである。
シェル経由の読取と部分読取は、パスの言及と全文への到達を区別できないため観測としない。
この限定により、シェル経由で読解した主体には当該文書を`Read`で読み直すまで警告が続く。
...
2つ目は、警告も設けず規範の条文だけで全文読解を求める案である。
読解の観測結果を実行主体へ伝えられず、判断を伴う場面での見落としを検出できないため採らない。
...
前のセッションを対象とする振り返り、即時対応と自動コードレビュー監査の処置は、選定工程の開始時からレーン工程と並行して進め、公開工程の開始より前に完了する。pickerの有効出力受領後の選定検証とレーン起動は、これらの終端を待たずに進める。全レーンのffマージとレーンごとの`adopt`・資源回収後に、メインは1件の終端担当へ委譲し、版数・生成物同期、統合後検証、push、CI及び終端工程を1回だけ実行させる。
...
新規計画は、実行レビュー完了まで`~/.claude/plans/<stem>.md`を計画作業root直下で更新する。stemは起動経路ごとに`dd-HHmm_process-wi_レーン<番号>`、`dd-HHmm_fast-process-wi`、`dd-HHmm_<名詞>`とし、衝突時だけ作成処理が`-<小文字16進数4桁>`を付ける。

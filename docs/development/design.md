...
`agent-toolkit:session-review`は、起動時の`session_review_model`で選んだ通常のサブエージェント1件が、証拠の走査から原因、対策及び採否の確定までを担う。メインは当該担当が書いた成果ファイルを読んで検収し、成果ファイルが示すlocatorが指す証拠だけを再取得する。走査と分析を別の主体へ分ける形は、同じ記録を親子で重ねて読み、返却本文の量を候補の件数へ比例させるため採用しない。全量分析をメインでも繰り返す案と専用advisor定義は、同じ母集団の二重処理になるため採用しない。振り返りが読む記録の量は常駐セッション本体が扱う量と異なるため、`orchestrate_model`を流用せず工程別モデル設定へ専用キーを置く。既定値は他の工程別キーと同じ`codex:gpt-5.6-sol/medium`とする。振り返り担当は問題候補ごとの判定記録を作成し、欠陥と判定した候補へ処置が対応することを振り返りの終端前に確認する。既存規範の存在だけを処置として扱う分類は、規範がある状態で発生した事象の再発経路を閉じないため採用しない。判定記録は列挙した介入、権限拒否、明示的エスカレーション、未解決証拠のlocator集合と表のlocator集合を一致させる形で確定し、成果の地の文での言及を行への対応として扱わない。`処置`は成果物への修正と再発防止策を判別できる形で分け、当該事象が生じた成果物の書き換えを再発防止策に数えない。処置の分離を検収するために列を増やす案と独立した台帳を設ける案は、既存の6列表と介入の全件一覧で同じ検収が成立するため採用しない。

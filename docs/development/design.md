...
重複抑止の判定前に、メインは`atk mq list --status=active --type=feedback --target-repo=<repo-path> --skip-pull`を一度だけ実行する。`planning`・`processing`配置を含む対象リポジトリの唯一のactive feedback一覧を取得する。取得した各項目について、状態が`active`、対象リポジトリが同一であることを検証する。この一覧は既存規範又はactiveなフィードバックとの重複抑止だけに用い、被覆候補の判定へ流用しない。

被覆候補の判定前に、メインは`atk mq list --status=processable --type=feedback --target-repo=<repo-path> --skip-pull`を一度だけ実行する。対象リポジトリのprocessable feedback一覧を取得する。その結果から状態が`inbox`又は`processing`で、表示上の判定が`ready`である項目だけを`process-loop`による被覆候補に用いる。TBD、planning、editing、hold及びreadyでない項目は被覆候補に含めない。
...
根本原因を成立させる各単位は、現行実装・テスト若しくは反復しない実測により有効性を確認した実装済み処置へ対応づける。被覆候補一覧に含まれる、同一対象リポジトリで`process-loop`が処理できる有効なprocessableでreadyのfeedbackも対応先にできる。

processableでreadyのfeedbackを被覆とする条件は、単なる処理可能性だけではない。フィードバック本文または対応計画が、対応づける各根本原因単位と、その単位に必要な是正処置・横展開処置・再発防止処置の全てを覆う根拠を確認する。

部分被覆では未被覆単位だけを候補とし、全単位の被覆を確認した場合だけ「提案無し」を許可する。

processableでreadyのfeedbackを被覆に使う場合は、ファイル名、状態、対象リポジトリ及び被覆単位を報告へ残す。本文または対応計画が各根本原因単位と必要な処置の全てを覆う根拠、処理可能性及び非重複理由も報告へ残す。既存規範が機能しなかった事象では、規範の存在を有効性の根拠にしない。

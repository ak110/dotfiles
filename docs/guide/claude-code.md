...
言語別ルール・計画モード・バグ対応・Claude設定記述ガイドがプラグイン側のスキルに移行されているため、プラグイン導入が前提となる。
...
現在のrules側配布対象は `agent.md` と `markdown.md` の2ファイルのみ。
言語別・場面特化型の指示は `agent-toolkit` プラグインのスキルへ移行済み。

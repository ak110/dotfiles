...
- プラグイン（`agent-toolkit/` 配下）— 本リポジトリ自体をClaude Codeのプラグインマーケットプレイスとして登録することで配布する
...
`agent-toolkit` プラグインの配布経路は以下の2系統。
...
実行時に `agent-toolkit` プラグインをGitHub型marketplaceからuser scopeへインストール・更新し、

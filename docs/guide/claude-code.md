...
配布先は配布元と常に完全一致するよう同期する（ステージングディレクトリへ全ファイルをダウンロードしてから原子的リネームで差し替える）。
...
実行時に `agent-toolkit` プラグインをuser scopeへインストール・更新し、併せて旧 `edit-guardrails` プラグインを自動アンインストールする。
...
配布元ディレクトリがSSOT。`claudize` とchezmoi経由の配布はディレクトリを丸ごと同期するため、ファイル追加・削除・リネーム時の追加作業は不要。
`install-claude.sh` / `install-claude.ps1` のみファイルリスト配列（`FILES` / `$files`）に手動で追従する必要がある（ワンライナー依存を減らすためGitHub APIは使わない）。

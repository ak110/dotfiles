...
ローカルまたは外部のプラグインを実際に追加または更新した場合と、公開インストーラーの
`codex plugin add`が成功した場合に限り、次の再起動コマンドを案内する。
...
マーケットプレイスの登録だけが変化した場合、プラグイン追加前の処理または追加自体が失敗した場合、
外部プラグインが導入済みの場合は、再起動案内を表示しない。

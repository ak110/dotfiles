...
- `spec-driven` — 軽量SDD（Spec-Driven Development）ワークフロー。
  ...
  - ステップ2はplan modeを1回出入りして計画ファイル1本（大枠方針と実装詳細を集約）を作成する。codexレビュー合格後に`ExitPlanMode`し、計画ファイルの判断経緯を開発中ドキュメントへ転記してから実装する
  - ドキュメント規約: 恒常配置は機能単位（`docs/features/{機能名}.md`・`docs/topics/{トピック名}.md`）、開発中配置は作業テーマ単位（`docs/v{next}/{作業テーマ名}.md`、例: `SSO追加.md`）。両者は粒度が異なりうるため、昇格時に`spec-driven-promote`で恒常配置の該当機能ファイルへマージ統合する。進行中の作業メモは`docs/v{next}/{作業テーマ名}.working.md`（一時、実装完了時に削除）。バージョン全体の概要と目次は`docs/v{next}/README.md`（昇格時に役目を終える）。`{next}`は次期リリースのバージョン番号でステップ1で確定する（末尾の`.0`は省略。例: `3.0.0`→`v3`、`1.2.0`→`v1.2`）。ここに記した具体パスは既定値で、プロジェクトのドキュメントで別の配置が規定されている場合はそちらに従う
  - 調査は`spec-researcher`、実装は`spec-implementer`の各サブエージェントへ分業し、メインセッションのコンテキスト消費を抑える
  - レビューは`spec-reviewer`（仕様適合性・ドキュメント整合性・計画ファイルからの転記漏れ・粒度差対応の整合）と`code-quality-reviewer`（コード品質）を作業テーマ単位で直列起動する。全タスク完了・最終反映・format/lint/test合格のあとに走らせる

...
- 計画ファイルは`<計画名>.md`（メイン側。人間・メインエージェント向け）と`<計画名>.detail.md`
  （detail側。実装者向け）の2ファイル構成とする。メイン側は`概要`、`実施内容`、`提示素材`、`変更履歴`、
  `検証区分`、`終端工程`、`進捗ログ`を、detail側は`恒久化・リファクタリング内容`、`実装資料`、`完了条件`
  を固定H2として検査する。バグ対応ではdetail側の先頭に`バグ調査結果`を置く。自由な見出しは`実装資料`配下のH3以下だけを許容する。
  バグ調査の本体は`~/.claude/plans/<計画stem>.bugs.md`へ分離し、計画から参照する。
  改訂前に作成済みの旧形式（単一ファイル9節）は読み取り互換として受理するが新規作成では生成しない。
  新旧の判別は対応する`<stem>.detail.md`ファイルの実在で行う
- `概要`直下の計画メタ情報では起動経路、対象リポジトリ、作業種別、ベースコミット、
  detail側ファイル名を示す`実装詳細`の5項目と記法を検査し、
  ベースコミットは対象リポジトリのcommitとして解決できること、`実装詳細`の値がstem導出値と
  一致することも検査する
- 変更履歴の5列表、進捗ログの3列表、提示素材の素材表・要求表、合意表の素材・要求参照を検査する。
  実施内容表は4列とし、採否6値と採否別の根拠規則を検査する。
  旧形式の提示素材・旧3列表・本文内のバグ調査表を読み取り互換として受理した場合はwarningを出力する。

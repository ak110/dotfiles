...
- `styles.md` — 構造と順序・言語と文体・日本語の表記ルール・コメントとドキュメント・コマンドラインオプションを
  まとめた記述スタイル指針（無条件ロード）
...
- `~/.claude/plans/*.md`編集後に計画ファイルの構成
 （必須H2の欠損・順序違反・予期せぬH2・`変更履歴`の末尾以外配置）を検査して警告する
...
- `gitlab-ci-usage` — `.gitlab-ci.yml`編集時のキーワード仕様・典型パターン・lint手段・
  トラブルシューティング観点の参照リファレンス。
  キーワードの網羅的な仕様は公式ドキュメントをWebFetchして参照する導線のみを持ち、
  スキル本体は代表URL表と典型パターンのスニペットに絞ってコンテキストを節約する
- `spec-driven` — 軽量SDD（Spec-Driven Development）ワークフロー。
  起動コマンド: `/spec-driven`（自動補完で`/agent-toolkit:spec-driven`になる）
  - 想定ユースケース: 大規模コードベースへの機能追加・既存機能改修で、
    検討漏れ・デグレードを抑え、設計判断を恒常配置ドキュメントに蓄積したい場合
  - Setup&Explore→Plan&Implement→Cleanupの3ステップで進行する。
    ステップ終端にユーザー確認ゲートは置かず、判断が必要な場面は都度ユーザーに確認する
  ...
  - ドキュメント規約（開発中配置側）: 開発中配置は作業テーマ単位で`docs/v{next}/{作業テーマ名}.md`に置く（例: `SSO追加.md`）。
    バージョン全体の概要と目次は`docs/v{next}/README.md`（昇格時に役目を終える）。
    既存改修時は作業版`.md`内の「改修前スナップショット」節に恒常配置の改修前該当節を貼り付け、
    Before/After検証材料とする（昇格時は破棄）
  - `{next}`は次期リリースのバージョン番号でステップ1で確定する（末尾の`.0`は省略。例: `3.0.0`→`v3`、`1.2.0`→`v1.2`）。
    ここに記した具体パスは既定値で、プロジェクトのドキュメントで別の配置が規定されている場合はそちらに従う
...
- `export-for-resume` — 議論が二転三転して会話のまとまりが失われた時に、
  現時点の決定事項・未決事項・現在の進捗を整理したmarkdownを作業プロジェクトルート直下へ書き出すスキル。
  起動コマンド: `/export-for-resume`（自動補完で`/agent-toolkit:export-for-resume`になる）
  - 想定ユースケース: コンテキストが肥大化して応答品質や速度が低下してきた時、
    またはセッション終了直前に現状スナップショットだけ残して後日再開したい時
  - `disable-model-invocation: true`を指定しており、モデルによる自動起動は発生しない。
    ユーザーが明示的にスキルを呼んだ時のみ動作する
  - 出力ファイル（`resume-<YYYYMMDD-HHmmss>.md`）を添えて新しいClaude Codeセッションを立ち上げ直し、
    冒頭でそのファイルを読ませて作業を再開する運用を想定する

一部のスキルは`/<skill-name>`形式で明示的に呼び出せる。
現在は`/tidy-unpushed-commits`・`/pyfltr-usage`・`/pytilpack-usage`・`/gitlab-ci-usage`・
`/spec-driven`・`/export-for-resume`を提供している。
...
- `spec-driven-init` — 既存プロジェクトにspec-drivenを導入するための初回整備スキル。
  ...
  - 想定ユースケース: 既存のソースコード・ドキュメント・既存`docs/v*/`群から、
    `docs/features/`・`docs/topics/`配下の恒常配置の初版を起こす（これらのパスは既定値で、プロジェクト規約で上書き可能）
...
- `spec-driven-promote` — 開発中バージョンディレクトリの作業版を恒常配置へマージ統合するスキル。
  ...
  - 想定ユースケース: リリース完了後のひと段落した時点で、`docs/v{next}/`配下のドキュメントを
    `docs/features/`・`docs/topics/`へ統合し、開発中バージョンディレクトリを削除する。
    上記のパスは既定値で、プロジェクト規約で上書き可能
  - 作業テーマと恒常配置の粒度差（1対多・多対1・1対1）を踏まえ、
    節別統合方針（追記・置換・併記の使い分け）でdiff提示→ユーザー合意→統合適用の半自動フローで進める

...
- 計画ファイルは`目的`、`実装契約`、`完了条件`、`進捗ログ`を検索用アンカーとして使い、順序変更と追加H2を許容する
- `実装契約`直下の計画メタ情報では対象リポジトリ、完全長のベースコミット、作業種別を検査する
- 対象ファイル一覧は通常箇条書きをSSOTとし、空、重複、危険パス、基準コミット上の状態矛盾を検査する
...
- `/plan-mode`: 計画ファイル作成と、実装後の二系統レビューを含む実行引き継ぎ
  - 計画確定時は4つの意味アンカーと最小メタ情報、対象一覧、参照実在を機械検査する
  - バグ対応計画は計画メタ情報の固定記法から判定する。4原因区分、原因起点の類似見直し、
    是正・横展開・再発防止を固定表ではなく意味契約として総合レビューする

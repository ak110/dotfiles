...
- 計画ファイルの`### 計画メタ情報`が存在するとき、ベースコミットの記載が無い場合と値が完全長のコミットハッシュでない場合にブロック
- 計画ファイルの作業種別が固定記法`- 作業種別: <固定値>`から外れている場合、または固定値が
  `バグ対応`・`通常変更`以外の場合に警告。固定記法のコロンはASCIIの`:`、コロン後は半角空白1字とする
...
- `/plan-mode`: 計画ファイル作成と、実装後の二系統レビューを含む実行引き継ぎ
  ...
  - バグ対応計画は計画メタ情報の固定記法から判定する。4原因区分、原因起点の類似見直し、
    是正・横展開・再発防止の順序と表全体の欠落を、バグ調査結果表の構造warningと総合レビューで検査する。
    調査結果節は文書全体で1件だけとし、その親H2が`## 背景`であることも構造warningで検査する

...
- `spec-driven` — 軽量SDD（Spec-Driven Development）ワークフロー。
  ...
  - ステップ1の成果物はリサーチ結果ファイル群のみに絞る（骨子・`.working.md`・`README.md`エントリは作らない）
  - ステップ2前半冒頭で`EnterPlanMode`を明示的に呼び出し、計画ファイル1本（大枠方針と実装詳細を集約）を作成してcodexレビュー合格まで回す
  - `ExitPlanMode`の直後に`spec-designer`を呼び出し、開発中`.md`骨子・`.working.md`初版・`README.md`エントリを一括生成させる。メインはその後TaskCreate登録と`spec-implementer`順次起動に進む
  ...
  - ドキュメント規約（開発中配置側）: 開発中配置は作業テーマ単位で`docs/v{next}/{作業テーマ名}.md`に置く（例: `SSO追加.md`）。作業メモは`docs/v{next}/{作業テーマ名}.working.md`（一時、実装完了時に削除）、バージョン全体の概要と目次は`docs/v{next}/README.md`（昇格時に役目を終える）
  - `{next}`は次期リリースのバージョン番号でステップ1で確定する（末尾の`.0`は省略。例: `3.0.0`→`v3`、`1.2.0`→`v1.2`）。ここに記した具体パスは既定値で、プロジェクトのドキュメントで別の配置が規定されている場合はそちらに従う
  - サブエージェント一時ファイルのサフィックス命名規則
    ...
    - レビュー指摘ファイルは差し戻しループで上書きされる。一時ファイル群はいずれもCleanupステップで`.working.md`とともに削除する
  - 調査は`spec-researcher`、設計ドキュメント立ち上げは`spec-designer`、実装は`spec-implementer`の各サブエージェントへ分業し、メインセッションのコンテキスト消費を抑える
  - レビューは`spec-reviewer`（仕様適合性・ドキュメント整合性・計画ファイルからの転記漏れ・粒度差対応の整合）と`code-quality-reviewer`（コード品質）を作業テーマ単位で直列起動する。全タスク完了・最終反映・format/lint/test合格のあとに走らせる。両レビュアーは指摘を上記サフィックス付きファイルへ書き出し、戻り値は判定・件数に絞ることでメインのコンテキスト消費を抑える
...
現在は`/tidy-unpushed-commits`・`/pyfltr-usage`・`/pytilpack-usage`・`/gitlab-ci-usage`・`/spec-driven`を提供している。
...
- `spec-driven-promote` — 開発中バージョンディレクトリの作業版を恒常配置へマージ統合するスキル。
  ...
  - 想定ユースケース: リリース完了後のひと段落した時点で、`docs/v{next}/`配下のドキュメントを`docs/features/`・`docs/topics/`へ統合し、開発中ディレクトリを削除する。上記のパスは既定値で、プロジェクト規約で上書き可能

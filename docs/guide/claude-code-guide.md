...
`agent-toolkit:fast-process-wi`では`dd-HHmm_fast-process-wi.md`とし、同じ名前が既にある場合だけ`-<小文字16進数4桁>`が付く。process-wiのレーン名は作成処理へ`lane-NN`を渡して生成し、呼び出し側で日付、時刻又は接頭辞を組み立てない。

...
| --- | --- | --- | --- |
| plugin `PreToolUse/pretooluse` | 編集内容とコマンドの事前検査。文字化け・他言語文字の混入・LF改行のみの`.ps1`書き込み・lockfileやシークレットの直接編集・codexサンドボックス指定の弱体化をブロックし、口語表現・ホーム絶対パスの混入・自動生成manifestの手編集を警告する。ユーザーが直接読む質問本文と計画本文にも同じ本文検査を適用し、当該2ツールでは口語表現も遮断し、あわせて誤字の辞書による警告を返す。`judgment-details.md`の読取を同一セッションで観測していない場合の`AskUserQuestion`の発行もブロックする。Bashでは`uv run python`の誤用、パターン一致によるプロセス終了、`sleep`直後の状態確認連結と除外指定の無い再帰`grep`を同一セッション内の再検出でブロックする。全量観測が必要なコマンド出力の単純な切り詰めは初回だけ全量保存へ補正し、同一セッションの2回目は`start_explore`又は`start_shell`への分離を要求して遮断する。同じ終了コードの連続失敗後は、成功した`start_shell`まで直接Bashを遮断する。高容量のユーザー領域を無限定に対象とする再帰検索・状態を変更するコマンドの出力を限定する指定と他コマンドとの連結・ヘルプ取得と同じ実行ファイルの実行の並置・version未更新・未検証コミット・一括ステージ・`codex exec`前の未決事項を警告し、`git log`へ`--decorate`を自動挿入する | 対応 | 部分対応。編集検査は口語表現・文字化け・他言語文字・ホーム絶対パス・lockfile・シークレット・manifest・否定規定表現・サンドボックス保護に対応する。`.ps1`改行、frontmatter同期注記と本文節参照の実在検証はpatch入力から判定できないため非対応。ユーザーが直接読む本文の検査は、対応する入力を持たないため非対応。Bash検査は現在の入力とcwdだけで判定するものと、編集成功状態による一括ステージ警告に対応する。コマンドの終了コードを取得できないため、`git log`確認・amend後の状態・検証実行・連続失敗ゲートに依存する検査は非対応 |
...
| plugin `PostToolUseFailure/posttooluse` | Bashの同一終了コードの連続失敗を記録し、2回目に警告と次回直接実行の遮断状態を設定する。中断と分類不能な失敗は連続性を解除し、他ツールの失敗は状態を変更しない | 対応 | 非対応。対応するイベントが存在しない |
...
`agent-toolkit:process-wi`のレーンでは`dd-HHmm_process-wi_レーン<番号>.md`、`agent-toolkit:fast-process-wi`では`dd-HHmm_fast-process-wi.md`とし、同じ名前が既にある場合だけ`-<小文字16進数4桁>`が付く。
...
- `agent-toolkit:add-awi`: 利用者向け要件を対話で確定し、AWI又はUWIを手動投入する

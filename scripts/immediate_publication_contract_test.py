...
_REQUIRED_CLAUSES = (
    (
        "development",
        "ユーザーが局所変更の即時公開を明示した場合だけ",
    ),
    (
        "process",
        "局所変更を現在のセッションで即時に公開することを明示した場合だけ",
    ),
    (
        "process",
        "即時修正と同じ要求は登録対象の外に置く。",
    ...
    (
        "finish",
        "`検証・CI方針: 即時対応`と、成功した近接検査のコマンド・終了コード、後続処置AWIのファイル名又は`なし`を渡す。",
    ),
    (
        "finish",
    ...
    (
        "finish",
        "省略した全体検査、未確定のCI、run URL及び後続処置AWIをcompletion-reportへ含める。",
    ),
    (
        "parent",
    ...
    (
        ...
        "`terminal_steps`が省略した全体検査と後続処置AWIを挙げる",
    ),
    (
        "subagent",
    ...
    (
        "subagent",
        "受領した近接検証結果が終了コード0を示すことを確認する。後続処置AWIは登録済みのファイル名又は`なし`として受理する。",
    ),
    (
        "subagent",
        "複数の書込主体の成果を統合した後にだけ成立する検査は、全体検査をCIへ委ねる判定と`即時対応`による省略の"

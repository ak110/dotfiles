...
def _main() -> int:
    ...
    if has_pyfltr:
        sections.append(
            ...
            "Start with the heading '## pyfltr改善提案' and list each item as "
            "'- <対象ファイル> — <提案内容>'. If none, write '指摘無し' under the same heading. "
    ...
    if has_agent_toolkit:
        sections.append(
            ...
            "`~/.claude/rules/agent-toolkit/`. Start with the heading '## agent-toolkit改善提案' "

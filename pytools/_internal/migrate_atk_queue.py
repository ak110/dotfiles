"""エージェント停止中にagent-toolkitの旧計画ファイルを移行する。"""
...
def run() -> bool:
    ...
    if result is None or result.returncode != 0:
        logger.warning(log_format.format_status(_TAG, f"計画の移行に失敗: {claude_common.format_cli_error(result)}"))
        return False
    return result.stdout.strip() != _PLAN_NO_CHANGE

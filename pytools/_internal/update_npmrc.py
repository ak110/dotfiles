...
def run(npmrc_path: Path | None = None) -> bool:
    ...
    if not path.exists():
        ...
        logger.info(log_format.format_status(short, f"作成し {_LINE} を設定しました"))
        return True
    ...
    if _PATTERN.search(content):
        ...
        if new_content == content:
            logger.info(log_format.format_status(short, f"{_LINE} は既に設定済み"))
            return False
        path.write_text(new_content, encoding="utf-8")
        logger.info(log_format.format_status(short, f"{_KEY} を更新しました"))
        return True

    # キーが無い: 末尾に追記 (末尾改行を保証)
    suffix = "" if content.endswith("\n") or content == "" else "\n"
    path.write_text(content + suffix + _LINE + "\n", encoding="utf-8")
    logger.info(log_format.format_status(short, f"{_LINE} を追加しました"))
    return True

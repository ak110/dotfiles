...
class TestRunFlow:
    ...
    def test_already_installed_version_drift_updates(self, monkeypatch: pytest.MonkeyPatch):
        ...
        refresh_index = next(
            i for i, c in enumerate(calls) if command_matches(c, ["claude", "plugin", "marketplace", "update"])
        )
        update_index = next(i for i, c in enumerate(calls) if command_matches(c, ["claude", "plugin", "update"]))
    ...
    def test_project_scope_ignored_in_version_check(self, monkeypatch: pytest.MonkeyPatch, tmp_path: pathlib.Path):
        ...
        uninstall_calls = [(cmd, cwd) for cmd, cwd in calls if command_matches(cmd, ["claude", "plugin", "uninstall"])]
        matched = [

...
description: >
  対象リポジトリの未処理フィードバックとTBD回答済み項目が0件になるまで、
...
# agent-toolkit:session-review・agent-toolkit:exit-sessionを組み合わせて呼び出す。
# dotfiles-fb process-loop CLIとの使い分け:
# - CLI版: dotfiles-fb process-loopは本スキルを単発の`claude ... /process-feedbacks-loop <path>`起動で内部呼び出しする
# - 本スキル版: 単一のClaude Codeセッション内で完結し、完了後にagent-toolkit:exit-sessionでセッション自体を終了する
---

# フィードバック消化ループ

手動操作なしでフィードバック消化と振り返り投入を反復する。
...
3. 件数が1以上の場合、`Agent`ツール（`subagent_type: claude`）で1反復分の処理を単独foreground委譲する。
   ...
   - apply-feedbackが計画分割した場合は、各計画を並列サブエージェント
     （`Agent`ツール・`subagent_type: claude`）へ個別foreground委譲すること
   - apply-feedback配下のplan-mode委譲では全工程を遵守すること
    （工程2〜8。工程2は「2.5」「2.6」「2.7」を含む）
...
ステップ3のサマリー報告直後に`agent-toolkit:exit-session`スキルを呼び出し、
Claude Codeセッションを終了する。
本ステップは0件到達による正常完遂時のみ実施し、
ユーザー明示中断・処理失敗中断・サブエージェント異常終了で反復が止まった場合は実施しない。
...
本スキルはapply-feedbackを直接起動しない（process-feedbacks経由でapply-feedbackへ委譲される）。

...
description: >
  ...
  process-feedbacksによる消化とsession-review-dotfilesによる振り返りの投入を単一Claude Codeセッション内で自動的に繰り返す。
...
1. `dotfiles-fb count --target-repo=<対象リポジトリ>`でinbox件数を取得する。
2. 件数が0の場合はステップ3へ進む。
...
反復本体を`Agent`ツール委譲へ切り替える案は不採用とする。
サブエージェントは`git push`および`dotfiles-fb adopt`/`reject`など不可逆な外部公開操作を実行しない規範
（`agent-toolkit/rules/claude-code.md`）と衝突するため。

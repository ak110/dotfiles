...
- 作業中プロジェクトに応じた改善提案章のプロジェクトドキュメント章への統合
  ...
  - dotfilesプロジェクトで作業中の場合はagent-toolkit改善提案章を統合する
...
各提案は`### <提案要約>`見出し配下に「理由」「反映先」「反映内容」を箇条書きで記述する。
...
- `--source=session-review`を付与することでfrontmatterに`source: session-review`が記録され、`apply-feedback`側で振り返り由来と判別できる

複数の対象リポジトリがある場合はリポジトリごとに呼び出す。

```sh
dotfiles-fb add --source=session-review <repo-path> <message1> [<message2> ...]
```

...
description: >
  ...
  推奨ガイド・共有ファイルへの追従作業でも使う。個人プロジェクトのフィードバック処理への着手時、
...
- 同期対象マトリクスで波及先とした各プロジェクト宛の未処理フィードバックを`atk mq list`と`atk mq grep`で照会し、実装レビューで確定した
...
比較の結果として上流吸収を選ぶ場合は、`agent-toolkit:add-feedback`で上流リポジトリ宛のフィードバックを1件投入する。
...
- 推奨ガイドの改訂を要する変更を`~/pyfltr`以外のリポジトリで確定した場合は、先に`~/pyfltr`向けのフィードバックを
  投入し、他プロジェクト向け追従提案の`depends_on`へそのファイル名を記録する。
  典拠のないままの追従着手による整合の崩れを防ぐためである

## 個人プロジェクト着手時の依存更新

利用者からの要求又は採用済みのフィードバックによる明示的な更新要求が一括更新コマンドの生成範囲に属する場合は、
...
明示的な更新要求がない個人プロジェクトのフィードバック処理では、対象プロジェクトのロックファイルの

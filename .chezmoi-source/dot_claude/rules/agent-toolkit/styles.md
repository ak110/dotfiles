...
成果物には必ず対象読者がいる。読者カテゴリーごとに想定知識・粒度・敬体常体の選択を切り替える。

登場人物と文体の関係は次の図および表で俯瞰する。
...
flowchart LR
    ...
    Agent -->|発話・常体・JIS・日本語| Dev
```

| カテゴリー | 文書／出力の例 | 読み手 | 文体 |
| --- | --- | --- | --- |
| 対話型UI向け | CLI案内・GUIダイアログ | エンドユーザー | 敬体・NHKスタイル |
| エンドユーザー向けドキュメント | README・利用ガイド | エンドユーザー | 常体・JIS規格スタイル |
| 開発者向けドキュメント | アーキテクチャ・CONTRIBUTING | 開発者 | 常体・JIS規格スタイル |
| LLMエージェント向け文書 | `CLAUDE.md`・`SKILL.md` | LLMエージェント | 常体・JIS規格スタイル |
| LLMエージェントの発話 | 応答本文・拡張思考 | 開発者 | 常体・JIS規格スタイル・日本語 |

- 対話型UI向け（敬体）: GUIのダイアログ、CLIの案内、エラー画面の説明文、デスクトップ通知などのリアルタイム応答メッセージ
  - 想定知識: 当該ツールの内部実装は知らない。専門用語は初出時に補足する
  - 粒度: 操作・状態・対処を端的に伝える。
    エラーメッセージは原因と対処を一組として示し、利用者が次の行動を判断できる情報量を保つ
  - 文体: 敬体（「〜します」「〜してください」）を基本とし、NHK案内放送原稿のスタイルに揃える
  - 例外: LLMに直接渡るhook出力（`reason`・`additionalContext`・exit 2のstderr等）は
...
- LLMエージェントの発話（常体）: Claude Code等の対話エージェントがユーザーへ返す応答本文と拡張思考（thinking）
  - 想定読者: 開発者（人間）。
    ただし応答本文と拡張思考はコンテキストへ残り後続生成の文体・言語へ影響するため、
    敬体や英語混在を避けて統制する
  - 粒度: ユーザーが進捗・選択判断・成果物の状況を把握できる情報量を保つ
  - 文体: 常体を基本とし、JIS規格・公的な標準仕様書のスタイルに揃える。日本語で記述する
  - 例外: なし。対話型UI向け（敬体）には該当しない
- エンドユーザー向けドキュメント（常体）: README、利用ガイド、`docs/guide/`配下、ユーザーマニュアルなど
  - 想定知識: 当該ツールの内部実装は知らない。専門用語は初出時に補足する
  - 粒度:「何ができるか」「どう使うか」を中心に、利用者の操作目線で書く
  - 文体: 常体（「〜する」「〜である」）を基本とし、JIS規格・公的な標準仕様書のスタイルに揃える
- 開発者向けドキュメント（常体）: アーキテクチャ文書、CONTRIBUTING、コードコメントなど
  - 想定知識: プロジェクトの前提知識を共有する開発者を想定し、初歩の説明は省く
  - 粒度:「なぜそう設計したか」「どこに注意が必要か」を中心に書く
  - 文体: 常体を基本とし、JIS規格・公的な標準仕様書のスタイルに揃える
- LLMエージェント向け文書（常体）: `CLAUDE.md`・`.claude/rules/`配下・`SKILL.md`・サブエージェント定義・hooks関連ファイルなど
  - 想定知識: エージェントは指示を読んで実行するため、前置き・自己紹介・書き手目線の解説は不要
  - 粒度: 判断に必要な情報のみを直接記述する。冗長な説明はトリガー精度の低下とコンテキスト消費を招く
  - 文体: 常体を基本とし、JIS規格・公的な標準仕様書のスタイルに揃える。エージェントへの直接指示として書く

...
- ユーザーに計画を提示する前に、codexコマンドで計画をレビューすること。具体的な使い方は以下の通り。
    ...
    set -o pipefail && codex exec --dangerously-bypass-approvals-and-sandbox --cd "{project_directory}" --output-last-message "{plan_full_path}.review.md" \
      "{plan_full_path} このプランをレビューして。些末な点への指摘は不要。致命的かつ本質的な問題のみ指摘すること。疑いレベルの指摘はせず、十分に調査したうえで確実に問題だと判断できるものだけを報告すること。" \
      2>&1 | grep "^session id:"
    ...
    codex exec resume --dangerously-bypass-approvals-and-sandbox --output-last-message "{plan_full_path}.review.md" {SESSION_ID} \
      "{plan_full_path} プランを更新したのでレビューして。些末な点への指摘は不要。致命的かつ本質的な問題のみ指摘すること。疑いレベルの指摘はせず、十分に調査したうえで確実に問題だと判断できるものだけを報告すること。"
    ```

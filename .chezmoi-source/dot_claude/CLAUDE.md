...
7. `## レビュー履歴`（2回目以降のみ）— 各回のcodex指摘と対応または見解を箇条書きで追記し、差分レビューを支援する

コンテキスト節約と整合性の確保のため次を守る。

- codexレビューに備えて前提条件・ユーザーの意向などを十分に記述しておく
...
- Contextや全体方針で抽象的な宣言文（「機能的挙動は変えず」「最小差分で」など）を書く場合、実装詳細と矛盾しないかを必ず確認する。先回りで縮退的制約を課すと本質的な改善を阻害することがあるため、断言は裏付けのある範囲に留める

...
- 調査と検証では標準エラーを保持する。接続先とコンテナー内のスクリプトにも適用する。`2>/dev/null`で破棄すると、終了コードだけでは原因を判定しにくい。内容が事前に確定し診断を要しない出力だけを破棄する。出力を比較、計数、パスの取り込みなどの機械処理へ渡すときは、標準エラーを別のファイルか端末へ分け、`2>&1`でデータの流れへ合流させない。合流させるとエラー文が比較や計数の入力になり、意味のない結果になる。出力から取り込むパスは、生成元の終了コード、空でない集合、全パスの実在を確かめる。

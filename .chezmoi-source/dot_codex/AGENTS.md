...
- 既存パスはユーザー入力、基準となる文書（計画・WI・規範文書など）、直前の出力、読了済み文書から解決する。構成から推測したパスは`rg --files`か`find`で実体を探し、出力から取り込むパスは標準エラーを分けて生成元の終了コード、空でない集合、全パスの実在を確かめる。外部コマンドへは絶対パスを渡し、作業ディレクトリを暗黙に使うCLIは対象のworktreeで起動する。内容検索の手段は対象のGit管理境界から`agent-toolkit:search`で選ぶ。誤ったパスは別の対象への変更につながる。

...
- 同コマンドでも、ファイルの作成・変更・削除、大量ファイルの走査・変換、外部ネットワークアクセス、依存関係の追加削除、認証情報へのアクセスを伴う場合は事前に確認する
- `node`・`ruby`・`bash -c`・`sh -c`のようにコード実行やワンライナー評価を伴うコマンドは、実行前に「何をするか」「何を読むまたは書くか」「何を確認したいか」を日本語で説明する
